...
<%if strcd_unidade <> "" Then
	...
	<tr bgcolor="<%=atividade%>">
		...
		<td><select name="cd_contacorrente">
						<option value="" SELECTED></option>
						<%xsql = "SELECT * FROM TBL_Banco"
						set rs_cc = dbconn.execute(xsql)
							while not rs_cc.EOF
							cd_contacorrente = rs_cc("cd_codigo")
							nm_banco =  rs_cc("nm_banco")
							nm_agengia =  rs_cc("nm_agengia")
							nm_conta_corrente =  rs_cc("nm_conta_corrente")%>
								<option value="<%=cd_contacorrente%>" <%if cd_contacorrente = cd_contacorrente then%>SELECTED<%end if%>><%=rs_cc("cd_contacorrente")%></option>
							<%rs_cc.movenext
							wend%>
			</select></td>
	</tr>
