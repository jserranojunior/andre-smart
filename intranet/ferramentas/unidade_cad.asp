...
<%if strcd_unidade <> "" Then
		...
		if not rs_unid.EOF then
			...
			nm_endereco = rs_unid("nm_endereco")
				if nm_endereco <> "" then
					...
					sep6 = instr(1,nm_endereco,"]",1)
					
					
					nm_rua = mid(nm_endereco,1,sep1-1)
					nm_no = mid(nm_endereco,sep1+1,sep2-sep1-1)
					nm_complemento = mid(nm_endereco,sep2+1,sep3-sep2-1)
					
					nm_cep = mid(nm_endereco,sep3+1,((sep4-sep3))-1)
					'	sepcep = instr(1,nm_cep,"-",1)
					'	if nm_cep <> "" Then
					'		nm_cep1 = mid(nm_cep,1,sepcep-1)
					'		nm_cep2 = mid(nm_cep,sepcep+1,len(nm_cep)-sepcep)
					'	end if
					'nm_bairro = mid(nm_endereco,sep4+1,((sep5-sep4))-1)
					'nm_cidade = mid(nm_endereco,sep5+1,((sep6-sep5))-1)
					'nm_estado = mid(nm_endereco,sep6+1,((len(nm_endereco)-sep6)))
				end if
	...
	<tr bgcolor="<%=atividade%>">
	    <td>&nbsp; Complemento:</td>
	    <td colspan="3"><input type="text" name="nm_complemento" size="51" value="<%=nm_complemento%>" class="inputs"></td>
	</tr>
	<tr bgcolor="<%=atividade%>">
	    <td>&nbsp; CEP:</td>
	...
	<tr bgcolor="<%=atividade%>">
	    ...
	    <td><input type="text" name="cd_prazo_faturamento" size="2" maxlength="2" value="<%=cd_prazo_faturamento%>" class="inputs"> (qtd dias)</td>
	</tr>
	<tr bgcolor="<%=atividade%>">
		<td>&nbsp; Conta p/ pagamento:</td>
		<td colspan="3"><%if cd_empresa > 0 Then%>
							<select name="cd_contacorrente">
							<option value="" SELECTED></option>
							<%xsql = "SELECT * FROM TBL_Banco WHERE cd_tipo_titular = 1 and cd_titular = "&cd_cliente_empresa&""
								set rs_cc = dbconn.execute(xsql)
									while not rs_cc.EOF
									strcd_contacorrente = rs_cc("cd_codigo")
									nm_banco =  rs_cc("nm_banco")
									nm_agencia =  rs_cc("nm_agencia")
									nm_contacorrente =  rs_cc("nm_contacorrente")%>
										<option value="<%=strcd_contacorrente%>" <%if strcd_contacorrente = cd_contacorrente then%>SELECTED<%end if%>><%=nm_banco&" &nbsp; - &nbsp; ag.:"&nm_agencia&" &nbsp; - &nbsp; C/C:"&nm_contacorrente%></option>
									<%rs_cc.movenext
									wend%>
							</select>
						<%else
							response.write("Após cadastrar a unidade e selecionar a empresa<br>da qual ela é cliente, selecione a conta para qual o Hospital<br>efetuará o pagamento")
