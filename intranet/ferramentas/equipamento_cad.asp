...
<form action="acao/cadastros_acao.asp" method="post" name="Form" id="Form"><!-- onsubmit="return validar_equipamento(document.Form);"-->
	...
	<td><select name="cd_tipo">
						...
						Set rs = dbconn.execute(strsql)
						 while not rs.EOF
							...
							<option value="<%=cd_equipamento_tipo%>" <%if cd_tipo = cd_equipamento_tipo then%>Selected<%end if%>><%=nm_equipamento_tipo%></option>
							<%rs.movenext
						 wend%>
				</select></td>
</tr>
...
function JsDelete(cod1)
{
  if (confirm ("Tem certeza que deseja excluir?"))
	  {
		document.location=('acoes/cadastros_acao.asp?tipo=equipamento&cd_equipamento='+cod1+'&modo=equipamento&status=2&acao=excluir');
	  }
}
function JsDesativar(cod1)
{
  if (confirm ("Tem certeza que deseja tornar o equipamento inativo?"))
	  {
		document.location=('acoes/cadastros_acao.asp?tipo=equipamento&cd_equipamento='+cod1+'&modo=equipamento&status=0&acao=excluir');
	  }
}
function JsReativar(cod1)
{
  if (confirm ("Tem certeza que deseja reativar o cadastro do equipamento?"))
	  {
		document.location=('acoes/cadastros_acao.asp?tipo=equipamento&cd_equipamento='+cod1+'&modo=equipamento&status=0&acao=editar');
	  }
}
</SCRIPT>
</body>
