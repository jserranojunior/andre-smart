...
data_nova = Year(now)&"-"&Month(now)&"-"&Day(now)'&"' "&Hour(now)&":"&Minute(now)&":"&Second(now)
...
nm_complemento = request("nm_complemento")
nm_bairro = request("nm_bairro")
nm_cidade = request("nm_cidade")
nm_estado = request("nm_estado")
nm_endereco_completo = nm_endereco&"!"&nm_numero&"#"&nm_complemento&"~"&nm_cep&"$"&nm_bairro&"["&nm_cidade&"]"&nm_estado
...
cd_contacorrente = request("cd_contacorrente")
...
nr_numero = request("nr_numero")
	nr_numero = replace(nr_numero,"-","")
nr_imei_1 = request("nr_imei_1")
nr_imei_2 = request("nr_imei_2")
cd_modelo = request("cd_modelo")
...
' ****  AÇÕES  *******************************************************************************
	...
	'Insere dados
	xsql = "up_aparelho_insert @nr_numero="&nr_numero&", @nr_imei_1="&nr_imei_1&", @nr_imei_2="&nr_imei_2&", @cd_modelo="&cd_modelo&", @cd_status=1, @cd_user="&cd_user&", @dt_cad='"&data_atual&"'"
	...
	'Altera dados
		'If cd_status = "1" then
			'xsql = "up_aparelho_updates @cd_fornecedor='"&cd_fornecedor&"', @nm_fornecedor='"&nm_fornecedor&"', @nm_endereco='"&nm_endereco&"', @nm_numero='"&nm_numero&"', @nm_bairro='"&nm_bairro&"', @nm_cep='"&nm_cep&"', @nm_cidade='"&nm_cidade&"', @nm_estado='"&nm_estado&"', @nm_cnpj='"&nm_cnpj&"', @nm_ie='"&nm_ie&"', @cd_prazo='"&cd_prazo&"', @cd_status='"&cd_status&"'"
			xsql = "sp_CRUD @cd_codigo="&cd_codigo&", @nr_numero='"&nr_numero&"', @nr_imei_1='"&nr_imei_1&"', @nr_imei_2='"&nr_imei_2&"', @cd_modelo="&cd_modelo&", @cd_status="&cd_status&", @cd_user="&cd_user&", @dt_cad='"&dt_cad&"', @acao=2"
			dbconn.execute(xsql)
			response.write("aparelho update ok<br>")
				if jan = "" Then
				response.redirect("../ferramentas.asp?tipo=fornecedores&status=1")
				end if
		'end if
	...
	'Apaga dados
		'If cd_status = "1" then
			'xsql = "up_aparelho_updates @cd_fornecedor='"&cd_fornecedor&"', @nm_fornecedor='"&nm_fornecedor&"', @nm_endereco='"&nm_endereco&"', @nm_numero='"&nm_numero&"', @nm_bairro='"&nm_bairro&"', @nm_cep='"&nm_cep&"', @nm_cidade='"&nm_cidade&"', @nm_estado='"&nm_estado&"', @nm_cnpj='"&nm_cnpj&"', @nm_ie='"&nm_ie&"', @cd_prazo='"&cd_prazo&"', @cd_status='"&cd_status&"'"
			xsql = "sp_CRUD @cd_codigo="&cd_codigo&",@acao=3"
			dbconn.execute(xsql)
			
			response.write("<br>*** "&cd_codigo)
			response.write(" *** aparelho exclusao ok<br>")
				if jan = "" Then
				'response.redirect("../ferramentas.asp?tipo=fornecedores&status=1")
				end if
		'end if
	...
	ELSEIf acao = "ativar" AND modo = "aparelho" then
		'elseif cd_status = 0 then
			xsql = "up_aparelho_inativar @cd_fornecedor='"&cd_fornecedor&"', @cd_status=1"
			action = "O equipamento foi reativado"
			dbconn.execute(xsql)
				if jan = "" Then
				response.redirect("../../ferramentas.asp?tipo=fornecedores&status=0")
		...
		'end if
	elseif acao = "desativar" AND modo = "aparelho" then
		'elseif cd_status = 0 then
			xsql = "up_aparelho_inativar @cd_fornecedor='"&cd_fornecedor&"', @cd_status=0"
			action = "O equipamento foi desativado"
			dbconn.execute(xsql)
				if jan = "" Then
				response.redirect("../../ferramentas.asp?tipo=fornecedores&status=1")
...
xsql = "sp_CRUD<br>
@nr_numero=<%=nr_numero%><br>
@nr_imei_1=<%=nr_imei_1%><br>
@nr_imei_2=<%=nr_imei_2%><br>
@cd_modelo=<%=cd_modelo%><br>
...
cd_contacorrente = <%=cd_contacorrente%><br>
