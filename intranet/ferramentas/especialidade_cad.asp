...
<head>
	<title>Cadastro de especialidades - Manutenção</title>
</head>
...
<input type="hidden" name="modo" value="especialidade">
...
<tr bgcolor="<%=atividade%>">
	...
	<a href="ferramentas.asp?tipo=especialidade&status=1">Novo</a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
	(Ir para: <%if status = 0then%>
	<a href="ferramentas.asp?tipo=especialidade&status=1">Ativos</a>
	<%elseif status = 1  or status = "" then%>
	<a href="ferramentas.asp?tipo=especialidade&status=0">Inativos</a>
	<%end if%>)
...
<tr>
	<td colspan="100%">&nbsp;<b>Busca: <input type="text" name="busca_especialidade" class="inputs" size="10" maxlength="30" value="<%=cd_especialidade%>" onKeyup="mostra_especialidade();"></td>
</tr>
<%end if%>
<!--tr bgcolor="#C0C0C0">
...
<tr>
	<td colspan="100%">
		<span id='divEspecialidade'>
			...
			<%elseif status = 0 then
			strsql ="SELECT * FROM TBL_especialidades where cd_status = 0 ORDER BY nm_especialidade"
			...
			<%Do while Not rs_especialidade.EOF
				...
				<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'');">			
					...
					<td valign="middle"><a href="ferramentas.asp?tipo=especialidade&cd_especialidade=<%=cd_especialidade%>&nm_especialidade=<%=nm_especialidade%>&acao=editar">&nbsp;<%=cd_especialidade%></a></td>
					<td valign="middle"><a href="ferramentas.asp?tipo=especialidade&cd_especialidade=<%=cd_especialidade%>&nm_especialidade=<%=nm_especialidade%>&acao=editar">&nbsp;<%=nm_especialidade%></td>
...
function JsDelete(cod1)
{
  if (confirm ("Tem certeza que deseja excluir?"))
	  {
		document.location.href('acoes/cadastros_acao.asp?tipo=especialidade&cd_especialidade='+cod1+'&modo=especialidade&status=2&acao=excluir');
	  }
}
function JsDesativar(cod1)
{
  if (confirm ("Tem certeza que deseja tornar o especialidade inativo?"))
	  {
		document.location.href('acoes/cadastros_acao.asp?tipo=especialidade&cd_especialidade='+cod1+'&modo=especialidade&status=0&acao=excluir');
	  }
}
function JsReativar(cod1)
{
  if (confirm ("Tem certeza que deseja reativar o cadastro do especialidade?"))
	  {
		document.location.href('acoes/cadastros_acao.asp?tipo=especialidade&cd_especialidade='+cod1+'&modo=especialidade&status=0&acao=editar');
	  }
}
</SCRIPT>
</body>
