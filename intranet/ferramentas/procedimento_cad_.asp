...
function foco(){
document.form.nm_procedimento.focus(); }
// -->	
</script>
<SCRIPT LANGUAGE="JavaScript">
<!-- Begin
nextfield = "nm_procedimento"; // nome do primeiro campo do site
netscape = "";
...
<br><br><!-- LISTAGEM DE procedimentos -->
<table class="txt" border="0" width="500" align="center">
<%if status <> 0 or status = "" then%>
<tr>
	<td colspan="100%">&nbsp;<b>Busca: <input type="text" name="busca_procedimento" class="inputs" size="10" maxlength="30" value="<%=cd_procedimento%>" onKeyup="mostra_procedimento();"></td>
</tr>
<%end if%>
<!--tr bgcolor="#C0C0C0">
		...
		<span id='divProc'>
			...
			<%Do while Not rs_procedimento.EOF
				...
				<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'');">			
					...
					<td valign="middle"><a href="ferramentas.asp?tipo=procedimento&cd_procedimento=<%=cd_procedimento%>&nm_procedimento=<%=nm_procedimento%>&acao=editar">&nbsp;<%=cd_procedimento%></a></td>
					<td valign="middle"><a href="ferramentas.asp?tipo=procedimento&cd_procedimento=<%=cd_procedimento%>&nm_procedimento=<%=nm_procedimento%>&acao=editar">&nbsp;<%=nm_procedimento%></td>
					<!--td valign="top">&nbsp;<%=dt_cadastro%></td-->
					<td id="no_print" valign="middle">
					<img src="imagens/ic_del.gif" onclick="javascript:JsDelete('<%=cd_procedimento%>')" type="button" value="A" title="Excluir">
...
function JsDelete(cod1)
{
  if (confirm ("Tem certeza que deseja excluir?"))
	  {
		document.location.href('acoes/cadastros_acao.asp?tipo=procedimento&cd_procedimento='+cod1+'&modo=procedimento&status=2&acao=excluir');
	  }
}
function JsDesativar(cod1)
{
  if (confirm ("Tem certeza que deseja tornar o procedimento inativo?"))
	  {
		document.location.href('acoes/cadastros_acao.asp?tipo=procedimento&cd_procedimento='+cod1+'&modo=procedimento&status=0&acao=excluir');
	  }
}
function JsReativar(cod1)
{
  if (confirm ("Tem certeza que deseja reativar o cadastro do procedimento?"))
	  {
		document.location.href('acoes/cadastros_acao.asp?tipo=procedimento&cd_procedimento='+cod1+'&modo=procedimento&status=0&acao=editar');
	  }
}
</SCRIPT>
</body>
