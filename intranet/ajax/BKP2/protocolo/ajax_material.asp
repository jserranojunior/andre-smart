...
If cd_material = "" Then '*** Mostra campo normal ***%>
	...
	<input type="button" name="somar_mat" value="+" onKeyup="nextfield ='cd_material';">&nbsp;
<%Elseif IsNumeric(cd_material) then ' *** Mostra Campo numérico ***%>
	...
	mat_sel = ""
	wend%>
	<input type="text" name="cd_material_1" value="<%=cd_cod_mat%>">
...
<%Elseif not IsNumeric(cd_material) then%>
		...
		Set rs_mat = dbconn.execute(xsql)%>	
		<select name="cd_material_1" onBlur="soma_mat(document.form.cd_material_1.value,document.form.cd_material_2.value,document.form.qtd_material.value,document.form.materiais.value)" onKeyup="nextfield ='cd_material';">
		...
		<%end if%>	
			...
			<input type="Button" name="somar_mat" value="ok" onFocus="nextfield ='cd_material_1';" onclick="soma_mat(document.form.cd_material_1.value,document.form.cd_material_2.value,document.form.qtd_material.value,document.form.materiais.value);">
			<%rs_mat.close
			set rs_mat = nothing%>
