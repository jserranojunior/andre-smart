...
If cd_especialidade = "" Then '*** Mostra campo normal ***%>
	<%xsql = "SELECT * FROM TBL_especialidade where cd_status = 1 Order by nm_especialidade"
...
<%Elseif IsNumeric(cd_especialidade) then ' *** Mostra Campo numérico ***%>
	<%xsql = "SELECT * FROM TBL_especialidade where cd_status = 1 Order by cd_codigo" '*** Mostra pelo nome
	Set rs_espec = dbconn.execute(xsql)%>
	<select size="1" id="lista" name="cd_especialidade_1">
	<%while not rs_espec.EOF%>
	<%cd_cod_espec = rs_espec("cd_codigo")
...
<%Elseif not IsNumeric(cd_especialidade) then%>
		<%xsql = "SELECT * FROM TBL_especialidade where nm_especialidade like '"&cd_especialidade&"%' AND cd_status = 1 order by nm_especialidade"
		Set rs_espec = dbconn.execute(xsql)%>	
		<select size="1" id="lista" name="cd_especialidade_1">
