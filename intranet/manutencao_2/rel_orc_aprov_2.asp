...
ordem_sel = request("ordem_sel")
	if ordem_sel = "" Then ordem_sel = 1
		'if tipo = financ then ordem_sel = 2
...
dt_mes = request("mes_sel")
	if dt_mes = "" OR dt_ano = "" then
		...
		ano_posterior = dt_ano
			...
			dt_ano_fim = ano_posterior
			dt_mes_fim = mes_posterior
					...
					if coluna_divisora	= compromissos then
						...
						for i = 0 to qtd_parc_rest
							if int(coluna_registro) > int(coluna_princ) then
								...
								mes_vf = coluna_princ -1
								entrega_vf = month(dt_entrega)
					
					if (mes_vf <> entrega_vf) then
					
					
													
							
								
								response.write(formatnumber(nr_valor_parcela,2))
							...
							if (i = 0) and (month(dt_entrega) = 12) and (dt_mes = 12)then
								nr_valor_parcela = 0
							else 	
								nr_valor_parcela = nm_valor / nr_parcela
							end if		
								...
								<td align="right">
									...
									<%if (i = 0) then response.write("<b>")%>
									
									<%if (month(dt_entrega) <> 12) or (i > 0) and (month(dt_entrega) = 12) or (i = 0) and (month(dt_entrega) <> 12) then	 
										response.write(formatnumber(nr_valor_parcela,2))
									
									
									elseif (dt_mes = 1) and (i = 0) or (dt_mes = 2) and (nr_parcela > 1) then
										response.write(formatnumber(nr_valor_parcela,2))
									end if
