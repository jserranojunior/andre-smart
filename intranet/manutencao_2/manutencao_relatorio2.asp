...
arquivo_loc = "manutencao_relatorio2.asp"
...
'***** Pendentes ******
'if os_status = "0" Then 'O.S. Pendente
'cond_fecha = "AND (fecha BETWEEN 0 AND 0)"
'ck_pend1 = "checked"
'elseif os_status = "1" then ' O.S. Encerrada
'cond_fecha = "AND (fecha BETWEEN 1 AND 1)"
'ck_pend2 = "checked"
'Elseif os_status = "" Then ' O.S. Todas
...
'***** Verifica quais campos foram selecionados *****
	...
	campo_0_1 = Instr(1,campos,"cd_codigo,cd_andamento",0)
	...
	'elseif os_status = 1 then
		...
		'end if
	'end if
	campo_8 = Instr(1,campos,"num_os_assist,cd_orcamento",0)
...
<table align="center" border="1">	
	...
	<tr><td colspan="2"><img src="imagens/blackdot.gif" alt="" width="100%" height="1" border="0"></td></tr>
			...
			<table border="1" bordercolor="#C0C0C0" cellpadding="0" cellspacing="0" bordercolordark="#FFFFFF" align="center">
				...
				<input type="hidden" name="tipo" value="relatorio2">
				<!-- 3ª Etapa -->
				...
				<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'#<%=bgc_linha%>');" bgcolor="#<%=bgc_linha%>">
					...
					<td colspan="4"><input type="radio" name="os_status" value="1" <%if os_status = 1 then response.write("checked")%>>&nbsp;Pendentes <b>(Abertas)</b> &nbsp; &nbsp; &nbsp;
									<input type="radio" name="os_status" value="2" <%if os_status = 2 then response.write("checked")%>>&nbsp;Encerradas <b>(Fechadas)</b> &nbsp; &nbsp; &nbsp;
									<input type="radio" name="os_status" value="0" <%if os_status = 0 OR os_status = "" then response.write("checked")%>>&nbsp;Todas
					</td>
					<td>
					<%ordem_var = ordem_2
					ordem_num="ordem_2"
...
<!-- 3 ------------------------------------------------------------------------------------------------------------------------------------------- -->
				...
				<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'#<%=bgc_linha%>');" bgcolor="#<%=bgc_linha%>">
					<td colspan="3"><%if campo_8 <> 0 then%><%ck_campo_8 ="checked"%><%end if%>
						<input type="checkbox" name="campos" value="num_os_assist,cd_orcamento" <%=ck_campo_8%>>O.S. Assistência
					</td>
...
<!-- --------------------------------------------------------------------------------- -->				
				...
				<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'#<%=bgc_linha%>');" bgcolor="#<%=bgc_linha%>">
					<td colspan="3"><%if campo_7 <> 0 then%><%ck_campo_7 ="checked"%><%end if%>
						...
						<input type="checkbox" name="campo_periodo" value="1" <%=ck_campo_7%>>Período<img src="../imagens/blackdot.gif" alt="" width="21" height="1" border="0">
						<input type="radio" name="os_periodo" value="1" <%if os_periodo = "" OR os_periodo = 1 Then response.write("checked")%>> O.S. <br><img src="../imagens/blackdot.gif" alt="" width="80" height="1" border="0">
						<input type="radio" name="os_periodo" value="2" <%if os_periodo = 2 Then response.write("checked")%>> Entrada<br><img src="../imagens/blackdot.gif" alt="" width="80" height="1" border="0">
						<input type="radio" name="os_periodo" value="3" <%if os_periodo = 3 Then response.write("checked")%>> ???
					</td>
					<!--td>&nbsp;132465798</td-->
					<td colspan="4">
						<select name="dia_i">
...
<%if campo_3 <> 0 Then%><td>&nbsp;A. Tec./Forn.</td><%end if%>
...
<%if campo_13 <> 0 Then%><td>&nbsp;Nat. Defeito</td><%end if%>
<%if campo_13_1 <> 0 Then%><td>&nbsp;Motivo</td><%end if%>
...
<%if campo_16 <> 0 Then%><td>&nbsp;Gestão</td><%end if%>
			...
			response.write("sp_manutencao_s01 @funcao='S', @os_periodo='"&os_periodo&"', @dt_inicial='"&data_i&"', @dt_final='"&data_f&"',@cd_unidade='27'")
			'xsql1000 = "sp_manutencao_s01 @funcao='S', @dt_inicial='"&data_i&"', @dt_final='"&data_f&"',@cd_unidade='27'"
			
			
			strsql = "sp_manutencao_s02 @funcao='S', @os_periodo='"&os_periodo&"', @dt_inicial='"&data_i&"', @dt_final='"&data_f&"'"
			...
			while Not rs.EOF 
				if campo_0 >  0 then
					...
					<%if campo_0 <> 0 Then%><td valign="top">
						<%if cd_user = 460 then '*** O.S.***%><a href="javascript:;" onclick="window.open('manutencao_2/manutencao_nova.asp?cd_codigo=<%=cod_codigo%>&campo=cd_codigo&visual=1&jan=1', 'principal', 'width=651, height=500,scrollbars=1'); return false;"><%=zero(cd_unidade)%>.<%=manutencao(num_os)%></a>
						<%elseif cd_user = 10 then'*** Andamento ***%><a href="javascript:;" onclick="window.open('manutencao_2/manutencao_andamento2.asp?cd_codigo=<%=cod_codigo%>&campo=cd_codigo&visual=1&jan=1', 'principal', 'width=651, height=500,scrollbars=1'); return false;"><%=zero(cd_unidade)%>.<%=manutencao(num_os)%></a>
						<%else%><a href="javascript:;" onclick="window.open('manutencao_2/manutencao_ver_janela2.asp?cd_codigo=<%=cod_codigo%>&campo=cd_codigo&visual=1&jan=1', 'principal', 'width=600, height=500,scrollbars=1'); return false;"><%=zero(cd_unidade)%>.<%=manutencao(num_os)%></a>
						<%end if%>
						</td>
					<%end if%>
					...
					<%if campo_8 <> 0 Then%><td valign="top"><%=num_os_assist%>
						<%=":"&cd_orcamento&"-"%>
						
					<%if isnull(cd_orcamento) Then
					
						
						strsql2 ="select * from tbl_manutencao_orcamento  where cd_fornecedor = '"&cd_fornecedor&"' AND nr_orcamento = '"&num_os_assist&"'"
						Set rs2 = dbconn.execute(strsql2)
						if NOT rs2.EOF Then
							cod_orcamento = rs2("cd_codigo")
							response.write("("&cod_orcamento&")")
							
							strsql3 =" UPDATE TBL_andamento2 SET cd_orcamento = '"&cod_orcamento&"' where cd_cod = '"&cod_cod&"'"
							'Set rs3 = dbconn.execute(strsql3)
							response.write("*"&cd_andamento)
					...
					<%if campo_12 <> 0 Then%><td valign="top"><%if cd_valor_orc<>"" Then%><%=formatnumber(cd_valor_orc,2)%><%end if%></td><%end if%>
