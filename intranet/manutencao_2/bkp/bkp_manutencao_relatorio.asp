...
<%if campo_6 <> 0 Then%><td>&nbsp;Marca</td><%end if%>
			...
			xsql = ""&selecao&" FROM VIEW_os_lista "&cond&" "&cond_periodo&" "&cond_ns&" "&cond_patrimonio&" "&cond_pat_vazio&" "&cond_eqp&" "&cond_und&" "&cond_forn&" "&cond_esp&" "&cond_aval&" "&cond_marca&" "&agrupamento&" "&ordem&" "&sentido&""'
...
<%if campo_0 <> 0 Then%><td valign="top"><a href="javascript:;" onclick="window.open('manutencao/manutencao_andamento.asp?cd_codigo=<%=cd_cod%>&campo=cd_codigo&visual=1', 'principal', 'width=600, height=500,scrollbars=1'); return false;"><%=num_os%></a></td><%end if%>
