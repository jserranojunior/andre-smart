...
arquivo_loc = "manutencao_nova.asp"
...
if num_os <> "" Then
	...
	Set rs_verifica = dbconn.execute(xsql)
		...
		loop
			if num_os_verifica = int(num_os) AND os_alt <> 1 then 'Redireciona à O.S. existente
				response.redirect("manutencao2.asp?tipo=andamento&cd_codigo="&cd_codigo_verifica&"&campo=cd_codigo")
			End if
end if
'*************************************
if cd_unidade <> "" AND num_os <> "" Then
				
				'if cd_codigo <> "" Then
			if cd_codigo_verifica <> "" Then
				...
				xsql ="sp_manutencao_s01 @funcao='SV', @dt_inicial='', @dt_final='', @cd_unidade='"&cd_unidade&"', @num_os='"&num_os&"'"
			...
			<!--Início da etapa 1.1-->
				...
				<tr bgcolor="#f5f5f5">
					...
					<td align="center" valign="bottom"><%'=cd_codigo%>
						<%if cd_os = "" Then%>
						...
						<%else%>
							<input type="text" name="cd_unidade" size="2" maxlength="2"  onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);" value="<%=cd_unidade%>">
							<input type="text" name="num_os" size="5" maxlength="4" class="inputs" value="<%=num_os%>" onFocus="nextfield ='dt_os';">
						<%end if%>
						<br><img src="imagens/px.gif" alt="" width="120" height="1" border="0">
					</td>
					<td align="center" valign="bottom"><b>NATUREZA:</b><br><img src="imagens/px.gif" alt="" width="80" height="1" border="0"><%'=cd_natureza_os%></td>
				...
				<tr bgcolor="#f5f5f5">
					...
					<td colspan="3">
						<table border="1" style="border-collapse:collapse;">
							<tr>
								<td style="width:<%=larg_1%>px;"><b>Pat:</b></td>
								<td style="width:<%=larg_2%>px; color:#339900;"><b><%=no_patrimonio%></b></td>
								<td style="width:<%=larg_1%>px;"><b>N/S:</b></td>
								<td style="width:<%=larg_2%>px;"><%=cd_ns%></td>
								<!--td colspan="2"><input type="text" name="cd_patrimonio" value="<%=cd_patrimonio%>"></td-->
							</tr>
						  	<tr>
								<td style="width:<%=larg_1%>px;"><b>Item:</b></td>
								<td style="width:<%=larg_2%>px;"><%=nm_equipamento_novo%></td>
								<td style="width:<%=larg_1%>px;"><b>Qtd:</b></td>
								<td style="width:<%=larg_2%>px;"><%=num_qtd%></td>
							</tr>
							<!--tr style="color:gray;">
								<td style="width:<%=larg_1%>px;"><b>Antigo:</b></td>
								<td style="width:<%=larg_2%>px;"><%=nm_equipamento%></td>
								<td style="width:<%=larg_1%>px;"><b>Patrimonio:</b></td>
								<td style="width:<%=larg_2%>px;"><%=no_pat_old%></td>			
							</tr-->
							<tr>
								<td style="width:<%=larg_1%>px;"><b>Marca:</b></td>
								<td style="width:<%=larg_2%>px;"><%=nm_marca%></td>
								<td style="width:<%=larg_1%>px;"><b>Especialidade:</b></td>
								<td style="width:<%=larg_2%>px;"><%=nm_especialidade%></td>
							</tr>
						</table>
					</td>
				</tr>
				...
				<tr>
					<td colspan="4" align="center">
						<%If acao = "1" Then%>
							<input  name="ok" type="submit" value="Cadastrar O.S.">
						<%Else%>
							<input type="submit" value="Modificar O.S.">
								<%if jan <> 1 then%>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								
								<a href="../manutencao2.asp?tipo=andamento&cd_codigo=<%=cd_codigo%>&campo=<%=campo%>">Voltar1</a>
								<%else%>
									<a href="manutencao_andamento2.asp?cd_codigo=<%=cd_codigo%>">Voltar2</a>
								http://vdlap.sytes.net:1014/manutencao_2/manutencao_andamento2.asp?cd_codigo=16915&campo=cd_codigo&visual=1&jan=1
								<%end if%>
						<%End if%>
