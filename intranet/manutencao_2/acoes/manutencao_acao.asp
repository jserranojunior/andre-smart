...
'Dados da O.S.
	...
	array_manut = request("array_manut")
...
'Dados da Manutenção
	...
	cd_os = request("cd_os")
	dt_encaminhado = request("dt_encaminhado")
	nm_orcamento = request("nm_orcamento")
...
'Manutencao & Orcamento
	...
	dt_prev_resposta = request("dt_prev_resposta")
	dt_prev_retorno = request("dt_retorno")
	...
	Response.Write(dt_prev_retorno)
	...
	If dt_retorno_un = "" Then
		...
		dt_retorno_un_ano = year(dt_retorno_un)
		dt_retorno_un_mes = month(dt_retorno_un)
		dt_retorno_un_dia = day(dt_retorno_un)
			if dt_retorno_un_dia > ultimodiames(dt_retorno_un_mes,dt_retorno_un_ano) then
				dt_retorno_un_dia = ultimodiames(dt_retorno_un_mes,dt_retorno_un_ano)
			end if
				'dt_retorno_un = "'"&month(dt_retorno_un)&"/"&day(dt_retorno_un)&"/"&year(dt_retorno_un)&"'"
				dt_retorno_un = "'"&dt_retorno_un_mes&"/"&dt_retorno_un_dia&"/"&dt_retorno_un_ano&"'"
	End if
...
' Cadastro de orçamentos 
	...
	dt_ano = request("dt_ano")
		If dt_dia = "" AND dt_mes = "" AND  dt_ano = "" Then
			...
			dt_orcamento = "'"&dt_mes&"/"&dt_dia&"/"&dt_ano&"'"
		End if
		
	dt_dia_aprov = request("dt_dia_aprov")
	dt_mes_aprov = request("dt_mes_aprov")
	dt_ano_aprov = request("dt_ano_aprov")
		 If dt_dia_aprov = "" OR dt_mes_aprov = "" OR dt_ano_aprov = "" Then
			dt_orcamento_aprov = "Null"
		else
...
If dt_conclusao = "" Then
	...
	dt_conclusao_ano = year(dt_conclusao)
	dt_conclusao_mes = month(dt_conclusao)
	dt_conclusao_dia = day(dt_conclusao)
		if dt_conclusao_dia > ultimodiames(dt_conclusao_mes,dt_conclusao_ano) then
			dt_conclusao_dia = ultimodiames(dt_conclusao_mes,dt_conclusao_ano)
		end if
			'dt_conclusao = "'"&month(dt_conclusao)&"/"&day(dt_conclusao)&"/"&year(dt_conclusao)&"'"
			dt_conclusao = "'"&dt_conclusao_mes&"/"&dt_conclusao_dia&"/"&dt_conclusao_ano&"'"
End if
...
if acao = "1" And filtro = "cadastra_os" then
	...
	wend
		...
		if cd_user = 460 then
			response.write("up_os @cd_unidade='"&cd_unidade&"', @num_os='"&num_os&"', @cd_natureza_os='"&cd_natureza_os&"', @dt_os='"&dt_os&"', @nm_solicitante='"&nm_solicitante&"', @cd_unidade_os="&cd_unidade&", @num_qtd='"&num_qtd&"', @cd_equipamento='"&cd_equipamento&"', @cd_especialidade='"&cd_especialidade&"', @cd_marca='"&cd_marca&"', @cd_ns='"&cd_ns&"', @cd_patrimonio="&cd_patrimonio&", @motivo='"&motivo&"', @dt_entrada='"&dt_entrada&"', @cd_avaliacao='"&cd_avaliacao&"', @cd_fornecedor='"&cd_fornecedor&"', @cd_liberacao='"&cd_liberacao&"', @observacoes='"&observacoes&"', @sequencia='"&sequencia&"', @cd_user="&cd_user&", @data_inc='"&data_atual&"', @fecha=0")
		Else
			xsql = "up_os_insert2 @cd_unidade='"&cd_unidade&"', @num_os='"&num_os&"', @cd_natureza_os='"&cd_natureza_os&"', @dt_os='"&dt_os&"', @nm_solicitante='"&nm_solicitante&"', @cd_unidade_os="&cd_unidade&", @num_qtd='"&num_qtd&"', @cd_equipamento='"&cd_equipamento&"', @cd_especialidade='"&cd_especialidade&"', @cd_marca='"&cd_marca&"', @cd_ns='"&cd_ns&"', @cd_patrimonio="&cd_patrimonio&", @motivo='"&motivo&"', @dt_entrada='"&dt_entrada&"', @cd_avaliacao='"&cd_avaliacao&"', @cd_fornecedor='"&cd_fornecedor&"', @cd_liberacao='"&cd_liberacao&"', @observacoes='"&observacoes&"', @sequencia='"&sequencia&"', @cd_user="&cd_user&", @data_inc='"&data_atual&"', @fecha=0"
			dbconn.execute(xsql)
...
elseif acao = "2" And filtro = "modifica_os" then
	'response.write("up_os_update2 @cd_codigo='"&cd_codigo&"', @num_os='"&num_os&"', @cd_natureza_os='"&cd_natureza_os&"', @dt_os='"&dt_os&"', @nm_solicitante='"&nm_solicitante&"', @cd_unidade_os="&cd_unidade&", @num_qtd='"&num_qtd&"', @cd_equipamento='"&cd_equipamento&"', @cd_especialidade='"&cd_especialidade&"', @cd_marca='"&cd_marca&"', @cd_ns = '"&cd_ns&"', @cd_patrimonio = "&cd_patrimonio&", @motivo='"&motivo&"', @dt_entrada='"&dt_entrada&"', @cd_avaliacao='"&cd_avaliacao&"', @cd_fornecedor='"&cd_fornecedor&"', @observacoes='"&observacoes&"',@data_inc='"&data_atual&"',@cd_user="&cd_user&",@sequencia='"&sequencia&"'<br><br>...<br>")
	xsql = "up_os_update2 @cd_codigo='"&cd_codigo&"', @num_os='"&num_os&"', @cd_natureza_os='"&cd_natureza_os&"', @dt_os='"&dt_os&"', @nm_solicitante='"&nm_solicitante&"', @cd_unidade_os="&cd_unidade&", @num_qtd='"&num_qtd&"', @cd_equipamento='"&cd_equipamento&"', @cd_especialidade='"&cd_especialidade&"', @cd_marca='"&cd_marca&"', @cd_ns = '"&cd_ns&"', @cd_patrimonio = "&cd_patrimonio&", @motivo='"&motivo&"', @dt_entrada='"&dt_entrada&"', @cd_avaliacao='"&cd_avaliacao&"', @cd_fornecedor='"&cd_fornecedor&"', @observacoes='"&observacoes&"',@data_inc='"&data_atual&"',@cd_user="&cd_user&",@sequencia='"&sequencia&"'"
	dbconn.execute(xsql)
...
Elseif acao = "1" AND filtro = "andamento" Then
	...
	'****************************
	'*** Inclui a providência *** (inclui na TBL_ordemServico2 o fornecedor, data envio, avaliação,etc)
	...
	wend
		...
		if andamento_gravado = "" then '(caso não existe andamento gravado, inclui um)
...
Elseif acao = "2" AND filtro = "andamento" Then
	...
	'***************************************************************************
	'*** Ação para preservar o banco de dados e O.S. atualização dos registros ***
		xsql = "up_os_update_andamento2 @cd_codigo='"&cd_codigo_os&"', @dt_entrada="&dt_entrada&", @cd_avaliacao='"&cd_avaliacao&"', @cd_fornecedor='"&cd_fornecedor&"', @cd_liberacao='"&cd_liberacao&"', @observacoes='"&observacoes&"',@data_inc='"&data_atual&"',@cd_user="&cd_user&",@sequencia='"&sequencia&"'"
		action = "editado com sucesso"
		dbconn.execute(xsql)
	'***************************************************************************
		xsql = "up_andamento_update2 @cd_codigo='"&cd_codigo&"',@cd_os='"&cd_os&"', @cd_cod='"&cd_cod&"', @dt_manut_orc="&dt_manut_orc&",@cd_fornecedor='"&cd_fornecedor&"', @nm_contato='"&nm_contato&"', @dt_prev_resposta="&dt_prev_resposta&", @cd_orcamento='"&cd_orcamento&"',@dt_resposta_orc="&dt_resposta_orc&", @nm_orcamento='"&nm_orcamento&"', @num_os_assist='"&num_os_assist&"', @dt_prev_retorno="&dt_prev_retorno&", @cd_situacao='"&cd_situacao&"', @cd_gestao='"&cd_gestao&"', @comentarios='"&comentarios&"', @dt_retorno="&dt_retorno&", @dt_retorno_un="&dt_retorno_un&", @cd_qtd_valor='"&cd_qtd_valor&"', @cd_valor_unit='"&cd_valor_unit&"', @cd_valor_orc='"&cd_valor_orc&"', @nm_natureza_defeito='"&nm_natureza_defeito&"', @sequencia='"&sequencia&"', @cd_alerta='"&cd_alerta&"'"
		response.write("<br>++++++cd_codigo='"&cd_codigo&"',@cd_os='"&cd_os&"', @cd_cod='"&cd_cod&"', @dt_manut_orc="&dt_manut_orc&",@cd_fornecedor='"&cd_fornecedor&"', @nm_contato='"&nm_contato&"', @dt_prev_resposta="&dt_prev_resposta&", @cd_orcamento='"&cd_orcamento&"',@dt_resposta_orc="&dt_resposta_orc&", @nm_orcamento='"&nm_orcamento&"', @num_os_assist='"&num_os_assist&"', @dt_prev_retorno="&dt_prev_retorno&", @cd_situacao='"&cd_situacao&"', @cd_gestao='"&cd_gestao&"', @comentarios='"&comentarios&"', @dt_retorno="&dt_retorno&", @dt_retorno_un="&dt_retorno_un&", @cd_qtd_valor='"&cd_qtd_valor&"', @cd_valor_unit='"&cd_valor_unit&"', @cd_valor_orc='"&cd_valor_orc&"', @nm_natureza_defeito='"&nm_natureza_defeito&"', @sequencia='"&sequencia&"', @cd_alerta='"&cd_alerta&"'++++<br>")
		dbconn.execute(xsql)
		action = "editado com sucesso..."
				if dt_resposta_orc <> "" AND num_os_assist <> "" Then
					'xsql_upd = "up_manutencao_orcamento_update @num_os_assist='"&num_os_assist&"', @dt_aprov_orc ='"&dt_resposta_orc&"'"
					xsql_upd = "up_manutencao_orcamento_update @num_os_assist='"&num_os_assist&"', @dt_aprov_orc="&dt_resposta_orc&", @funcao=S"
					dbconn.execute(xsql_upd)
				end if
						...
						'*************************************************************
						'************** Retorno/entrega à Vd Lap *********************
						if dt_retorno <> "Null" Then 
							'*** Verifica se a OS possui orçamento cadastrado *** 
							strsql = "SELECT * FROM TBL_manutencao_orcamento WHERE nr_orcamento = '"&num_os_assist&"'"
							Set rs = dbconn.execute(strsql)
							if not rs.EOF then
								cd_orcamento_forn = rs("cd_codigo")
								'nr_orcamento = rs("nr_orcamento")
							end if
							
								'***insere a data de entrega à Vd Lap ***.
								if cd_orcamento_forn <> "" Then
									xsql = "up_orcamento_baixa @cd_orcamento='"&cd_orcamento_forn&"', @dt_entrega="&dt_retorno&""
									dbconn.execute(xsql)
								end if
						End if
	...
	if jan <> 1 then
		'response.redirect("../../manutencao2.asp?tipo=pendencias&strtop=20&filtro=pendentes")
		'response.redirect("../../manutencao2.asp?tipo=andamento&cd_codigo="&cd_codigo_os&"&campo=cd_codigo")
		response.redirect("../../manutencao2.asp?tipo=nova2")
	end if
...
Elseif acao = "1" AND filtro = "orc_cad" Then
	...
	xsql = "up_manutencao_orcamento_insert	@cd_fornecedor="&cd_fornecedor&", @nr_orcamento='"&nr_orcamento&"', @dt_orcamento="&dt_orcamento&", @dt_aprov_orc="&dt_orcamento_aprov&", @nm_valor='"&nm_valor&"', @nr_parcela="&nr_parcela&", @cd_user="&cd_user&""
	Action = "editado com sucesso"
	dbconn.execute(xsql)
...
Elseif acao = "1" AND filtro = "orc_cad_upd" Then
	'response.write("up_manutencao_orcamento_update @cd_orcamento="&cd_orcamento&",	@cd_fornecedor="&cd_fornecedor&", @dt_aprov_orc="&dt_orcamento_aprov&", @nr_orcamento='"&nr_orcamento&"', @dt_orcamento="&dt_orcamento&", @nm_valor='"&nm_valor&"', @nr_parcela="&nr_parcela&", @cd_user="&cd_user&", @funcao='U'")
	xsql = "up_manutencao_orcamento_update @cd_orcamento="&cd_orcamento&",	@cd_fornecedor="&cd_fornecedor&", @dt_aprov_orc="&dt_orcamento_aprov&", @nr_orcamento='"&nr_orcamento&"', @dt_orcamento="&dt_orcamento&", @nm_valor='"&nm_valor&"', @nr_parcela="&nr_parcela&", @cd_user="&cd_user&", @funcao='U'"
	dbconn.execute(xsql)
...
Elseif acao = "4" AND filtro = "orc_vinc_os" Then
	...
	For Each item_junto In OS_array
		if item_junto <> "" Then
			...
			strcod_andamento = replace(mid(item_junto,1,separador1),"!","") 'extrai o codigo do Andamento da OS da string e apaga o sinal de exclamação
			stros_valor = replace(mid(item_junto,separador1,separador2-separador1),"!","") 'extrai o valor da string e apaga o sinal de exclamação
			strqtd_itens = replace(mid(item_junto,separador2,len(item_junto)),"_","") 'extrai qtd da string e apaga o "under line"
				cd_valor_orc = stros_valor * strqtd_itens
			xsql = "sp_CRUD_manutencao @funcao='VO',@cd_codigo='"&cd_orcamento&"',@cd_os='"&strcod_andamento&"',@cd_valor_unit='"&stros_valor&"', @num_qtd='"&strqtd_itens&"', @cd_valor_orc='"&cd_valor_orc&"'"
			dbconn.execute(xsql)
		end if
	next
'*******************************************************************************
...
Elseif acao = "5" AND filtro = "orc_vinc_os" Then
	xsql = "sp_CRUD_manutencao @funcao='DO', @cd_codigo='"&cd_os&"'"
	dbconn.execute(xsql)
...
<body onload="window.close<%if cd_user = 46 then response.write("s")%>();" onunload="window.opener.location.reload(true);">
<%if cd_user = 46 then%>
<br>
*** Movimentação ***<br>
jan: <%=jan%><br>
...
num_os_assist = <%=num_os_assist%><br>
dt_resposta_orc = <%=dt_resposta_orc%><br>
<br>
