...
mes = month(now)
				...
				xsql_contrato = "Select * From View_funcionario_contrato_lista Where cd_codigo = "&cd_funcionario&""
...
<input type="hidden" name="cod" value="<%=cod_contrato%>">
...
<table border="0" align="center">
  ...
  %>
	<tr bgcolor="<%=cor_linha%>" style="color:black; font-family:verdana; font-size:12px; background-color:#e9e9e9;">
		...
		<td>&nbsp;<%if day(dt_demissao) = "" then%><%=day(dt_demissao)%>/<%=mesdoano(Month(dt_demissao))%>/<%=year(dt_demissao)%><%end if%></td>	
		<!--td align="center">
