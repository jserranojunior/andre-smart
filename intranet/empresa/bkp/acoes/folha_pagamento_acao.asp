...
if x = "" then
	...
	if nr_faltas <> "" Then
		...
		set rs = dbconn.execute(xsql)
		while not rs.EOF
			cod_faltas = rs("cd_codigo")
			rs.movenext
		wend
			'*** INSERE ***
				if cod_faltas = "" then
					xsql = "up_funcionario_valor_insert @cd_funcionario='"&strcd_funcionario&"', @cd_valor_tipo=15, @nr_valor='"&nr_faltas&"', @dt_valor_atualizacao='"&dt_mes&"/"&dt_dia&"/"&dt_ano&"', @cd_indice = NULL, @nm_valor_obs='"&nm_valor_obs&"', @cd_sessao='"&cd_user&"', @dt_cadastro='"&month(now)&"/"&day(now)&"/"&year(now)&"'"
					dbconn.execute(xsql)
			'*** ALTERA ***
				else
					xsql = "up_funcionario_valor_update @cd_valor='"&cd_faltas&"', @nr_valor='"&nr_faltas&"', @nm_valor_obs='"&str_valor_obs&"', @cd_sessao='"&session_cd_usuario&"',@dt_atualizador='"&month(now)&"/"&day(now)&"/"&year(now)&"'"
					dbconn.execute(xsql)
				end if
			'**********************
	end if
