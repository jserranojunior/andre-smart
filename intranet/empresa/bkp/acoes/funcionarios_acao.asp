...
'dia_vt_cancela = request("dia_vt_cancela")
'mes_vt_cancela = request("mes_vt_cancela")
'ano_vt_cancela = request("ano_vt_cancela")
	'if vtransp_cancela = 1 then
	'	if dia_vt_cancela <> "" OR mes_vt_cancela <> "" OR ano_vt_cancela <> ""  and then
	'		dt_vt_cancela = mes_vt_cancela&"/"&dia_vt_cancela&"/"&ano_vt_cancela
	'	else
	'		dt_vt_cancela = month(now)&"/"&day(now)&"/"&year(now)
	'	end if
...
if x = "" then
	If stracao = "insert" Then
		...
		dbconn.execute(xsql)
			...
			if vtransp_cancela = 1 then
			'Cancela o vale transporte e consequentemente o desconto de 6% na folha de pagamento
				xsql = "up_funcionario_valor_insert @cd_funcionario='"&cd_cod_funcionario&"', @cd_valor_tipo=4, @nr_valor=0, @dt_valor_atualizacao="&Strdt_contratado&",  @nm_valor_obs='"&nm_valor_obs&"', @cd_sessao='"&cd_user&"', @dt_cadastro='"&month(now)&"/"&day(now)&"/"&year(now)&"'"
				dbconn.execute(xsql)
			end if
