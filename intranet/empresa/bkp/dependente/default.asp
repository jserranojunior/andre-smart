...
function adiciona_dependente(a,b,c,d,e,dependentes_total){
	...
	dependentes_total = dependentes_total + separador
		if (a != ""){
			...
			document.dependente.dependentes_result.value=a;} //adiciona				
						
		if (b != ""){
			dependentes_total = dependentes_total.replace(b,'');
			document.dependente.dependentes_total.value=dependentes_total;} //subtrai			

	//document.dependente.dependentes_result.value=a; //a
	//document.dependentecontato.nm_nome_contato.value=b; //b
	document.dependente.dependentes_total.value=(dependentes_total.replace("$$", "$"));	
	document.dependente.dependentes_total.value=(dependentes_total); //c
	
	document.dependente.nm_dependente_nome.value="";
	document.dependente.cd_dependente_parentesco.value="";
	document.dependente.dt_dependente_nascimento.value="";
	document.dependente.cd_dependente_obs.value="";
	document.dependente.nm_dependente_nome.focus();
	//chama Ajax	
	{
	    ...
	    oHTTPRequest_dependentes.send("dependentes_total=" + dependente.dependentes_total.value);
	}
...
<table border="1">
	...
	<tr>
		...
		<td><select name="cd_dependente_parentesco" class="inputs">
				<option value=""></option>
				<%strsql = "Select * From TBL_dependente_parentesco order by idade"'Where cd_codigo=1 or cd_codigo = 2"
		...
		<td><input type="button" name="inclui_dependente" value="+" onFocus="adiciona_dependente(document.dependente.nm_dependente_nome.value,'',document.dependente.cd_dependente_parentesco.value,document.dependente.dt_dependente_nascimento.value,document.dependente.cd_dependente_obs.value,document.dependente.dependentes_total.value)"></td>
	</tr>
