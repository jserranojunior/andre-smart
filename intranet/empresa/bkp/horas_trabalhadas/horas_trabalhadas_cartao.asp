...
'strmes_sel = int(request("mes_sel"))
...
strdia_in  = dia_in&"-"&mes_in&"-"&ano_sel
strdia_out = dia_out&"-"&mes_out&"-"&ano_sel


xsql_verif = "up_Verifica_fechamento_mes @cd_mes='"&mes_sel&"', @cd_ano='"&ano_sel&"', @cd_origem=2"
...
<table cellspacing="0" cellpadding="0" align="center" border="1" width="100">
	<tr>
		<td align="center">
			<table  width="100" align="center" border="0" cellspacing="1" cellpadding="0" bordercolordark="#FFFFFF" class="table">
					<tr bgcolor=#cococo><td align=center colspan="100%"><img src="imagens/blackdot.gif" width="300" height=2></td></tr>
					<tr id="no_print">
						<td colspan="100%">&nbsp;<b>FUNCIONÁRIOS - <font color="red">Horas trabalhadas - Digitação</font></b></td>
					</tr>
					<tr id="ok_print">
						<td colspan="100%">&nbsp;<b>FUNCIONÁRIOS - <font color="red">Horas trabalhadas - <%=mes_selecionado(mes_sel)%>/<%=ano_sel%></font></b></td>
					</tr>
					<tr id="no_print">
						<td colspan="100%" align="center">
						[<a href="empresa.asp?tipo=digitacao_horas&cd_codigo=<%=cd_codigo%>">Mudar Ano</a>] &nbsp;&nbsp; [<a href="empresa.asp?tipo=digitacao_horas&cd_codigo=<%=cd_codigo%>&ano_sel=<%=ano_sel%>">Mudar Mês</a>]  &nbsp;&nbsp;/  &nbsp;&nbsp;[<a href="empresa.asp?tipo=digitacao_horas&cd_codigo=">Mudar Funcionário</a>] </td>
					</tr>
					
				<%'*********************************************************
				  '*				      1ª Parte	 					  '*
				  '*********************************************************
				  '* 1.1 - Listagem de Funcionários *
				  '**********************************
					if mes_sel = "" AND ano_sel =  "" AND cd_codigo = "" Then%>
			<table width="100" cellspacing="1" cellpadding="1" border="0"  class="table">
					<tr bgcolor=#cococo>
					...
					%>
					<tr onmouseover="mOvr(this,'#CFC8FF');"  onclick="javascript:location.replace('empresa.asp?tipo=digitacao_horas&cd_codigo=<%=cd_codigo%>&tela=<%=tela%>');" onmouseout="mOut(this,'');">
						<td>&nbsp;</td>
				...
				<%'*********************************************************
				  '*				      2ª Parte	 					   *
				  '*********************************************************
				  '* 1.2 Seleçao do período *
				  '**************************
					elseif ano_sel = "" AND cd_codigo <> "" OR ano_sel <> ""  AND mes_sel = "" AND cd_codigo <> "" Then
					nome_arquivo = "empresa.asp"
					'response.write("segunda parte")%>
				  <tr><td colspan="100%" align="center"><!--#include file="../../includes/selecao_periodo.asp"--></td></tr>
				...
				<%'*********************************************************
				  ...
				  '*********************************************************
					...
					<tr id="no_print">
						<td colspan="100%" align="center">[<a href="empresa.asp?tipo=digitacao_horas&cd_codigo=<%=cd_codigo%>&arquivo=empresa.asp&ano_sel=<%=ano_sel%>&mes_sel=<%=mes_sel%>&tela=1">Tela inteira</a>:<a href="empresa.asp?tipo=digitacao_horas&cd_codigo=<%=cd_codigo%>&arquivo=empresa.asp&ano_sel=<%=ano_sel%>&mes_sel=<%=mes_sel%>&tela=0">Tela normal</a>]</td>
					</tr>
					<tr bgcolor=#cococo>
						<td align=center colspan="100%"><img src="imagens/blackdot.gif" width="100%" height=0></td>
					</tr>
					<tr bgcolor=#FFFFFF id="ok_print"><!-- espaçador impressão -->
						<td><img src="imagens/blackdot.gif" width=28 height=0></td>
						<td><img src="imagens/blackdot.gif" width=50 height=0></td>
						<td colspan="2"><img src="imagens/blackdot.gif" width=80 height=0></td>
						<td><img src="imagens/blackdot.gif" width=50 height=0></td>
						<td><img src="imagens/blackdot.gif" width=60 height=0></td>
						<td><img src="imagens/blackdot.gif" width=60 height=0></td>
						<td><img src="imagens/blackdot.gif" width=130 height=0></td>
					</tr>
					<tr>
						<td align=left colspan="4" bgcolor="#C0C0C0"><br>&nbsp;<%=nm_nome%>&nbsp;<%=nm_sobrenome%> <%'=" - "&dt_contratado&" - "%><%'=dt_desliga%><%'=data_atual%><b><br>&nbsp;</td>
						<%'Meses anteriores
						if mes_sel = 1 then 
							mes_ant = 12
							ano_ant = ano_sel - 1
							...
							If year(dt_contratado)&month(dt_contratado) = ano_sel&zero(mes_sel) then
								seta_a = ""
							Else
								seta_a = "<"
							end if
						...
						if mes_sel = 12 then 
							...
							'If year(dt_desliga)&month(dt_desliga) = ano_sel&mes_sel OR ano_prod&mes_prod  = ano_sel&mes_sel then
							If ano_prod&mes_prod  = ano_sel&zero(mes_sel) then
								seta_p = ""
							Else
								seta_p = ">"
							end if%>
							
			    		<td align=center colspan="4" bgcolor="#C0C0C0" id="no_print"><a href="empresa.asp?tipo=digitacao_horas&cd_codigo=<%=cd_codigo%>&ano_sel=<%=ano_ant%>&mes_sel=<%=mes_ant%>&tela=<%=tela%>"><%=seta_a%></a>&nbsp;<%=mes_selecionado(mes_sel)%>/<%=ano_sel%></b>&nbsp;<a href="empresa.asp?tipo=digitacao_horas&cd_codigo=<%=cd_codigo%>&mes_sel=<%=mes_prox%>&ano_sel=<%=ano_prox%>&tela=<%=tela%>""><%=seta_p%></a></td>
				...
				<table cellspacing="1" cellpadding="1" border="0" class="table" width=100>
				<tr bgcolor=#FFFFFF id="no_print">
						<!--td><img src="imagens/blackdot.gif" width=25 height=0--></td>
						<td align="left"><img src="imagens/blackdot.gif" width=48 height=0></td>
						<td align="left"><img src="imagens/blackdot.gif" width=69 height=0></td>
						<td colspan="2" align="left"><img src="imagens/blackdot.gif" width=115 height=0></td>
						<td align="left"><img src="imagens/blackdot.gif" width=80 height=0></td>
						<td align="left"><img src="imagens/blackdot.gif" width=80 height=0></td>
						<td align="left"><img src="imagens/blackdot.gif" width=110 height=0></td>
						<td align="left"><img src="imagens/blackdot.gif" width=130 height=0></td>
						<td align="left"><img src="imagens/blackdot.gif" width=80 height=0></td>
						<td align="left"><img src="imagens/blackdot.gif" width=15 height=0></td>
					</tr>
				...
				<tr bgcolor="#cococo">
					...
					<td align=center colspan="2">Intervalo</td>
							...
							<td colspan="100%">
								<div class="divrolagem">
									<table cellspacing="1" cellpadding="1" border="0" class="table">
										<tr bgcolor=#FFFFFF id="no_print">
					<!--td><img src="imagens/blackdot.gif" width=8 height=0></td-->
					<td align="left"><img src="imagens/blackdot.gif" width=46 height=0></td>
					<td align="left"><img src="imagens/blackdot.gif" width=69 height=0></td>
					<td align="left"><img src="imagens/blackdot.gif" width=54 height=0></td>
					<td align="left"><img src="imagens/blackdot.gif" width=54 height=0></td>
					<td align="left"><img src="imagens/blackdot.gif" width=69 height=0></td>
					<td align="left"><img src="imagens/blackdot.gif" width=78 height=0></td>
					<td align="left"><img src="imagens/blackdot.gif" width=115 height=0></td>
					<td align="left"><img src="imagens/blackdot.gif" width=130 height=0></td>
					<td align="left"><img src="imagens/blackdot.gif" width=80 height=0></td>
					<%if tela = "1" Then%><td>
						<img src="imagens/blackdot.gif" width=13 height=0></td>
					<%end if%>
				</tr>
<%'*********************************************************
					conta_dias = 0
...
%>
					<tr bgcolor="<%=str_cor_dia%>" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=strcor%>');" class="txt_">
						...
						<td align=center><%'=mens%><font color="<%=dia_cor%>"><%=zero(data_entrada_day)%></font></td>
						<td align=center><%=data_entrada_hour%>:<%=data_entrada_minute%>:<%=data_entrada_second%></td>
						<td align=center><%if data_sai = data_entrada Then%><%response.write("&nbsp;-")%><%else%><%=data_sai_hour%>:<%=data_sai_minute%> 
						</td>
						<td align=center><%=data_entr_Hour%>:<%=data_entr_minute%><%end if%></td>
				   ...
				   <!--ok--><img src="imagens/ic_copy.gif" onclick="javascript:JsCopy('<%=data_entrada_day%>','<%=data_entrada_month%>','<%=data_entrada_year%>','<%=cd_codigo%>','<%=tela%>')" type="button" value="C" title="Copiar"> 						
				   <!--ok--><a href="javascript:void(0)" onclick="window.open('empresa/horas_trabalhadas/horas_trabalhadas_abono.asp?cd_funcionario=<%=cd_codigo%>&dt_data=<%=str_dia%>', 'principal', 'width=200, height=400'); return false;"><img src="imagens/ic_clock.gif" border="0" alt="Abono"></a>
				   <!--ok--><img src="imagens/ic_editar.gif" onclick="javascript:JsEdit('<%=data_entrada_day%>','<%=data_entrada_month%>','<%=data_entrada_year%>','<%=cd_codigo%>','<%=tela%>')" type="button" value="E" title="Editar"> 
				   <!--ok--><a href="javascript:void(0)" onclick="window.open('empresa/janelas/horas_trabalhadas_excluir.asp?cd_cod_horas=<%=cd_cod_horas%>&cd_cod_abono=<%=cd_cod_abono%>', 'principal', 'width=200, height=200'); return false;"><img src="imagens/ic_del.gif" type="button" value="A" title="Apagar" border="0"></a>
						</td>
							<%if tela = "1" Then%>
						<td id="no_print"><img src="../../imagens/blackdot.gif" width=10 height=0></td>
							<%end if%>
					<%end if%>
					</tr>
					<%rs2.movenext
...
'***************************************************
'** 2		/Linhas dos dias de folga/falta\	 ***
'***************************************************
'***********************************************************
						'*	Verifica se há horas abonadas para os dias trabalhados *
						'***********************************************************
...
'**************************************************************
		%>
					<tr bgcolor="<%=str_cor_dia%>" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=str_cor_dia%>');">
						<td align=center><%'=mens%><font color="<%=dia_cor%>"><%=zero(dia_mes)%></font></td>
			   ...
			   <!--ok--><img src="imagens/ic_editar.gif" onclick="javascript:JsEditfolga('<%=dia_mes%>','<%=mes_sel%>','<%=ano_sel%>','<%=cd_codigo%>','<%=tela%>','<%=txt_obs%>','<%=cd_folga%>','<%=cd_cod_folga%>')" type="button" value="E" title="Editar"> 
			   <!--OK--><img src="imagens/ic_del.gif" onclick="javascript:JsDelfolga('<%=cd_codigo%>','<%=cd_cod_folga%>','<%=cd_folga%>','<%=cd_dsr%>')" type="button" value="A" title="Apagar">
							<%if tela = "1" Then%>
							<td id="no_print"><img src="imagens/blackdot.gif" width=10 height=0></td>
							<%end if%>
					<%end if%>
...
if data_entrada_day = "" AND cd_folga < "1" Then%>
					<tr bgcolor="#F5F5F5" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'#F5F5F5');">
						<!--td>&nbsp;</td-->
						<td align=center><%'=mens%><font color="<%=dia_cor%>"><%=zero(dia_mes)%></font></td>
						<td align=left colspan="4"><%=msg_abono%>&nbsp;</td>
						<!--td align=center><img src="imagens/blackdot.gif" width="1" height="0"></td-->
						<!--td align=center></td-->
						<td align=center><%=str_abono_tempo%><img src="imagens/blackdot.gif" width="1" height="0"></td>
						<td align=center>&nbsp;</td>
					...
					<%Else%>
						...
						<img src="imagens/ic_novo.gif" onclick="javascript:JsInsert('<%=dia_mes%>','<%=mes_sel%>','<%=ano_sel%>','<%=cd_codigo%>','<%=tela%>')" type="button" value="I" title="Incluir"><img src="imagens/blackdot.gif" width="2" height="0">
			   <!--ok--><a href="javascript:void(0)" onclick="window.open('empresa/horas_trabalhadas/horas_trabalhadas_abono.asp?cd_funcionario=<%=cd_codigo%>&dt_data=<%=str_dia%>', 'principal', 'width=200, height=400'); return false;"><img src="imagens/ic_clock.gif" border="0"></a>
						<img src="imagens/blackdot.gif" width="11" height="1">
					<%if cd_cod_abono <> "" Then%>
			   <!--ok--><a href="javascript:void(0)" onclick="window.open('empresa/janelas/horas_trabalhadas_excluir.asp?cd_cod_horas=<%=cd_cod_horas%>&cd_cod_abono=<%=cd_cod_abono%>', 'principal', 'width=200, height=150'); return false;"><img src="imagens/ic_del.gif" type="button" value="A" title="Apagar" border="0"></a><%end if%>
						</td>
							<%if tela = "1" Then%>
							<td id="no_print">&nbsp;</td>
							<%end if%>
...
Loop
		...
		<tr id="ok_print"><td colspan="100%"><img src="imagens/blakdot.gif" width="100%" height="1"></td></tr>
				...
				<tr bgcolor="<%=strcor%>">
					...
					<td align="center"><img src="imagens/blackdot.gif" width=1 height=0></td>
					<td id="no_print"><img src="imagens/blackdot.gif" width=1 height=0></td>
					<td><img src="imagens/blackdot.gif" width=1 height=0></td>
				</tr>
				<!--tr bgcolor="<%=strcor%>"-->
					<!--td><img src="imagens/blackdot.gif" width=1 height=0></td>
			...
			'			Loop
				...
				'******************************************************
					...
					if nm_acao = "editar" AND ausencia <> "0" Then
					nm_obs = request("obs")
					ausencia = request("ausencia")
						if ausencia = "1" Then 
						folga = "checked"
						elseif ausencia = "3" then
						falta = "checked"
						end if
					end if
				'******************************************************
				%>
				<form name="form" action="../../acoes/horas_trabalhadas_acao.asp" id="form" method="post">
				<input type="hidden" name="cd_codigo" value="<%=cd_codigo%>">
				<input type="hidden" name="cd_cod_folga" value="<%=cd_cod_folga%>">
				...
				<input type="hidden" name="tela" value="<%=tela%>">
					...
					<td align=center><input type="txt" name="diaentrada" size=2 maxlength="2" value="<%=dia_sel%>" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);"></td>
					...
					<td align=center><input type="radio" name="cd_folga" value="1" size=1 maxlength="1" <%=folga%>><!--***Folga*** -->
									 <!--input type="radio" name="cd_folga" value="2"  size=1 maxlength="1"--><!--***Folga DSR*** -->
									 <input type="radio" name="cd_folga" value="3"  size=1 maxlength="1" <%=falta%>><!--***Falta*** -->
								</td>
					<td align=center><input type="text" name="nm_obs" size="20" value="<%=nm_obs%>"></td>
					<td align=center id="no_print"><input type="submit" value="<%=nm_acao%>"><%=ausencia%>
				</tr>
				<%else%>
				<tr id="no_print">
				...
				<tr bgcolor="<%=strcor%>">
					...
					<td colspan="2"><img src="imagens/blackdot.gif" width=1 height=0><!--input type="submit" value="<%'=acao%>"--></td>
				</tr>
				<tr bgcolor="<%=strcor%>">
					<td><img src="imagens/blackdot.gif" width=1 height=0></td>
					<td><img src="imagens/blackdot.gif" width=1 height=0></td>
				...
				<tr bgcolor="<%=strcor%>">
					...
					<td><img src="imagens/blackdot.gif" width=1 height=0></td><input type="hidden" name="cd_codigo" value="<%=cd_codigo%>">
					<td><img src="imagens/blackdot.gif" width=1 height=0></td><input type="hidden" name="anoentrada" value="<%=ano_sel%>">
					...
					<td colspan="3"><img src="imagens/blackdot.gif" width=1 height=0><%If fechado <> "1" Then%><!--input type="submit" name="banco_horas" value="Banco de horas" id="no_print"--><%end if%></td>
				</tr></form>
				<tr bgcolor=#FFFFFF id="no_print"><!--Separador e limitador de espaços-->
					<td colspan="100%">
						<p>&nbsp;</p>
				...
				<tr id="no_print"><td>&nbsp;<br><br><br></td></tr>
				<%else%>
				<tr><td>&nbsp;Fim</td></tr>
					<%End if%>
			</table>
...
{
		document.location.href('empresa.asp?tipo=digitacao_horas&dia_sel='+cod_a1+'&mes_sel='+cod_a2+'&ano_sel='+cod_a3+'&cd_codigo='+cod_a4+'&tela='+cod_a5+'&acao=inserir');
}
function JsEdit(cod_a, cod_b, cod_c, cod_d, cod_e)
{
		document.location.href('empresa.asp?tipo=digitacao_horas&dia_sel='+cod_a+'&mes_sel='+cod_b+'&ano_sel='+cod_c+'&cd_codigo='+cod_d+'&tela='+cod_e+'&acao=editar');
}
function JsDelete(cod1, cod2, cod3, cod4, cod5, cod6)
{
  if (confirm ("Tem certeza que deseja excluir?"))
	  {
		document.location.href('acoes/horas_trabalhadas_acao.asp?cd_codigo='+cod1+'&cd_cod_horas='+cod2+'&acao=excluir&mesentrada=<%=mes_entrada%>&anoentrada=<%=ano_entrada%>&menos_min='+cod3+'&cd_excedente=<%=excedente%>&cd_minutos_trab='+cod4+'&tela='+cod5+'&cd_cod_abono='+cod6+'');
	  }
}
function JsCopy (cod_a, cod_b, cod_c, cod_d, cod_e)
{
		document.location.href('empresa.asp?tipo=digitacao_horas&dia_sel='+cod_a+'&mes_sel='+cod_b+'&ano_sel='+cod_c+'&cd_codigo='+cod_d+'&tela='+cod_e+'&acao=copiar');
}
function JsEditfolga(cod_a, cod_b, cod_c, cod_d, cod_e, cod_f, cod_g, cod_h)
{
		document.location.href('empresa.asp?tipo=digitacao_horas&dia_sel='+cod_a+'&mes_sel='+cod_b+'&ano_sel='+cod_c+'&cd_codigo='+cod_d+'&tela='+cod_e+'&obs='+cod_f+'&ausencia='+cod_g+'&cd_cod_folga='+cod_h+'&acao=editar');
}
