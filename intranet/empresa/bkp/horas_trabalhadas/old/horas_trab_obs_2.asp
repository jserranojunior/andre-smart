		...
		<td align="center">
			...
			<table width="100%" cellspacing="1" cellpadding="1" border="0" class="textoPadrao">
				...
				<tr>
					...
					'do while Not dia_mes = 10 'ultimo_dia' + 1
	do while not dia_mes = ultimo_dia' + 1
					
				
					
			if not rs2.eof then 'Testa se checgou ao fim do BD
...
'****************
					...
					end if 'Se chegou no fim do BD, pára aqui

					if dia_mes < 1 then
					'rs2.movenext
					else 'if dia_mes <= 1 then
						if not rs2.EOF then
						rs2.movenext
						end if
					end if
					'*** Grava os dias que já foram impressos na tela
					
					if data_entrada_day = dia_mes then
					'response.write("igual")
					aviso = dia_mes & " - igual"
					teste = teste
					'Elseif data_entrada_day = "" then
					'dia_mes = dia_mes
					Elseif data_entrada = "" then
					aviso = "vazio"
					Elseif teste = int(0) then
					dia_mes = dia_mes
					teste = teste
					aviso = "zero"
					'Elseif data_entrada_day = int(1) then
					'dia_mes = dia_mes - 1
					'teste = teste - 1
					'aviso = dia_mes & " - menor<br>"
					Elseif data_entrada_day < dia_mes then
					'dia_mes = dia_mes - 1
					'Elseif data_entrada_day > dia_mes then
					'aviso = "Looping"
					aviso = "menor"
					...
					'if dias_avaliacao <> 0then 
					'dia_mes = dia_mes - 1
					'end if
					
					'dias_unicos = dias_unicos&"," & data_entrada_day
					
					dias_avaliacao = instr(1,dias_unicos,dia_mes,1)
					
					response.write(dias_unicos)
					response.write(" ,("&dias_avaliacao&")")
					
				if data_entrada_day = dia_mes then ' Escreve apenas as linhas presentes no BD.
					'rs2.movenext
					...
					<tr bgcolor="<%=strcor%>"  onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=strcor%>');">
						<td align=center width=172><%=mens%><%=dia_mes%><br><%=data_entrada_day%><%=aviso%></td>
						<td align=center width=172><%=teste%> - <%=data_entrada_hour%>:<%=data_entrada_minute%>:<%=data_entrada_second%></td>
						...
						<td align=center width=172>&nbsp;<%=BH%></td>
						<td align=left width=172>&nbsp;<%=nm_obs%></td>
					<%
					If fechado = "1" Then%>
					...
					excedente = strsun - cd_carga_horaria%>
						...
						<input onclick="javascript:JsInsert('<%=data_entrada_month%>','<%=data_entrada_year%>','<%=cd_codigo%>')" type="button" value="I" title="Incluir"> 
			...
			<%end if 'Fim das linhas%>
					...
					dias_unicos = dias_unicos&"," & data_entrada_day
					'dias_unicos = dias_unicos&"," & data_entrada_day
					'dias_avaliacao = instr(1,dia_mes,dias_unicos ,1)
					
'***** Untitled fim ******
						dias_unicos = dias_unicos
						dia_mes = dia_mes + 1
						'data_entrada_day = dia_mes
					Loop
							rs2.close
							set rs2 = nothing
					%>
				...
				<tr>
					...
					<td align=center>Entrada Intervalo</td>
					<td align=center>Saida</td>
					<td colspan="2" align="center">Observações</td>
					<td width=5 align=center><img src="imagens/blackdot.gif" width=1 height=0></td>					
				<tr>
			   <%if acao = "editar" then
			...
			'			Loop
				...
				<input type="hidden" name="total_geral_trabalhado" value="<%=strsun%>">
					...
					<td align=center class="txt_cinza"><input type="txt" name="diaentrada" size=2 maxlength="2" value="<%if dia_entrada > 0 AND dia_entrada < 10 Then%><%response.write("0"&dia_entrada)%><%else%><%=dia_entrada%><%end if%>" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);"></td>
...
{
		document.location.href('empresa_funcionarios_obs.asp?mes_sel='+cod_a1+'&ano_sel='+cod_a2+'&cd_codigo='+cod_a3+'&acao=inserir');
}
