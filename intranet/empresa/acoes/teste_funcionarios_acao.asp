...
strhr_entrada = dt_atualizacao&" "&request("hr_entrada")
	'strhr_entrada 
strhr_saida = request("hr_saida")
	'Verifica se o horário é noturno e altera o dia
	if hour(strhr_entrada) > hour(strhr_saida) then
		dia_atualizacao = day(dt_atualizacao) '+ 1
		mes_atualizacao = month(dt_atualizacao)+1
		ano_atualizacao = year(dt_atualizacao)
			if dia_atualizacao > ultimodiames(mes_atualizacao,ano_atualizacao) then
				dia_atualizacao = 1
		...
		strhr_saida = dt_atualizacao&" "&request("hr_saida")
	end if
...
if x = "" then
	If stracao = "insert" Then
		...
		dbconn.execute(xsql)
			...
			'Insere o horário do funcionario recem cadastrado
				xsql = "up_funcionario_horario_insert @cd_funcionario='"&cd_cod_funcionario&"', @hr_entrada='"&strhr_entrada&"', @nm_intervalo='"&strnm_intervalo&"', @hr_saida='"&strhr_saida&"', @dt_atualizacao="&Strdt_contratado&""
				dbconn.execute(xsql)
	...
	'*****************************************************************************
			...
			'********************************************************************************
				...
				'response.redirect "../funcionario/funcionarios_cadastro.asp?tipo=cadastro&pag=&cod="&cd_cod_funcionario&"&msg="&strmsg&""
	...
	ElseIf stracao = "altera" Then
		...
		response.redirect "../funcionario/funcionarios_cadastro.asp?tipo=cadastro&cod="&strcod&""
	...
	ElseIf stracao = "horario" Then
		'Insere dados
		xsql = "up_funcionario_horario_insert @cd_funcionario='"&strcod&"', @hr_entrada='"&strhr_entrada&"', @nm_intervalo='"&strnm_intervalo&"', @hr_saida='"&strhr_saida&"', @dt_atualizacao='"&cd_mes&"/"&cd_dia&"/"&cd_ano&"'"
		dbconn.execute(xsql)
		fecha_janela = 1
