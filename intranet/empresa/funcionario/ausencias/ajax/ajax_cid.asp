...
<%Elseif IsNumeric(mid(cd_cid,2,2)) then ' *** Mostra as subcategorias ***%>
	<%xsql = "SELECT top 10 * FROM TBL_cid10_subcategoria where subcategoria Like '%"&cd_cid&"%' Order by subcategoria" '*** Mostra o nome da subcategoria
		...
		<%if not rs_cid.EOF Then%>
			...
			%>
			<a href="javascript:viod(0);" onclick="preenche_cid('<%=trim(subcategoria)%>');"><%=mid(subcategoria,1,3)%> <%if int(len(trim(subcategoria))) > 3 then response.write("."&mid(subcategoria,4,4)) end if%></a> - <%=descricao%> <%=rest_sexo%> <%=restricao%> <%=aviso%><br>
			<%rs_cid.MoveNext
			espec_sel = ""
			wend
		else%>
...
<%Elseif not IsNumeric(mid(cd_cid,2,2)) then%>
	...
	descricao = rs_cid("descricao")
		...
		if len(descricao) > 77 then
			...
			<a href="javascript:viod(0);" onclick="preenche_cid('<%=trim(subcategoria)%>');"><%=mid(subcategoria,1,3)%>.<%=mid(subcategoria,4,4)%></a> - <%=descricao%><br>
		<%rs_cid.MoveNext
		wend%>
	<%else%>
		<b style="color:yellow; border:1 solid gray; background-color:gray; width:587px;">CÓD. &nbsp; - &nbsp;CATEGORIA </b><br>
		* CID não encontrado * <%=cd_cid%>
	<%end if%>
