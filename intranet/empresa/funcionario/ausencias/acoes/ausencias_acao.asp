...
If acao = "insert" then
	'*** Registra a falta ***
		...
		dbconn.execute(xsql)
		response.redirect "../../../../empresa.asp?tipo=cad_ausencia&cod="&cd_funcionario&""
Elseif acao = "update" then
elseif acao = "delete" then

elseif acao = "insert_ferias" Then
	'*** Registra a falta ***
		xsql ="up_funcionario_ferias_insert @cd_funcionario="&cd_funcionario&", @cd_empresa="&cd_empresa&", @cd_unidade="&cd_unidade&", @cd_motivo="&cd_motivo_falta&", @nm_cid='"&cd_cid&"', @cd_user="&cd_user&", @dt_cadastro='"&data_atual&"', @dt_ausencia="&dt_ausencia&", @dt_afastamento_i="&dt_afastamento_i&", @dt_afastamento_f="&dt_afastamento_f&", @nm_obs='"&nm_obs&"'"
		dbconn.execute(xsql)
		response.redirect "../../../../empresa.asp?tipo=cad_ausencia&cod="&cd_funcionario&""
End if
