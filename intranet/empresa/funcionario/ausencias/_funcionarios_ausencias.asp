	...
	if not rs.eof then%>		
			...
			Do While Not rs.eof 
				...
				desliga = rs("dt_desliga")
					...
					%>
				 <tr onmouseover="mOvr(this,'#CFC8FF');"  onclick="javascript:location.replace('empresa.asp?tipo=cad_ausencia&pag=<%=strpag%>&cod=<%=rs("cd_funcionario")%>&busca=<%=strbusca%>');" onmouseout="mOut(this,'<%=cor_linha%>');" style="height:20px;" bgcolor="<%=cor_linha%>">
