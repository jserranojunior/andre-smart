...
unidade = request("unidade")
	'if unidade > "0" Then
		'busca_unidade = " AND cd_unidade = "&unidade&" "
		'busca_unidade = " AND nm_sigla = HSLI"
	'else
	'	unidade = 0
	'end if
ctps = request("ctps")
...
'----------------------------------------------------------------
outras_variaveis = busca_nome&busca_sexo&busca_endereco&busca_numero&busca_complemento&busca_cep&busca_bairro&busca_cidade
outras_variaveis = outras_variaveis&busca_naturalidade&busca_nascimento&busca_filiacao_m&busca_filiacao_p&busca_est_civil&busca_conjuge
outras_variaveis = outras_variaveis&busca_rg&busca_cpf&busca_coren&busca_ctps&busca_pis&busca_tit_eleitor
outras_variaveis = outras_variaveis&busca_contratos&busca_agencia&busca_conta
outras_variaveis = outras_variaveis&busca_transp&busca_refei&busca_alim&busca_medic&busca_odonto
...
<table align="center" border="1" class="no_print">
	...
	<%end if%>
	<tr><td colspan="100%" align="center" style="background-color:#000099; color:white; font-size=15; font-weight:bold;">LISTAGEM <%=sexo%></td></tr>
	...
	<tr class="<%=str_seccao%>" style="display:<%=escolha_d_prof%>;">
		...
		<td><%strsql ="TBL_cargos where cd_especificidade = 3 order by nm_cargo"
			Set rs_cargo = dbconn.execute(strsql)%>
				<select name="cargo" class="inputs"> 
					<option value="0">Selecione</option>
					<%Do While Not rs_cargo.eof
						cod_cargo = rs_cargo("cd_codigo")
						nome_cargo = rs_cargo("nm_cargo")%>
					<option value="<%=cod_cargo%>" <%if int(cod_cargo) = abs(cargo) then response.write("selected")%>><%=nome_cargo%> <%=cod_cargo&":"&cargo%></option>
					<%rs_cargo.movenext
					loop
					rs_cargo.close
		...
		<td><%strsql ="TBL_unidades where cd_hospital > 0 and cd_status = 1"
			Set rs_uni = dbconn.execute(strsql)%>
				<select name="unidade" class="inputs"> 
					...
					<option value="<%=cod_unidade%>" <%if int(cod_unidade) = abs(unidade) then response.write("selected")%>><%=nome_unidade%></option>
					<%rs_uni.movenext
					loop
					rs_uni.close
					Set rs_uni = nothing%>
	...
	<tr>
					...
					'	response.write(func_ativos)
							...
							while not rs_func.EOF
								...
								if unidade <> "0" OR cargo <> "0" Then
									 if int(cd_unidade) <> int(unidade) then mostrar_linha_uni = 0 else mostrar_linha_uni = 1
									 if int(cd_cargo) <> int(cargo) then mostrar_linha_carg = 0 else mostrar_linha_carg = 1
									'end if
								end if
								
									if mostrar_linha_uni <> "0" AND mostrar_linha_carg <> "0" then '*** Inibe linhas pela busca ***
									'if mostrar_linha = "1" then '*** Inibe linhas pela busca ***
										%>
									<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=cor_linha%>');" style="height:20px; color:<%=cor_registro%>;" bgcolor="<%=cor_linha%>">
										<td align="center" valign="top" style="color:gray;">
											...
											<%'=%></td>
										<td align="right" valign="top"><%=cd_matricula%> * <%=mostrar_linha_uni&","&mostrar_linha_carg%></td>
										<td valign="top" onclick="javascript:window.open('empresa/funcionario/funcionarios_ficha.asp?tipo=cadastro&pag=<%=strpag%>&cod=<%=rs_func("cd_funcionario")%>&busca=<%=strbusca%>','ficha_<%=rs_func("cd_funcionario")%>','width=700,height=700,scrollbars=1');"><%=nm_funcionario%></td>
										...
										<%if campo_sex <> 0 Then%><td valign="top"><%=nm_sexo%> <%=unidade%></td><%end if%>
										...
										<%'if campo_unidade <> 0 Then%><td valign="top"><%=nm_sigla%>-(<%=unidade&":"&cd_unidade%>)</td><%'end if%>
									...
									<%if campo_contratos <> 0 Then%>
										<%xsql = "SELECT * FROM  VIEW_funcionario_contrato_lista WHERE (cd_funcionario = '"&cd_funcionario&"') order by dt_admissao"
											Set rs_contr = dbconn.execute(xsql)
												...
												wend%>									
									<%end if%>
										<td class="no_print"><%'=unidade&" - "&cargo%><!--img src="../../imagens/ic_editar.gif" alt="Editar" width="13" height="14" border="0" onclick="javascript:location.replace('empresa.asp?tipo=cadastro&pag=<%=strpag%>&cod=<%=rs_func("cd_funcionario")%>&busca=<%=strbusca%>');"-->
										<img src="../../imagens/ic_editar.gif" alt="Editar" width="13" height="14" border="0" onclick="javascript:window.open('empresa/funcionario/funcionarios_cadastro.asp?tipo=cadastro&pag=<%=strpag%>&cod=<%=rs_func("cd_funcionario")%>&busca=<%=strbusca%>','Cadastro_<%=rs_func("cd_funcionario")%>','width=760,height=700,scrollbars=1');"></td>
									</tr>
								<%
							...
							end if '*** Inibe linhas pela busca ***
