...
<%qtd_dias_dsr = qtd_domingos_comp + qtd_feriados%>
					...
					<table border="1" cellspacing="0" cellpadding="2" align="center" width="100" class="no_print">
						...
						<form name="data" action="<%=strcat%>.asp"  method="POST">
							...
							<tr>
								<td colspan="4" align="center" class="cabecalho" style="background-color:black; color:white;"><%'=data_atual%> <b>VARIÁVEIS - <%=tit_doc%> <%if strregime_trabalho <> "" Then%><%response.write("- "&strregime_trabalho&"."&strmatricula)%><%end if%></b></td>
							...
							<tr>
								<td align="left" colspan="1" style="background-color:silver;"><input type="button" name="Contabilidade" value="Contabilidade" onClick="window.open('empresa/funcionario/funcionarios_folhapagamento_var_contabilidade.asp','nome','location=0,status=1,width=750,height=600,scrollbars=1')"></td>
								<td align="left" colspan="1" style="background-color:silver;"><input type="submit" name="ok" value="Gerar variaveis"></td></tr>
						</form>
					</table>
			...
			<!-- ---------------------------------------------------------------- -->				
					...
					xsql = "up_funcionario_folhapagamento_geral @dt_comp_i='"&mes_competencia&"/1/"&ano_competencia&"', @dt_comp_f='"&mes_competencia&"/"&final_competencia&"/"&ano_competencia&"', @dt_i = '"&dt_mes&"/1/"&dt_ano&"', @dt_f = '"&dt_mes&"/"&dia_final&"/"&dt_ano&"',@nm_ordem='"&nm_ordem&"'"
						Set rs = dbconn.execute(xsql)
							while not rs.EOF
					...
					<!-- *** Benefícios *** -->
								...
								<td align="right"><%if nr_vt >= 1 then%><%=zero(nr_vt)%><%else%>0<%end if%></td>
								<td align="right"><%if nr_vt_extra >= 1 then%><%=zero(nr_vt_extra)%><%else%>0<%end if%>
								<td align="right"><%if total_vt >= 1 then%><%=zero(total_vt)%><%else%>0<%end if%></td>
					...
					<!-- *** Total Liquido *** -->	
						...
						<%num_funcionario = num_funcionario + 1
							...
							<tr>
								...
								<td align="right">&nbsp;<b><%=tt_hextra%></b></td>
								<td align="right">&nbsp;<b><%=tt_hextra_plus%></b></td>
								<td align="right">&nbsp;<b><%=tt_faltas_justif%></b></td>
								<td align="right">&nbsp;<b><%=tt_faltas_injust%></b></td>
								<%'if strtipo <> "var_enf" then%><!--td>&nbsp;DSR</td-->	<%'end if%>
								<td align="right">&nbsp;<b><%=tt_atrasos%></b></td>
								<%if strtipo <> "var_enf" then%><td align="right">&nbsp;<b><%=tt_vr%></b></td><%end if%>
									...
									<td align="right">&nbsp;<b><%=tt_total_vr%></b></td>
									
								<%if strtipo <> "var_enf" then%><td align="right"><b><%=tt_vt%></b></td><%end if%>
									<td align="right">&nbsp;<b><%if tt_vt_extra > 0 then%><%=tt_vt_extra%><%else%>0<%end if%></b></td>
									<td align="right">&nbsp;<b><%=tt_total_vt%></b></td>
							</tr>
