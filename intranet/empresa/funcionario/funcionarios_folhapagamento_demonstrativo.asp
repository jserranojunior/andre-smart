...
-->
					...
					<table border="1" cellspacing="0" cellpadding="2" align="center" width="100" class="no_print" style="border-collapse:collapse;">
							...
							<tr>
								...
								<td align="center" style="background-color:silver;">Competência:</td>
								<td align="center" style="background-color:silver;" colspan="2"><b><%=mesdoano(mes_competencia)&"/"&ano_competencia%></b></td>									
							</tr>
			...
			<!-- ---------------------------------------------------------------- -->				
							...
							xsql = "SELECT * FROM TBL_funcionario_valores_padroes WHERE (cd_tipo = 12) AND dt_atualizacao <= '"&mes_competencia&"/"&final_competencia&"/"&ano_competencia&"'"
							Set rs = dbconn.execute(xsql)
							nr_salminimo = rs("nr_valor")
							'****************************
							'*** VALOR Aux. Creche	  ***
							xsql = "SELECT * FROM TBL_funcionario_valores_padroes WHERE (cd_tipo = 6) AND dt_atualizacao <= '"&mes_competencia&"/"&final_competencia&"/"&ano_competencia&"'"
							Set rs = dbconn.execute(xsql)
			...
			<!-- ------------------------------------------- V E N C I M E N T O S ------------------------------------------- -->
							...
							'end if%>
							<%'*** Informa os dados o dependente menor que 7 anos ***
			...
			<!-- ------------------------------------------- D E S C O N T O S ------------------------------------------- -->
							...
							<tr style="background-color:<%=cor_linha%>;">
								...
								<td align="right" colspan="3">&nbsp;Desc. dependente até 21a (<%=qtd_dependentes_21%>): &nbsp; &nbsp; &nbsp; <%=deduc_dependente%></td>
