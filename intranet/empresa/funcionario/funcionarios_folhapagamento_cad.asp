...
<body onload="foco();" onunload="window.opener.data.submit(true);">
					<table border="1" cellspacing="0" cellpadding="2" align="center" width="100" class="no_print" style="border-collapse:collapse">
						...
						<form name="data" action="funcionarios_folhapagamento_cad.asp"  method="POST">
							...
							<tr>
								...
								<td align="center" style="background-color:silver;" colspan="5"><b><%=mesdoano(dt_mes)&"/"&dt_ano%></b>
									<!--select name="dt_mes">
										<%for i = 1 to 12%>
			...
			<!-- ---------------------------------------------------------------- -->				
						...
						<form name="form" action="../acoes/folha_pagamento_acao.asp"  method="POST">
							...
							<tr>
								...
								<td colspan="6" onClick="window.open('../../empresa/funcionario/funcionarios_ficha.asp?tipo=cadastro&cod=<%=strcod%>','','width=630,height=600,scrollbars=1')"><%=nm_nome%></td>
							</tr>
							<tr>
			...
			<!-- ------------------------------------------- V A R I A V E I S ------------------------------------------- -->							
							<tr style="background-color:<%=cor_linha%>;">
								...
								<%if strcat <> "enfermagem" then%><td align="center"><b>Qtd. DSR<br>( Semanas )</b><input type="hidden" name="cd_dsr_faltas" value="<%=cd_dsr_faltas%>"><br></td><%end if%>
							...
							<tr style="background-color:<%=cor_linha%>;">
								...
								<td align="center">&nbsp;<%if competencia >= 0 then%><input type="text" name="qtd_plantoes" size="3" maxlength="3" onFocus="nextfield ='ad_noturno';" <%=styleinput%>><%else%>-<%end if%></td>
								...
								<td align="center">&nbsp;<%if competencia >= 0 then%><input type="text" name="nr_faltas_justif" size="3" maxlength="2" onFocus="nextfield ='nr_dsr_faltas_justif';" <%=styleinput%>><%else%>-<%end if%></td>
								<td align="center">&nbsp;<%if competencia >= 0 then%><input type="text" name="nr_faltas_injust" size="3" maxlength="2" onFocus="nextfield ='nr_dsr_faltas_injust';" <%=styleinput%>><%else%>-<%end if%></td>
								<%if strcat <> "enfermagem" then%><td align="center">&nbsp;<%if competencia >= 0 then%><input type="text" name="nr_dsr_faltas" size="2" maxlength="1" onFocus="nextfield ='nr_atrasos'" <%=styleinput%>><%else%>-<%end if%></td><%end if%>
							...
							<tr style="background-color:<%=cor_linha%>;">
								...
								<td align="right"><%=zero(qtd_plantoes)%></td>
								...
								<td align="right">&nbsp;<%=nr_faltas_justif%><%if nr_faltas_justif = 1 then%> dia<%elseif nr_faltas_justif > 1 then%> dias<%end if%></td>
								<td align="right">&nbsp;<%=nr_faltas_injust%><%if nr_faltas_injust = 1 then%> dia<%elseif nr_faltas_injust > 1 then%> dias<%end if%></td>
								<%if strcat <> "enfermagem" then%><td align="right">&nbsp;<%=nr_dsr_faltas%><%if nr_dsr_faltas >= 1 then%> dom.<%end if%></td><%end if%>
						...
						<%if strcat <> "enfermagem" then%>
							<tr style="background-color:<%=cor_linha%>;">
								...
								<td align="right">&nbsp;<%if nr_faltas_justif >=1 then  response.write(FormatNumber(nr_faltas_justif*nr_salario_dia,2))%></td>
								<td align="right">&nbsp;<%if nr_faltas_injust >=1 then  response.write(FormatNumber(nr_faltas_injust*nr_salario_dia,2))%></td>
								<%if strcat <> "enfermagem" then%><td align="right">&nbsp;<%if nr_dsr_faltas >=1 then  response.write(FormatNumber(nr_dsr_faltas*nr_salario_dia,2))%></td><%end if%>
		...
		<!-- ************* FIM DOS DESCONTOS **************** -->
				...
				<!-- ------------------------------------------- T O T A L		L Í Q U I D O ------------------------------------------- -->
							...
							<tr>
								...
								<td align="center" colspan="7">&nbsp;<%if competencia >= 0 then%><input type="image" src="../../imagens/bt.gif" alt="Confirmar" width="119" height="22" border="0"><%end if%></td>
							</tr>
							<%else%>
							<tr>
