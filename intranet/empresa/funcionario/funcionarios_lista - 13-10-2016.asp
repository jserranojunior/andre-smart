...
campos = request("campos")
	...
	campo_vacina = Instr(1,campos,"nr_hepatite_b,nr_dupla_adulto,dt_dupla_adulto_validade,nr_scr",0)
	'campo_vacina = Instr(1,campos,"nr_hepatite_b,nr_dupla_adulto",0)
...
<tr><td colspan="4"><b>Vacinas</b>
	...
	<tr class="<%=str_seccao%>" style="display:<%=escolha_vacina%>;">
		<td>&nbsp;</td>
		<td><input type="checkbox" name="campos" value="_hepatite_b" <%if campo_vacina <> 0 Then response.write("CHECKED") end if%> style=border:0px;> Vacina</td>
		<td align="right">&nbsp;</td>
...
<table align="center">
	...
	<tr><td colspan="11" align="center" style="background-color:gray;color:white; font-size:14px;"><b>Listagem de vacinação - Hospital São Luiz Morumbi - 2012</b></td></tr>
	<tr>
					<%cor_linha = "#FFFFFF"
					cor = 1
					cabecalho = 0
							...
							while not rs_func.EOF
								...
								'*** Mostra o cabeçalho ****
									...
									if cabecalho = "0" then%>
										...
										<tr bgcolor=#c0c0c0>
											...
											<td align="center"><b>hepat</b></td>
											<td align="center"><b>Dupla</b></td>
											<td align="center"><b>Dpl/val</b></td>
											<td align="center"><b>SCR</b></td>
								...
								IF mostrar_linha = 1 Then 'inibe a linha desejada%>
									...
									<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=cor_linha%>');" style="height:20px; color:<%=cor_registro%>;" bgcolor="<%=cor_linha%>">
										...
										<td><%if nr_hepatite_b > 0 then%><%=nr_hepatite_b%>º dose<%end if%></td>
										<td><%if nr_dupla_adulto > 0 then%><%=nr_dupla_adulto%>º dose<%end if%></td>
										<td><%if dt_dupla_adulto_validade <> "" then%><%=zero(day(dt_dupla_adulto_validade))&"/"&zero(month(dt_dupla_adulto_validade))&"/"&year(dt_dupla_adulto_validade)%><%end if%></td>
										<td><%if nr_scr > 0 then%><%=nr_scr%>º dose<%end if%></td>
									...
									<%end if%>
										...
										<img src="../../imagens/ic_editar.gif" alt="Editar" width="13" height="14" border="0" onclick="javascript:window.open('empresa/funcionario/funcionarios_cadastro.asp?tipo=cadastro&pag=<%=strpag%>&cod=<%=rs_func("cd_funcionario")%>&busca=<%=strbusca%>','Cadastro_<%=rs_func("cd_funcionario")%>','width=770,height=700,scrollbars=1');"></td>
									</tr>
								<%if cor > 0 then
									cor_linha = "#d7d7d7"
									cor = 0
	...
	<%if func_ativos <> 2 Then
		...
		xsql = "SELECT DISTINCT cd_contrato, nm_regime_trab FROM VIEW_funcionario_contrato_lista WHERE admissao <= '"&ano_sel&mes_sel&"' AND demissao > '"&ano_sel&mes_sel&"' OR admissao <= '"&ano_sel&mes_sel&"' AND dt_demissao IS NULL GROUP BY cd_contrato,nm_regime_trab"
			Set rs_contr = dbconn.execute(xsql)
				...
				nm_regime_trab = rs_contr("nm_regime_trab")%>
		<tr class="no_print"><td colspan="3"><hr></td></tr>
		<tr class="no_print">
				...
				data_sel = ano_anterior&zero(mes_anterior)
					...
					Set rs_conta = dbconn.execute(xsql)
						...
						wend
							...
							%> 
			<tr class="no_print"><td colspan="3">Saldo inicio Mês: <b><%=int(conta)%></b> <%'="("&data_sel&")"%></td></tr>
			<tr class="no_print"><td colspan="3">Admissões: <b style="color:green;"><%=total_admissao%></b></td></tr>
			<tr class="no_print"><td colspan="3">Recisões: <b style="color:red;"><%=total_recisao%></b></td></tr>
			<tr class="no_print"><td colspan="3">TOTAL: <b><%total_geral=(conta+total_admissao)-total_recisao%> <%=total_geral%></b></td></tr>			
		...
		wend%>
		<tr class="no_print"><td colspan="3"><hr></td></tr>
		<tr class="no_print"><td colspan="3"><b>HEAD COUNT: <%=total_funcionarios%></b> <%if total_funcionarios > 1 then%>colaboradores<%else%>Colaborador<%end if%> </td></tr>
		<%end if%>
		<!--tr class="no_print">
			<td colspan="100%">
				SELECT * FROM VIEW_funcionario_contrato_lista <br>
		...
		<tr>
			...
			<%if campo_sexo <> 0 Then%><td><img src="../../imagens/px.gif" alt="" width="30" height="1" border="0"></td><%end if%>
			...
			<%if campo_ctps <> 0 Then%><td><img src="../../imagens/px.gif" alt="" width="100" height="1" border="0"></td><%end if%>
			<%if campo_pis <> 0 Then%><td><img src="../../imagens/px.gif" alt="" width="100" height="1" border="0"></td><%end if%>
