...
<body onload="foco();" <%=atualiza_origem%>>
					<table border="1" cellspacing="0" cellpadding="2" align="center" width="100" class="no_print" style="border-collapse:collapse">
						...
						<form name="data" action="funcionarios_folhapagamento_cad.asp"  method="POST">
							...
							<tr>
								<td colspan="9" align="center" class="cabecalho" style="background-color:black; color:white;"><b>RCM - <%=strtipo%><%if strregime_trabalho <> "" Then%><%response.write("- "&strregime_trabalho&"."&strmatricula)%><%end if%></b></td>
							...
							<tr>
								<td colspan="9" align="center">&nbsp;
									<table border="1" style="border-collapse:collapse;">
										<%strsql = "SELECT * FROM View_funcionario_rcm WHERE cd_funcionario="&strcod&" AND dt_rcm BETWEEN '"&mes_competencia&"/1/"&ano_competencia&"' AND '"&mes_competencia&"/"&final_competencia&"/"&ano_competencia&"' order by dt_rcm"
										...
										<tr>
											...
											<td align="center" valign="top"><%=sinal_arredonda%><%=(qtd_he)%></td>
											...
											<td align="center" valign="top"><%if nm_obs <> "" then%><a href="javascript:void(0);" return false;" onmouseout="hideandshow_a(<%=cd_rcm%>);"><div class="<%=cd_rcm%>" style="position:static; left:460px; top:100px; background-color:white; border:2px solid white; display:none;"><%=nm_obs%></div></a><a href="javascript:void(0);" return false;" onmouseover="hideandshow_a(<%=cd_rcm%>);"><b style="color:orange; display:block;" class="<%=cd_rcm%>">X</b></a><%else%>&nbsp;<%end if%></td>
											<td align="center" valign="top">&nbsp;<!--img src="../../imagens/ic_editar.gif" alt="" width="13" height="14" border="0"--></td>
											<td align="center" valign="top"><%if strtipo = "cadastro" then%><img src="../../imagens/ic_del.gif" alt="" width="13" height="14" border="0" onClick="javascript:JsDelete('<%=cd_rcm%>','<%=strcod%>','<%=dt_dia%>','<%=dt_mes%>','<%=dt_ano%>');"><%else%>&nbsp;<%end if%></td>
										</tr>
										<%dt_rcm = ""
										hora_i = ""
										hora_f = ""
										...
										<tr bgcolor="silver">
											...
											<td align="center">&nbsp;</td>											
											<td align="center">&nbsp;<b><%if total_he > 0 then%><%=(total_he)%><%end if%></b></td>
			...
			<!-- ------------------------------------------- V A R I A V E I S ------------------------------------------- -->							
							<tr style="background-color:<%=cor_linha%>;">
								<td align="center"><b>Dia</b><br><img src="../../imagens/px.gif" alt="" width="60" height="1" hspace="0" vspace="0" border="0"></td>
							...
							<tr style="background-color:<%=cor_linha%>;">
								...
								<td align="center">&nbsp;<%if competencia >= -1 then%><input type="text" name="dt_hora_f" size="2" maxlength="2" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);" <%=styleinput%>>:<input type="text" name="dt_min_f" size="2" maxlength="2" <%=styleinput%>><%else%>-<%end if%></td>
								...
								<td align="center">&nbsp;<%if competencia >= -1 then%><input type="text" name="nr_atrasos" size="2" maxlength="2" <%=styleinput%>><%else%>-<%end if%></td>
							...
							<%End if%>
							<tr><td colspan="9"><input type="checkbox" name="nao_fecha_janela" value="1" <%if nao_fecha_janela = 1 then response.write("Checked")%> > Impede o fechamento desta janela.&nbsp;<!--* hora extra arredondada--></td></tr>
							</form>
						<%End if%>
						</table>
	...
	function JsDelete(cod,cod1,cod2,cod3,cod4)
		{//alert(cod);
			if (confirm ("Tem certeza que deseja excluir o registro?"))
				{
					document.location.href('../acoes/rcm_acao.asp?cd_rcm='+cod+'&cd_funcionario='+cod1+'&dt_dia='+cod2+'&dt_mes='+cod3+'&dt_ano='+cod4+'&acao=rcm_excluir');
				}
