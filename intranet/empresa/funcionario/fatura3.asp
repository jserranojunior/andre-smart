...
data_atual = month(now)&"/"&day(now)&"/"&year(now)&" "&hour(now)&":"&minute(now)%>
						...
						<td>
							<table align="left" border="1" style="border-collapse:collapse;border:1px solid gray;">
								...
								SET rs_proc = dbconn.execute(xsql)
									while not rs_proc.EOF
										...
										nm_procedimento_tipo = rs_proc("nm_procedimento_tipo_abrev")
											...
											<td align="center"><b>Num</b><br><img src="../imagens/px.gif" alt="" width="40" height="1" border="0"></td>
											...
											<tr>
												...
												SET rs_valor = dbconn.execute(strsql)
													...
													<%if cab = 0 then%>											
															...
															<input type="hidden" name="dt_recebimento_<%=strcd_unidade&"_"&cab%>" value="<%=mes_recebimento&"/"&dia_recebimento&"/"&ano_recebimento%>">												
														<%else%>
															-
														<%end if%></td>
											</tr>
								...
								<tr>
									...
									<input type="text" name="qtd_empr_<%=strcd_unidade%>" size="3" maxlength="5" value="<%=nm_qtd_empr%>"></td>
									<td colspan="2">&nbsp;</td>
									<td colspan="1">&nbsp;<%if not isnull(nm_dias_faturamento2) then response.write(zero(dia_recebimento2)&"/"&zero(mes_recebimento2))%></td>
								</tr>
							</table>							
						</td>
			...
			Set rs_pref = dbconn.execute(xsql)
				if not rs_pref.EOF Then
					...
					cd_impress_pg3 = rs_pref("cd_impress_pg3")
						...
						if nm_dias_faturamento2 > 30 then
							dia_vencimento2 = nm_dias_faturamento2 - 30
							mes_vencimento2 = dt_mes + 1
							ano_vencimento2 = dt_ano
								if mes_vencimento2 > 12 then
									mes_vencimento2 = 1
									ano_vencimento2 = dt_ano + 1
								end if
						else
							dia_vencimento2 = nm_dias_faturamento2
							mes_vencimento2 = dt_mes
							ano_vencimento2 = dt_ano
						end if
			...
			<table align="center" width="500" border="0" style="border-collapse:collapse;">
				...
				<tr>
					...
					<td colspan="2" valign="top" style="font-family:arial; font-size:18px; text-align:justify; text-justify:inter-word;">
						<%if cd_valor_fixo = 1 then%>
							Recebi da <b><%=nm_empresa%></b>, o recibo de pagamento da locação dos equipamentos de videocirurgia relativo ao mesmo mês com vencimento em <b><%=dia_vencimento%> <%if dia_vencimento2 <> "" Then response.write(" e "&dia_vencimento2)%> de <%=mes_selecionado(mes_vencimento)&" de "&ano_vencimento%></b>.
						<%else%>
							Recebi da <b><%=nm_empresa%></b>, o relatório dos procedimentos realizados por videocirurgia no mês de <%=mes_selecionado(mes_anterior)&" de "&ano_anterior%>, no <b><%=nm_unidade_nome%></b>, junto com o recibo de pagamento da locação dos equipamentos de videocirurgia relativo ao mês com vencimento em <b><%=dia_vencimento%> <%if dia_vencimento2 <> "" Then response.write(" e "&dia_vencimento2)%> de <%=mes_selecionado(mes_vencimento)&" de "&ano_vencimento%></b>.
						<%end if%></td>
				</tr>
								...
								<table border="1" width="500" style="font-family:arial; font-size:15px; border-collapse:collapse;padding:5px 4px; background:transparent;" align="center">
										...
										SET rs_proc = dbconn.execute(strsql)
											while not rs_proc.EOF
												...
												strsql = "SELECT * FROM TBL_unidades_valores_procedimentos WHERE (cd_unidade = "&cod_unidade&") order by dt_atualizacao desc"
												SET rs_valor = dbconn.execute(strsql)
													if not rs_valor.EOF then
												...
												<tr>
													<td align="center" style="padding:15px 5px;"><%if cd_tipo_proc <>  1 then%><%=nm_procedimento_tipo%><%else%><b>Total</b><%end if%></td>
										...
										<tr>
											...
											<td align="center" style="padding:15px 5px;" colspan="3"><%=zero(dia_vencimento)&"/"&zero(mes_vencimento)&"/"&ano_vencimento%><%if not isnull(nm_dias_faturamento2) then response.write(" - "&formatCurrency(valor_total/2))%><br>
											<%if not isnull(nm_dias_faturamento2) then response.write(zero(dia_vencimento2)&"/"&zero(mes_vencimento2)&"/"&ano_vencimento2&" - "&formatCurrency(valor_total/2))%></td>
										</tr>
								</table>
	...
	*****************************************-->
			...
			<table width="500" border="0" align="center" style="border-collapse:collapse;">
				...
				<tr>
					<td>&nbsp;<!-- limitador de linha --></td>
					<td colspan="2" style="font-family:arial; font-size:15px; text-align:right;">São Paulo, <%=zero(dia_vencimento)&" de "&mes_selecionado(mes_vencimento)&" de "&ano_vencimento%>.</td>
				</tr>
				<tr>
					...
					<td colspan="2" valign="top" style="font-family:arial; font-size:15px; text-align:justify; text-justify:inter-word;">
						<table border="0" width="625">
							<tr>
								...
								Recebemos da empresa infra citada a quantia de <%=formatCurrency(valor_total_recibo)%> (<%response.write(extenso(valor_total_recibo,1))%>) referente a locação de equipamentos e instrumentais para videocirurgias, no período de 01 a <%=ultimodiames(mes_anterior,ano_anterior)&" de "&mes_selecionado(mes_anterior)&" de "&ano_anterior%>
