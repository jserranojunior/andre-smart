...
<table align="center" border="1" class="no_print">
	<%if session("cd_codigo") = 46 then%>
		<tr><td colspan="100" align="center"><span style="font-size=8px;color:red;">funcionarios_documentacao.asp</span></td></tr>
	<%end if%>
	<tr><td colspan="100%" align="center" style="background-color:#000099; color:white; font-size=15; font-weight:bold;">Documentação</td></tr>
	<form action="../../empresa.asp" method="post" name="form">
	<input type="hidden" name="tipo" value="documentacao">
	...
	<tr class="no_print">
		...
		<td colspan="2">&nbsp;<a href="javascript:void(0);" onclick="javascript:window.open('empresa/funcionario/funcionarios_cadastro.asp?tipo=cadastro','Cadastro','width=780,height=700,scrollbars=1'); return false;"><img src="../../imagens/ic_editar.gif" alt="Editar" width="13" height="14" border="0">Novo colaborador</a></td>
		<td><a href="javascript:void();" onClick="limpa_ordem()" title="Limpa a ordem">limpa ordem</a></td>
	</tr>
	</form>
</table>
<table align="center" width="400">
	<%if session("cd_codigo") = 460 then%>
	<tr>
		<td colspan="100%">
	...
	<tr>
					...
					'	response.write(func_ativos)
							...
							while not rs_func.EOF
								...
								cd_estado_civil = rs_func("cd_estado_civil")
								...
								'*** Mostra o cabeçalho ****
									...
									if cabecalho = "0" then%>
										...
										<tr bgcolor=#c0c0c0>
											...
											<%if campo_endereco <> 0 Then%><td><b>End.</b></td><%end if%>
											...
											<%if campo_tit_eleitor <> 0 Then%><td><b>Titulo <br> Eleitor</b></td><%end if%>
											...
											<%if campo_unidade <> 0 Then%><td><b>Und</b></td><%end if%>
											<%if campo_situacao <> 0 Then%><td><b>Status</b></td><%end if%>
											<%if campo_natural <> 0 Then%><td><b>Natural</b></td><%end if%>
											<%if campo_conta_sal <> 0 Then%><td><b>C/C</b></td><%end if%>
											...
											<%if campo_nascimento <> 0 Then%><td><b>Nasc.</b></td><%end if%>		
											<!--td><b>Dependentes</b></td-->
											<%if campo_contatos <> 0 Then%><td><b>Contato</b></td><%end if%>
											<%if campo_transporte <> 0 Then%><td><b>VT</b></td><%end if%>
											<%if campo_alimentacao <> 0 Then%><td><b>Refeição</b></td><%end if%>
											<%if campo_alimentacao <> 0 Then%><td><b>Aliment.</b></td><%end if%>
											<%if campo_medica <> 0 Then%><td><b>Assist. Medica</b></td><%end if%>
											<!--<td><b>Assist. odontológica</b></td>-->
										</tr>
									<%cabecalho = 1
									conta_func = 0
									end if%>
								...
								%>
								<%if trim(nm_sexo) = "" OR trim(nm_endereco) = "" OR trim(nr_numero) = "" OR trim(nm_bairro) = ""  OR trim(nm_cidade) = ""  OR trim(nm_estado) = "" OR trim(nm_rg) = "" OR trim(nm_cpf) = "" OR trim(cd_ctps) = "" AND trim(cd_ctps_serie) = "" OR trim(cd_pis) = ""  OR trim(nm_tit_eleitor) = "" OR trim(nr_tit_eleitor_zona) = "" OR trim(nr_tit_eleitor_seccao) = "" OR trim(cd_numreg) = "" OR trim(nm_cargo) = "" OR trim(hr_entrada) = "" OR trim(hr_saida) = "" OR trim(nm_sigla) = "" OR trim(nm_status) = "" OR trim(nm_naturalidade) = "" OR trim(cd_banco_ag) = "" OR  trim(cd_banco_conta) = "" OR trim(nm_banco) = "" OR trim(nm_mae) = "" OR trim(nm_pai) = ""  OR trim(nm_estado_civil) = "" OR cd_estado_civil = 2 and trim(nm_conjuge) = ""  OR trim(dt_nascimento) = ""  Then%>
									<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=cor_linha%>');" style="height:20px; color:<%=cor_registro%>;" bgcolor="<%=cor_linha%>" onclick="javascript:window.open('empresa/funcionario/funcionarios_cadastro.asp?tipo=cadastro&pag=<%=strpag%>&cod=<%=rs_func("cd_funcionario")%>&busca=<%=strbusca%>','ficha_<%=rs_func("cd_funcionario")%>','width=700,height=700,scrollbars=1');">
										...
										<%if campo_sex <> 0 Then%><td valign="top" align="center"><%if trim(nm_sexo) = "" Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_endereco <> 0 Then%><td valign="top" align="center"><%if trim(nm_endereco) = "" OR trim(nr_numero) = "" OR trim(nm_bairro) = ""  OR trim(nm_cidade) = ""  OR trim(nm_estado) = "" Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_rg <> 0 Then%><td valign="top" align="center"><%if trim(nm_rg) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_cpf <> 0 Then%><td valign="top" align="center"><%if trim(nm_cpf) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_ctps <> 0 Then%><td valign="top" align="center"><%if trim(cd_ctps) = "" AND trim(cd_ctps_serie) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_pis <> 0 Then%><td valign="top" align="center"><%if trim(cd_pis) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_tit_eleitor <> 0 Then%><td valign="top" align="center"><%if trim(nm_tit_eleitor) = "" OR trim(nr_tit_eleitor_zona) = "" OR trim(nr_tit_eleitor_seccao) = "" Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_rp <> 0 Then%><td valign="top" align="center"><%if trim(cd_numreg) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_cargo <> 0 Then%><td valign="top" align="center"><%if trim(nm_cargo) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_hora <> 0 Then%><td valign="top" align="center"><%if trim(hr_entrada) = "" OR trim(hr_saida) = "" Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_unidade <> 0 Then%><td valign="top" align="center"><%if trim(nm_sigla) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_situacao <> 0 Then%><td valign="top" align="center"><%if trim(nm_status) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_natural <> 0 Then%><td valign="top" align="center"><%if trim(nm_naturalidade) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_conta_sal <> 0 Then%><td valign="top" align="center"><%if trim(cd_banco_ag) = "" OR  trim(cd_banco_conta) = "" OR trim(nm_banco) = "" Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_filia <> 0 Then%><td valign="top" align="center"><%if trim(nm_mae) = "" Then response.write("<b>X</b>")%></td>
																	<td valign="top" align="center"><%if trim(nm_pai) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_civil <> 0 Then%><td valign="top" align="center"><%if trim(nm_estado_civil) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_conjuge <> 0 Then%><td valign="top" align="center"><%if cd_estado_civil = 2 and trim(nm_conjuge) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_nascimento <> 0 Then%><td valign="top" align="center"><%if trim(dt_nascimento) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<!--<td valign="top">
											<%'*** DEPENDENTES ***
										...
										<%if campo_contatos <> 0 Then%><td valign="top" align="center">
											...
											Set rs_contato = dbconn.execute(xsql)
												...
												rs_contato.movenext
												wend%>
												<%if trim(cd_contato) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_transporte <> 0 Then%><td valign="top" align="center"><%if trim(cd_sptrans) = "" AND trim(cd_cmt_bom) = ""  Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_alimentacao <> 0 Then%><td valign="top" align="center"><%if trim(cd_vr) = "" Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_alimentacao <> 0 Then%><td valign="top" align="center"><%if trim(cd_vale_alimentacao) = "" Then response.write("<b>X</b>")%></td><%end if%>
										<%if campo_medica <> 0 Then%><td valign="top" align="center"><%if trim(cd_assistencia_medica_matricula) = "" Then response.write("<b>X</b>")%></td>
										<!--td valign="top"><%=cd_assistencia_odonto_matricula%></td--><%end if%>
									</tr>
								<%if cor > 0 then
									cor_linha = "#d7d7d7"
									cor = 0
