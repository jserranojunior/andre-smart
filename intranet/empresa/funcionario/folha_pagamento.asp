...
<table align="center" style="border_collapse:collapse;>
	<tr>
					<%x = 0
							...
							while not rs_func.EOF
								...
								'*** Mostra o cabeçalho ****
									if x = "0" then%>
										...
										<tr bgcolor="#c0c0c0" style="font-size:9;">
											<td align="center">&nbsp;</td>
											<td align="center"><b>Und.</b></td>
											<td align="center"><b>Mat.</b></td>
											<td><b>Funcionário</b></td>
											<td><b>Salário</b></td>
											<!--td><b>Aj. Custo</b></td-->
											<td align="center"><b>Insal.</b></td>
											<td><b>Ad. not</b></td>
											<td><b>Dias not</b></td>
											<td><b>Creche</b></td>
											<td><b>Qtd. HE</b></td>
								...
								<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=cor_linha%>');" bgcolor="<%=cor_linha%>" style="color:<%=cor_registro%>; font-size:9;font-family:verdana;">
									<td align="center" valign="top" style="color:gray;">
										...
										<%'=%></td>
									<td align="center" valign="top"><%=nm_sigla%></td>
									<td align="right" valign="top"><%=cd_matricula%>&nbsp;</td>
									<td align="left" valign="top" onclick="javascript:window.open('empresa/funcionario/funcionarios_folhapagamento_cad.asp?tipo=cadastro&pag=<%=strpag%>&cod=<%=rs_func("cd_funcionario")%>&busca=<%=strbusca%>');"><%=nm_funcionario%> <%'=cd_funcionario%></td>
									...
									<%'=ajuda_custo%></td-->
									<td align="right">
									<%if cd_hospital = 1 then
										'************************************************************************
										'***                         Insalubridade                            ***
										'************************************************************************
										xsql = "Select * From VIEW_funcionario_valores_padroes Where cd_tipo=7 AND dt_atualizador<='"&mes_sel&"/"&dia_sel&"/"&ano_sel&"' ORDER BY dt_atualizacao DESC"
											Set rs_valor = dbconn.execute(xsql)
											if not rs_valor.EOF then
									...
									t_func_receita = abs(nr_salario) + abs(insalubridade) + abs(ad_noturno) + abs(horas_extras)+ abs(horas_extras_dsr) + abs(ad_noturno_dsr)
									...
									<td align="right"><%if not total_func_liq = "" then%><b><%=formatnumber(abs(total_func_liq),2)%></b><%end if%></td>
									
									
									<!--td class="no_print"><img src="../../imagens/ic_editar.gif" alt="Editar" width="13" height="14" border="0" onclick="javascript:location.replace('empresa.asp?tipo=cadastro&pag=<%=strpag%>&cod=<%=rs_func("cd_funcionario")%>&busca=<%=strbusca%>');"></td-->
								</tr>
