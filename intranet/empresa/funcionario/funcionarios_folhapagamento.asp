...
nm_ordem = request("nm_ordem")
	if nm_ordem = "" Then nm_ordem = "nm_nome"
...
<%qtd_dias_dsr = qtd_domingos_comp + qtd_feriados - qtd_feriado_dsr%>
					<table border="1" cellspacing="0" cellpadding="2" align="center" width="100" class="no_print">
						...
						<form name="data" action="empresa.asp"  method="POST">
							...
							<tr>
								...
								<td><select name="nm_ordem" class="inputs">
										<option value="nm_unidade,nm_nome">Unidade</option>
										<option value="nm_nome" SELECTED>Alfabética</option>
									</select></td>
							</tr>
							<tr>
			...
			<!-- ---------------------------------------------------------------- -->				
					...
					'xsql = "up_funcionario_folhapagamento_geral @dt_pagamento_i='"&mes_pagamento&"/1/"&ano_pagamento&"', @dt_pagamento_f='"&mes_pagamento&"/"&final_pagamento&"/"&ano_pagamento&"', @dt_i = '"&dt_mes&"/1/"&dt_ano&"', @dt_f = '"&dt_mes&"/"&dia_final&"/"&dt_ano&"'"
					xsql = "up_funcionario_folhapagamento_geral @dt_comp_i='"&mes_competencia&"/1/"&ano_competencia&"', @dt_comp_f='"&mes_competencia&"/"&final_competencia&"/"&ano_competencia&"', @dt_i = '"&mes_competencia&"/1/"&ano_competencia&"', @dt_f = '"&mes_competencia&"/"&final_competencia&"/"&ano_competencia&"',@outras_cond='',@nm_ordem='"&nm_ordem&"'"
						Set rs = dbconn.execute(xsql)
...
'								qtd_hextra = rs("qtd_hextra")
									'if qtd_hextra > 36 then 'Caso as horas extras excederem 36h, suprime o excedente
									'	qtd_hextra = "36"
									'	he_marca = "1"
									'end if
...
'								nr_atrasos = rs("nr_atrasos")
								...
								'*** Salário desconto faltas ****
									if IsNumeric(nr_faltas_injustf) then
									'if abs(nr_faltas) >= "1" then
										total_faltas_mes = abs(nr_faltas_injustf) + abs(nr_dsr_faltas_injustf)
					...
					<!-- *** DSR Adicional Noturno *** -->
								<td align="right"><%if qtd_dia_noturno > 0 then
														...
														if carga_mensal <> "0" Then ad_noturno = (((nr_salario*0.40)/carga_mensal)*7)*qtd_dia_noturno
					...
					<!-- *** DSR hora Extra *** -->
								<td align="right">
									<%if qtd_hextra > 0 AND carga_mensal > 0 then
	...
	<!--*********************************************** D E S C O N T O S ********************************************************************************-->
					<!-- *** Faltas *** -->
								<td align="right" style="border-left:gray solid 2px;"><%=nr_faltas_injust%></td>
								<td align="right"><%=nr_dsr_faltas%></td>
								<td align="right"><%if nr_faltas_injust <> "" Then response.write(FormatNumber(nr_valor_falta,2))%></td>	
