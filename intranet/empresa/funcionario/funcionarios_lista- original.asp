								...
								'*** Mostra o cabeçalho ****
									...
									if cabecalho = "0" then%>
										...
										<tr bgcolor=#c0c0c0>
											...
											<%if campo_1_admissao <> 0 then%><td><b>1ª Admissão</b><br><img src="../../imagens/px.gif" alt="" width="60" height="1" border="0"></td><%end if%>
					...
					*************************************************************-->
								...
								IF mostrar_linha = 1 Then 'inibe a linha desejada%>
									...
									<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=cor_linha%>');" style="height:20px; color:<%=cor_registro%>;" bgcolor="<%=cor_linha%>">
										...
										<%if campo_nascimento <> 0 Then%><td valign="top"><%=zero(day(dt_nascimento))&"/"&zero(month(dt_nascimento))&"/"&year(dt_nascimento)%></td><td valign="top"> <%=idade_func%></td><%end if%>
