...
<%qtd_dias_dsr = qtd_domingos_comp + qtd_feriados%>
					...
					<table border="1" cellspacing="0" cellpadding="2" align="center" width="100" class="no_print">
						...
						<form name="data" action="<%if jan = 1 then%><%="funcionarios_folhapagamento_var"%><%else%><%=strcat%><%end if%>.asp"  method="POST">
							...
							<tr>
								<td colspan="4" align="center" class="cabecalho" style="background-color:black; color:white;"><%'=dt_mes&"/"&dia_final&"/"&dt_ano%> <b>VARIÁVEIS - <%=tit_doc%> <%if strregime_trabalho <> "" Then%><%response.write("- "&strregime_trabalho&"."&strmatricula)%><%end if%> <b>(Antigo)</b></b></td>
			...
			<!-- ---------------------------------------------------------------- -->				
							...
							<tr style="background-color:gray; color:white;">
								...
								<td align="center"><b>Funcion&aacute;rio</b><br><img src="../../imagens/px.gif" alt="" width="240" height="1" border="0"></td>
								...
								<td align="center"><b>Total faltas</b><br><img src="../../imagens/px.gif" alt="" width="40" height="1" border="0"></td>
							...
							while not rs.EOF
								...
								nr_dsr_faltas = rs("nr_dsr_faltas")
									if  not IsNumeric(nr_dsr_faltas) Then nr_dsr_faltas = "0"
										...
										total_faltas = abs(nr_faltas_justif) + abs(nr_faltas_injust)
					...
					'if int(filtros)="00" AND  int(total_faltas)>=0 AND int(cd_unidade) = int(strcd_unidade) OR int(filtros)="00" AND int(total_faltas)<=0 AND int(cd_unidade) = int(strcd_unidade) OR int(filtros)="00" AND  int(total_faltas)>=1 AND int(cd_unidade) = int(strcd_unidade)then
							...
							if nr_vr_extra <> "" Then
								total_vr = int(nr_vr) + int(nr_vr_extra)
							else
								total_vr = int(nr_vr)
							end if
						...
						%>
								...
								<td style="color:<%=cor_registro%>;" align="left" onClick="window.open('<%=variaveis1%>')"><%=nm_nome%> <%if cd_vt_canc > 0 then response.write("*")%> &nbsp;<%if expediente = 6 then%>**<%end if%></td>
								<td style="color:<%=cor_registro%>;" align="center" onClick="window.open('<%=variaveis1%>')"><%=nm_sigla%></td>
								
					<!-- *** Qtd Plantões *** -->
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if qtd_plantoes >= 1 then%><%=(qtd_plantoes)%><%else%>0<%end if%></td>
					<!-- *** Qtd. Adicional Noturno *** -->
								<td align="center" onClick="window.open('<%=variaveis2%>')">&nbsp;<%if qtd_dia_noturno > 0 then%><%=int(qtd_dia_noturno)%><%else%>0<%end if%><!-- <%'if qtd_dia_noturno > "" then%> dias<%'elseif qtd_dia_noturno <> "" then%>dia<%'end if%>--></td>
					<td bgcolor="silver"><img src="../../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
					<!-- *** Qtd Hora Extra *** -->
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if qtd_hextra > 0 then%><%=(qtd_hextra)%><%else%>0<%end if%></td>
					<!-- *** Qtd Hora Extra Excedentes*** -->
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if qtd_hextra_plus > 0 then%><span style="color:<%if int(qtd_hextra_plus) > 0 then%>red;<%else%>black;<%end if%>"><%=zero(qtd_hextra_plus)%><%else%>0</span><%end if%></td>
					<td bgcolor="silver"><img src="../../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
					<!-- *** Faltas justificadas *** -->
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if nr_faltas_justif > 0 then%><%=(nr_faltas_justif)%><%else%>0<%end if%></td>
					<!-- *** Faltas Não justificadas *** -->
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if nr_faltas_injust > 0 then%><%=(nr_faltas_injust)%><%else%>0<%end if%></td>
					<!-- *** Total Faltas  *** -->
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if total_faltas > 0 then%><%=(total_faltas)%><%else%>0<%end if%></td>
					<%'if strtipo <> "var_enf" then%>
					<!-- *** DSR Faltas Não justificadas *** -->
								<!--td align="right"><%'if nr_dsr_faltas > 0 then%><%'=(nr_dsr_faltas)%><%'end if%></td><%'end if%>
					<!-- *** Atrasos *** -->
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if nr_atrasos > 0 then%><%=zero(nr_atrasos)%><%else%>0<%end if%></td>
					<%'if strtipo <> "var_enf" then%><%'end if%>
					<td bgcolor="silver"><img src="../../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
					<!-- *** Benefícios *** -->
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if nr_vr >= 1 then%><%=(nr_vr)%><%else%>0<%end if%></td>
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if nr_vr_extra >= 1 then%><%=(nr_vr_extra)%><%else%>0<%end if%></td>
								<td align="center"><%if total_vr >= 1 then%><%=(total_vr)%><%else%>0<%end if%></td>
					<td bgcolor="silver"><img src="../../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if nr_vt >= 1 then%><%=(nr_vt)%><%else%>0<%end if%></td>
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if nr_vt_extra >= 1 then%><%=(nr_vt_extra)%><%else%>0<%end if%>
								<td align="center" onClick="window.open('<%=variaveis2%>')"><%if total_vt >= 1 then%><%=(total_vt)%><%else%>0<%end if%></td>
					...
					<!-- *** Total Liquido *** -->	
						...
						<tr>
							...
							<td bgcolor="silver"><img src="../../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
								...
								<td align="center">&nbsp;<b><%=tt_hextra_plus%></b></td>
							<td bgcolor="silver"><img src="../../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
								<td align="center">&nbsp;<b><%=tt_faltas_justif%></b></td>
								<td align="center">&nbsp;<b><%=tt_faltas_injust%></b></td>
								<td align="center">&nbsp;<b><%=tt_faltas_justif + tt_faltas_injust%></b></td>
								<td align="center">&nbsp;<b><%=tt_atrasos%></b></td>
							<td bgcolor="silver"><img src="../../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
									<td align="center">&nbsp;<b><%=tt_vr%></b></td><%'end if%>
									<td align="center">&nbsp;<b><%if tt_vr_extra > 0 then%><%=tt_vr_extra%><%else%>0<%end if%></b></td>
									<td align="center">&nbsp;<b><%=tt_vr+tt_vr_extra%></b></td>
								<td bgcolor="silver"><img src="../../imagens/px.gif" alt="" width="1" height="1" border="0"></td>	
									<td align="center"><b><%=tt_vt%></b></td><%'end if%>
									<td align="center">&nbsp;<b><%if tt_vt_extra > 0 then%><%=tt_vt_extra%><%else%>0<%end if%></b></td>
									<td align="center">&nbsp;<b><%=tt_vt+tt_vt_extra%></b></td>
							</tr>
							<tr><td colspan="21">&nbsp;</td></tr>
							<tr><td colspan="21">&nbsp;<span style="font-size:9px;">* optou por não receber vale transporte &nbsp; / &nbsp; ** expediente de 6h não recebe vale refeição</span></td></tr>
					</table>
