...
<table align="center" style="border-collapse:collapse;  id="escala"">
	...
	<tr>
		...
		<tr>
			...
			'xsql_cargo = "Select * From View_funcionario_horario Where cd_funcionario='"&cd_funcionario&"' AND dt_admissao <= '"&data&"' AND dt_demissao = NULL ORDER BY dt_atualizacao desc"
			xsql_cargo = "Select * From View_funcionario_horario Where cd_funcionario='"&cd_funcionario&"'  ORDER BY dt_atualizacao desc"
					Set rs_horario = dbconn.execute(xsql_cargo)
						if not rs_horario.EOF then
							...
							'dt_admissao = rs_horario("dt_admissao")
						end if%>
