...
<input type="hidden" name="acao" value="aparelhos">
<input type="hidden" name="cod" value="<%=cd_funcionario%>">
<tr><td bgcolor="#808080" style="color:white;" align="center" colspan="2"><b>Aparelhos</b></td></tr>
<tr bgcolor="#e4e4e4">
    <td>&nbsp; Nome: &nbsp; </td>
	<td>&nbsp;<%=nm_nome%></td>
</tr>
...
<tr bgcolor="#e4e4e4">
    <td>&nbsp; Aparelho Atual &nbsp; </td>
	<td>&nbsp;<%=nr_numero%></td>
</tr>
<% strsql = "TBL_aparelhos_comunicacao order by nr_numero, cd_modelo"
  Set rs_aparelho = dbconn.execute(strsql)
%>
<tr bgcolor="#e4e4e4">
    <td>&nbsp; Novo Número &nbsp; </td>
	<td><select name="cd_aparelhos" class="inputs"> 
	<option value="">Selecione</option>
	<%Do While Not rs_aparelho.EOF
		cd_aparelho = rs_aparelho("cd_codigo")
		nr_numero = rs_aparelho("nr_numero")
			if nr_numero <> "" Then
				nr_numero_1 = mid(nr_numero,1,1)&"-"
				nr_numero_2 = mid(nr_numero,2,4)&"-"
				nr_numero = nr_numero_1&nr_numero_2&mid(nr_numero,6,4)
		...
		%>
	<option value="<%=rs_aparelho("cd_codigo")%>"<%if strcd_aparelho = CInt(rs_aparelho("cd_codigo")) then%>selected<%End if%>><%=nr_numero%></option>
	  <%rs_aparelho.movenext
		loop

		rs_aparelho.close
		Set rs_aparelho = nothing
	  %></td>	  
</tr>
<tr bgcolor="#e4e4e4">
    <td>&nbsp; Data: &nbsp; </td>		
...
<table border="0" align="center">
	...
	<%strsql = "SELECT TOP 5 * FROM View_funcionario_aparelhos WHERE cd_funcionario='"&cd_funcionario&"' ORDER BY dt_atualizacao DESC"
  ...
  %>
	<tr bgcolor="<%=cor_linha%>">
		<td>&nbsp;<%=zero(nr)%></td>
		<td>&nbsp;<a href="aparelhos"></a><%=nr_numero%></td>
		<td>&nbsp;<%=day(dt_atualizacao)%>/<%=mesdoano(Month(dt_atualizacao))%>/<%=year(dt_atualizacao)%></td>	
		<td align="center">
			<%if cd_suspenso = 1 then%>
				<a href="../acoes/funcionarios_acao.asp?cod=<%=cd_codigo%>&cd_suspenso=0&tbl=aparelhos&acao=suspende&cd_funcionario=<%=cd_funcionario%>"><img src="../../imagens/ic_pendente.gif" alt="<%=cd_codigo%>" width="10" height="12" border="0"></a>	
			<%else%>
				<a href="../acoes/funcionarios_acao.asp?cod=<%=cd_codigo%>&cd_suspenso=1&tbl=aparelhos&acao=suspende&cd_funcionario=<%=cd_funcionario%>"><img src="../../imagens/check_ok.gif" alt="<%=cd_codigo%>" width="25" height="12" border="0"></a>
			<%end if%></td>
	</tr>
	<%
	nr = nr + 1
