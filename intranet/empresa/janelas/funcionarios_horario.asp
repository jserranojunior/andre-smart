...
pasta_loc = "empresa\janelas\"
arquivo_loc = "funcionarios_horario.asp"
...
hoje = mes&"/"&dia&"/"&ano
	...
	if not rs_EOF Then
		...
		xsql = "Select * From View_funcionario_horario Where cd_funcionario = "&cd_funcionario&" and dt_atualizacao <= '"&hoje&"' and cd_suspenso <> 1 order by dt_atualizacao DESC"
...
<tr bgcolor="#e4e4e4"">
    <td>Novo</td>
	<td><input type="text" name="hr_entrada" size="2" maxlength="2"  class="inputs" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);">:<input type="text" name="min_entrada" size="2" maxlength="2"  class="inputs" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);"></td>
	...
	<td><input type="text" name="hr_saida" size="2" maxlength="2"  class="inputs"onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);">:<input type="text" name="min_saida" size="2" maxlength="2"  class="inputs"onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);"></td>
</tr>
<tr bgcolor="#e4e4e4"">
    <td>Data:</td>		
...
<table border="0" align="center">
	<tr bgcolor="#808080" style="color:white;" align="center">
		<td colspan="6">Histórico (ordem decrescente) <input type="checkbox" name="mostrar_todos" value="1" <%if mostrar_todos = 1 then response.write("checked")%>></td>
	</tr>
	...
	<%'strsql = "SELECT TOP 5 * FROM View_funcionario_horario WHERE cd_funcionario='"&cd_funcionario&"' AND cd_contrato = 2 ORDER BY dt_atualizacao DESC"
  	'strsql = "SELECT TOP 5 * FROM View_funcionario_horario WHERE cd_funcionario='"&cd_funcionario&"' AND cd_contrato = 1 ORDER BY dt_atualizacao DESC"
  	strsql = "Select * From TBL_funcionario_horario Where cd_funcionario = "&cd_funcionario&" ORDER BY dt_atualizacao DESC"
	...
	<tr bgcolor="<%=cor_linha%>">
		...
		<td align="center">
			...
			<%end if%></td>
		<%if cd_user = 46 Then%><td><img src="../../imagens/ic_del.gif" alt="" width="13" height="14" border="0" onClick="javascript:JsHorarioDelete('<%=cd_codigo%>','<%=cd_funcionario%>')"><%'=cd_codigo%></td><%end if%>
	</tr>
	<%
	nr = nr + 1
 ...
 <SCRIPT LANGUAGE="javascript">
	...
	function JsHorarioDelete(cod, cod1)
		{
			if (confirm ("Tem certeza que deseja excluir esse horário?"))
		 		{
		 		document.location.href('../../empresa/acoes/funcionarios_acao.asp?cod_horario='+cod+'&acao=horario_delete&cd_funcionario='+cod1+'');
				}
		} 
