...
'***************************************************************************************************************************
	...
	'***	Unidades e racks.	***
	'******************************
	'if cd_situacao = "5" AND cd_cancela <> "" then '*** CANCELAMENTO ***
	'if str_situacao = "5" AND cd_cancela <> "" then '*** CANCELAMENTO ***
	'	strsql ="SELECT * FROM View_patrimonio_lista WHERE cd_codigo = "&cd_pat_codigo&"" ' *** Cancela o comodato e muda o 
	'	Set rs_cancela = dbconn.execute(strsql)
	'		cd_unidade_des = rs_cancela("cd_unidade")
	'		cd_rack_des = rs_cancela("cd_rack")
	...
	'elseif cd_situacao = "1" AND cd_unidade <> "" AND cd_unidade_des <> "" AND dia_empr_devolucao = "" then ' Descarta
	'else
	...
	'elseif str_situacao = "5" AND cd_unidade = cd_unidade_comodato AND cd_unidade_des = "" AND dia_empr_devolucao >= 0 then'efetiva e termina comodato (ver1)
	'elseif str_situacao = "5" AND cd_unidade = cd_unidade_comodato AND cd_unidade_des = "" AND dia_empr_devolucao >= 0 then'efetiva e termina comodato (ver1)
	...
	'elseif str_situacao = "5" AND cd_unidade <> cd_unidade_comodato AND cd_unidade_des = "" AND dia_empr_devolucao > 0 then'efetiva e termina comodato (ver2)
	...
	elseif str_situacao = "6" then ' Estoque
		...
		cd_unidade_comodato = cd_unidade_des
		cd_rack_comodato = cd_rack_des
		erro = "Estoque"
...
'******************************		
	ElseIf acao = "editar" AND cd_tipo = "patrimonio" Then
		...
		response.write("patrimonio edita OK")
			...
			if cd_unidade_origem <> "" AND cd_unidade_destino <> "" Then
...
'**********************************
	...
	'*** Emprestimo/comodato ***
	'***************************
	Elseif str_situacao = 4 OR str_situacao = 5 Then
			...
			'else
				dt_retorno = "NULL"
			'end if
		xsql = "up_patrimonio_empresta @cd_patrimonio='"&cd_pat_codigo&"', @cd_situacao='"&str_situacao&"', @nm_solicitante='"&nm_solicitante&"', @cd_unidade_ori='"&cd_unidade1&"', @cd_unidade_des='"&cd_unidade_des&"', @dt_emprestimo='"&dt_emprestimo&"', @dt_retorno="&dt_retorno&", @nm_empr_obs='"&nm_empr_obs&"'"
		dbconn.execute(xsql)
		'response.write("***emprestado!!!***")
	...
	elseif str_situacao = 6 Then
		...
		'*** MOVIMENTAÇÃO ***
			...
			'*** busca o ultimo número de movimentação do patrimônio ***
				...
				set rs = dbconn.execute(strsql)
					ultimo_mov = int(rs("ultimo"))+1
						xsql = "up_patrimonio_movimentacao_insert @cd_patrimonio='"&cd_pat_codigo&"', @cd_unidade_origem='"&cd_unidade1&"', @cd_rack_origem='"&cd_rack1&"', @cd_unidade_destino='"&cd_unidade_des&"', @cd_rack_destino='"&cd_rack_des&"', @cd_tipo_movimentacao="&str_situacao&", @dt_ida='"&dt_estoque&"', @dt_retorno='"&dt_estoque&"', @id_movimento="&ultimo_mov&", @cd_user="&cd_user&",@dt_cadastro='"&dt_hoje&"'"
						dbconn.execute(xsql)
