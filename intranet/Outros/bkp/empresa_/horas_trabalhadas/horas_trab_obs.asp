...
'Trocar estas sentenças para que as alterações funcionem em produção
'empresa_funcionarios_obs.asp = empresa_funcionarios_obs.asp
'horas_trab_cartao_acao_obs.asp = horas_trab_cartao_acao_obs.asp
		...
		<td align="left">
			<table cellspacing="1" cellpadding="0" bordercolordark="#FFFFFF" class="table">
				...
				<%'*********************************************************
				  ...
				  '************************
					...
					<form action="../../empresa_funcionarios_obs.asp" name="Ano" method="get">
					<tr>
					...
					<tr>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td colspan="5">[<a href="empresa_funcionarios_obs.asp">Mudar Ano</a>] &nbsp;&nbsp; <b><%=ano_sel%></b></td>
					</tr>
					<tr bgcolor=#cococo>
					...
					<form action="../../empresa_funcionarios_obs.asp" name="mes" method="get">
					...
					<tr>
						...
						<select name="mes_sel">
								...
								<option value="<%=mes_inicio%>"><%=mes_selecionado(mes_inicio)%></option>
			...
			<table cellspacing="1" cellpadding="1" border="0"  class="table">
					...
					<tr>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td colspan="5">[<a href="empresa_funcionarios_obs.asp">Mudar Ano</a>] &nbsp;&nbsp; [<a href="empresa_funcionarios_obs.asp?ano_sel=<%=ano_sel%>">Mudar Mês</a>] &nbsp;&nbsp;<b><%=mes_selecionado(mes_sel)%>/<%=ano_sel%></b></td>
					</tr>
					<tr bgcolor=#cococo>
						<td align=center colspan="100%"><img src="imagens/px.gif" height="0"></td>
					</tr>
					...
					<%conta_coop = "1"
					'condicao = " Where  cd_regime_trabalho = 1 AND (MONTH(dt_contratado) <= "&mes_sel&") AND (YEAR(dt_contratado) <= "&ano_sel&") AND dt_desliga >= "&data_atual&" OR cd_regime_trabalho = 1 AND (MONTH(dt_contratado) <= "&mes_sel&") AND (YEAR(dt_contratado) <= "&ano_sel&") AND dt_desliga IS null "
					...
					%>
					<tr onmouseover="mOvr(this,'#CFC8FF');"  onclick="javascript:location.replace('empresa_funcionarios_obs.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>&cd_codigo=<%=cd_codigo%>&tela=<%=tela%>');" onmouseout="mOut(this,'');">
						<td>&nbsp;</td>
						<td><font color="<%=cor_font%>"><%=cd_codigo%></font></td>
						<td><font color="<%=cor_font%>"><%=cd_matricula%></font></td>
						<td><font color="<%=cor_font%>"><%=nm_nome%>&nbsp;<%=nm_sobrenome%><%'=cd_status%> <%'=dt_desliga%> <%'=data_atual%></font></td>
					</tr>
					<%
				...
				<%'*********************************************************
				  ...
				  '*********************************************************
					...
					<tr id="no_print">
						<td></td>
						<td colspan="9">
						[<a href="empresa_funcionarios_obs.asp">Mudar Ano</a>] &nbsp;&nbsp; [<a href="empresa_funcionarios_obs.asp?ano_sel=<%=ano_sel%>">Mudar Mês</a>]  &nbsp;&nbsp;/  &nbsp;&nbsp;[<a href="empresa_funcionarios_obs.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>">Mudar Funcionário</a>]  &nbsp;&nbsp;/  &nbsp;&nbsp; [<a href="empresa_funcionarios_obs.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>&cd_codigo=<%=cd_codigo%>&tela=1">Tela inteira</a>:<a href="empresa_funcionarios_obs.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>&cd_codigo=<%=cd_codigo%>&tela=0">Tela normal</a>]</td>
					</tr>
					<tr bgcolor=#cococo>
						<td align=center colspan="100%"><img src="imagens/px.gif" height=0></td>
					</tr>
					...
					<tr>
						<td align=left colspan="4" bgcolor="#C0C0C0"><br>&nbsp;<%=nm_nome%>&nbsp;<%=nm_sobrenome%><b><br>&nbsp;</td>
...
%>
					<tr bgcolor="<%=str_cor_dia%>" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=strcor%>');" class="txt_">
						...
						<td align=left>&nbsp;<%=str_aviso%> <%=nm_obs%> <%'=total_mes(tempo_interv)%> <%'=total_mes(tempo_interv2)%> <%'=carga_diaria%> <%=msg_1%><!--(<%'=total_mes(strsun)%>) (<%'=cd_carga_horaria%>) (<%'=total_mes(saldo_dia)%>)--></td>
					<%If fechado = "1" Then%>
	...
	'if data_entrada_day = "" AND dt_data <> "" then
		'if data_entrada_day = "" then 'AND cd_folga = "1" then%>
					<tr bgcolor="<%=str_cor_dia%>" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=str_cor_dia%>');">
						...
						<td align=left>&nbsp;<%=txt_obs%>&nbsp;<%=msg_2%></td>
					    <%If fechado = "1" Then%>
					    <Td align=center id="no_print"><input type="button" value="   ...   "></td>
					<%Else%>
...
{
		document.location.href('empresa_funcionarios_obs.asp?dia_sel='+cod_a1+'&mes_sel='+cod_a2+'&ano_sel='+cod_a3+'&cd_codigo='+cod_a4+'&tela='+cod_a5+'&acao=inserir');
}
function JsEdit(cod_a, cod_b, cod_c, cod_d, cod_e)
{
		document.location.href('empresa_funcionarios_obs.asp?dia_sel='+cod_a+'&mes_sel='+cod_b+'&ano_sel='+cod_c+'&cd_codigo='+cod_d+'&tela='+cod_e+'&acao=editar');
}
...
{
		document.location.href('empresa_funcionarios_obs.asp?dia_sel='+cod_a+'&mes_sel='+cod_b+'&ano_sel='+cod_c+'&cd_codigo='+cod_d+'&tela='+cod_e+'&acao=copiar');
}
