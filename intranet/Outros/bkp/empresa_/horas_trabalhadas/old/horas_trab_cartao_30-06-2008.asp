...
<table width="500" cellspacing="0" cellpadding="0" border="1" class="textoPadrao" align="center">
	<tr>
		<td align="center">
			<table width="100%" cellspacing="1" cellpadding="0" border="1" class="textoPadrao" bordercolordark="#FFFFFF">
			...
			<table width="100%" cellspacing="1" cellpadding="1" border="0" class="textoPadrao">
				...
				<%'*********************************************************
				  ...
				  '*********************************************************
					...
					<tr>
						<td>&nbsp;</td>
						<td colspan="9">[<a href="empresa_funcionarios_horas.asp">Mudar Mês</a>]  &nbsp;&nbsp;/  &nbsp;&nbsp;[<a href="empresa_funcionarios_horas.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>">Mudar funcionário</a>]  &nbsp;&nbsp;/  &nbsp;&nbsp; [<a href="empresa_funcionarios_horas.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>&cd_codigo=<%=cd_codigo%>&tela=1">Tela inteira</a>:<a href="empresa_funcionarios_horas.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>&cd_codigo=<%=cd_codigo%>&tela=">Tela normal</a>]</td>
					</tr>
					<tr >
				...
				<table cellspacing="1" cellpadding="1" border="0" class="textoPadrao">
				<tr bgcolor="#cococo">
					...
					<td align=center>Saldo de horas</td>
								...
								<div class="divrolagem">
									<table width="100%" cellspacing="1" cellpadding="1" border="0" class="textoPadrao">
										<tr bgcolor=#FFFFFF>
											...
											<td><img src="imagens/blackdot.gif" width=81 height=0></td>
...
do while not dia_mes = ultimo_dia + 1
					...
					<tr bgcolor="<%=strcor%>"  onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=strcor%>');">
						...
						<td align=center width=172><font color=<%=cor_saldo_dia%>>&nbsp;<%=BH%><%'=total_mes(saldo_dia)%></font></td>
						<td align=left width=172>&nbsp;<font color=<%=str_cor_dia%>><%=nm_obs%> (<%=tempo_interv%>/<%=intervalo_livre%>)</font></td>
					<%If fechado = "1" Then%>
					...
					excedente = strsun - cd_carga_horaria%>
						...
						<img src="imagens/ic_editar.gif" onclick="javascript:JsEdit('<%=data_entrada_day%>','<%=data_entrada_month%>','<%=data_entrada_year%>','<%=cd_codigo%>')" type="button" value="E" title="Editar"> 
						<img src="imagens/ic_del.gif" onclick="javascript:JsDelete('<%=cd_codigo%>','<%=cd_cod_horas%>','<%=min%>','<%=min%>')" type="button" value="A" title="Apagar">
						</td>
							<%if tela = "1" Then%>
							<td><img src="../../imagens/px.gif" width=10 height=0></td>
							<%end if%>
					<%end if%>
					</tr>
					<%rs2.movenext
	...
	'if data_entrada_day = "" AND dt_data <> "" then
		'if data_entrada_day = "" then 'AND cd_folga = "1" then%>
					<tr bgcolor="#DFEADF" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'#DFEADF');">
						<td align=center width=172><%=mens%><%=dia_mes%></a></td>
						<td align=Left colspan=4>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; F&nbsp;&nbsp;&nbsp;&nbsp; O&nbsp;&nbsp;&nbsp;&nbsp; L&nbsp;&nbsp;&nbsp;&nbsp; G&nbsp;&nbsp;&nbsp;&nbsp; A</td>
						<td align=center width=172>&nbsp;</td>
						<!--td align=center width=172>&nbsp;</td>
						<td align=center width=5><img src="../imagens/px.gif" width="1" height="20"></td-->
						<td width=200 align=left>&nbsp;<%=txt_obs%></td>
					    <Td width=200 align=right>
						...
						<img src="imagens/ic_del.gif" onclick="javascript:JsDelfolga('<%=cd_codigo%>','<%=cd_cod_folga%>','<%=cd_folga%>')" type="button" value="A" title="Apagar">
						</td>
							<%if tela = "1" Then%>
							<td><img src="imagens/px.gif" width=10 height=0></td>
							<%end if%>
	...
	if data_entrada_day = "" AND cd_folga <> "1" Then'AND str_duplos_2 = 0 then%>
					<tr bgcolor="#F5F5F5" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'#F5F5F5');">
						...
						<img src="imagens/ic_novo.gif" onclick="javascript:JsInsert('<%=dia_mes%>','<%=mes_sel%>','<%=ano_sel%>','<%=cd_codigo%>')" type="button" value="I" title="Incluir"> 
						<!--input onclick="javascript:JsEdit('<%=dia_mes%>','<%=mes_sel%>','<%=ano_sel%>','<%=cd_codigo%>')" type="button" value="E" title="Editar"--> 
						</td>
							<%if tela = "1" Then%>
							<td><img src="imagens/px.gif" width=10 height=0></td>
...
Loop
				...
				<!--tr bgcolor="<%=strcor%>">
					...
					<td align="center"><%=total_mes(saldo_mes)%></td>
					<td><img src="imagens/blackdot.gif" width=1 height=0></td>
					<td><img src="imagens/blackdot.gif" width=1 height=0></td>
				</tr-->
					<tr bgcolor=#FFFFFF><!--Separador e limitador de espaços-->
			...
			'			Loop
				...
				<%end if%>
				<tr bgcolor=#FFFFFF><!--Separador e limitador de espaços-->
						<td colspan="100%"><img src="imagens/blackdot.gif" width=100% height=0><%'=excedente%> <%'=minutos_trab%></td>
				</tr>
				<tr bgcolor="<%=strcor%>">
					<td colspan="100%">&nbsp;</td>
				</tr>
				...
				<!--tr bgcolor="<%=strcor%>">
					...
					<td><img src="imagens/blackdot.gif" width=1 height=0></td>
					<td>&nbsp;Carga diária: </td>
					<td align="right"><%=total_mes(carga_diaria)%> h/dia</td>
					<input type="text" name="cd_carga_diaria" size="6" maxlength="6" value="<%=total_mes(cd_carga_diaria)%>"> <b>h/mês</b></td>
					<td colspan="3"><img src="imagens/blackdot.gif" width=1 height=0><!--input type="submit" value="<%'=acao%>"--></td>
				</tr--></form>
				...
				<tr bgcolor="<%=strcor%>">
					...
					<td><img src="imagens/px.gif" width=1 height=0></td><input type="hidden" name="mesentrada" value="<%=mes_sel%>">
					<td>&nbsp;Saldo do mês: </td><input type="hidden" name="cd_saldo_mes" value="<%=excedente%>">
					<td align="right"><b><%=total_mes(excedente)%> h/mês</b></td>
					<td colspan="3"><img src="imagens/blackdot.gif" width=1 height=0><!--input type="submit" name="banco_horas" value="Banco de horas"--></td>
				</tr></form>
				<tr>
					<td colspan = "100%"><br>
					<br>
					...
					Total de dias trabalhados: <%=conta_dias%>
				-->
					</td>
				</tr>
					<%End if%>
...
{
		document.location.href('empresa_funcionarios_horas.asp?dia_sel='+cod_a1+'&mes_sel='+cod_a2+'&ano_sel='+cod_a3+'&cd_codigo='+cod_a4+'&acao=inserir');
}
...
function JsDelete(cod1, cod2, cod3, cod4)
{
  if (confirm ("Tem certeza que deseja excluir?"))
	  {
		document.location.href('empresa/horas_trabalhadas/horas_trab_cartao_acao.asp?cd_codigo='+cod1+'&cd_cod_horas='+cod2+'&acao=excluir&mesentrada=<%=mes_entrada%>&anoentrada=<%=ano_entrada%>&menos_min='+cod3+'&cd_excedente=<%=excedente%>&cd_minutos_trab='+cod4+'');
	  }
}
