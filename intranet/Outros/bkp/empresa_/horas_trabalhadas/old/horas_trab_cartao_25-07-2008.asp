		...
		<td align="left">
			...
			<table cellspacing="1" cellpadding="1" border="0"  class="table">
					<tr>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td colspan="5">[<a href="empresa_funcionarios_horas.asp">Mudar Mês</a>] &nbsp;&nbsp; <b><%=mes_selecionado(mes_sel)%>/<%=ano_sel%></b></td>
					</tr>
					<tr bgcolor=#cococo>
				...
				<%'*********************************************************
				  ...
				  '*********************************************************
					...
					<tr id="no_print">
						<td>&nbsp;</td>
						<td colspan="9">[<a href="empresa_funcionarios_horas.asp">Mudar Mês</a>]  &nbsp;&nbsp;/  &nbsp;&nbsp;[<a href="empresa_funcionarios_horas.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>">Mudar Funcionário</a>]  &nbsp;&nbsp;/  &nbsp;&nbsp; [<a href="empresa_funcionarios_horas.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>&cd_codigo=<%=cd_codigo%>&tela=1">Tela inteira</a>:<a href="empresa_funcionarios_horas.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>&cd_codigo=<%=cd_codigo%>&tela=0">Tela normal</a>]</td>
					</tr>
					<tr bgcolor=#cococo>
						<td align=center colspan="100%"><img src="imagens/blackdot.gif" width="100%" height=0></td>
					</tr>
...
do while not dia_mes = ultimo_dia + 1
					...
					do while not rs2.eof
						...
						'Se saida ou entrada do intervalo for igual a nada, então calcular a diferença entre a hora de saída e a hora de entrada.
								...
								'******** Tempo de intervalo - Não desconta para quem faz 12x36  *******
								'If tempo_interv = 60 AND cd_carga_horaria = 156 AND cd_carga_diaria = 12 then
								'If tempo_interv = 60 AND cd_carga_diaria = 12 then
								If cd_carga_diaria = 12 then
								'intervalo_livre = "60"
								intervalo_livre = tempo_interv
								min = min + intervalo_livre
								'Elseif tempo_interv > 60 AND cd_carga_horaria = 156 and cd_carga_diaria = 12 then
								'intervalo_livre = "60"
								'min = min + intervalo_livre
								'response.write("é")
								End if
								
								'******** Tempo de intervalo - Não desconta para quem faz 6 horas  *******
								'If tempo_interv = 15 AND cd_carga_horaria = 144 AND cd_carga_diaria = 6 then
								'If tempo_interv = 15 AND cd_carga_diaria = 6 then
								If cd_carga_diaria = 6 then
								'intervalo_livre = "15"
								intervalo_livre = tempo_interv
								min = min + intervalo_livre
								'response.write("isso")
...
%>
					<tr bgcolor="<%=str_cor_dia%>" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=strcor%>');" class="txt_">
						...
						<td align=center><%if data_entr = data_entrada Then%><%response.write("&nbsp;-")%><%else%><%=data_sai_hour%>:<%=data_sai_minute%> às <%=data_entr_Hour%>:<%=data_entr_minute%><%end if%></td>
						...
						<td align=left>&nbsp;<%=str_aviso%> <%=nm_obs%> <%'=tempo_interv%> <%'=carga_diaria%> <%=msg_1%><!--(<%=total_mes(strsun)%>) (<%=cd_carga_horaria%>) (<%=total_mes(saldo_dia)%>)--></td>
					<%If fechado = "1" Then%>
...
Loop
		...
		<tr id="ok_print"><td colspan="100%"><img src="imagens/blakdot.gif" width="100%" height="1"></td></tr>
			...
			'			Loop
				...
				<tr><td style=" page-break-after: always">&nbsp;<br><br><br></td></tr>
					<%End if%>
...
function JsDelete(cod1, cod2, cod3, cod4, cod5)
{
  if (confirm ("Tem certeza que deseja excluir?"))
	  {
		document.location.href('empresa/horas_trabalhadas/horas_trab_cartao_acao.asp?cd_codigo='+cod1+'&cd_cod_horas='+cod2+'&acao=excluir&mesentrada=<%=mes_entrada%>&anoentrada=<%=ano_entrada%>&menos_min='+cod3+'&cd_excedente=<%=excedente%>&cd_minutos_trab='+cod4+'&tela='+cod5+'');
	  }
}
