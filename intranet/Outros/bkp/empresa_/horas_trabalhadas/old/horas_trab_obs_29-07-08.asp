		...
		<td align="left">
				...
				<%'*********************************************************
				  ...
				  '*********************************************************
					...
					<tr align="center">
						...
						%>
						<a href=empresa_funcionarios_obs.asp?mes_sel=<%=mes_ant-3%>&ano_sel=<%=ano_ant%>><%=mes_selecionado(mes_ant-3)%>/<%=ano_ant%></a></td>
					</tr>
					<tr align="center">
					    <td colspan="100%"><%
						if mes_prod = "1" Then
						...
						%>
						<a href=empresa_funcionarios_obs.asp?mes_sel=<%=mes_ant-2%>&ano_sel=<%=ano_ant%>><%=mes_selecionado(mes_ant-2)%>/<%=ano_ant%></a></td>
					</tr>
					<tr align="center">
					    <td colspan="100%"><%
						if mes_prod = "1" Then
			...
			<table cellspacing="1" cellpadding="1" border="0"  class="table">
					<tr>
						<td>&nbsp;</td>
						<td>&nbsp;</td>
						<td colspan="5">[<a href="empresa_funcionarios_obs.asp">Mudar Mês</a>] &nbsp;&nbsp; <b><%=mes_selecionado(mes_sel)%>/<%=ano_sel%></b></td>
					</tr>
					<tr bgcolor=#cococo>
					...
					<tr onmouseover="mOvr(this,'#CFC8FF');"  onclick="javascript:location.replace('empresa_funcionarios_obs.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>&cd_codigo=<%=cd_codigo%>&tela=<%=tela%>');" onmouseout="mOut(this,'');">
						...
						<td><font color="<%=cor_font%>"><%=nm_nome%>&nbsp;<%=nm_sobrenome%><%'=cd_status%> <%'=dt_demissao%> <%'=data_atual%></font></td>
					</tr>
					<%
				...
				<%'*********************************************************
				  ...
				  '*********************************************************
					...
					<tr id="no_print">
						<td>&nbsp;</td>
						<td colspan="9">[<a href="empresa_funcionarios_obs.asp">Mudar Mês</a>]  &nbsp;&nbsp;/  &nbsp;&nbsp;[<a href="empresa_funcionarios_obs.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>">Mudar Funcionário</a>]  &nbsp;&nbsp;/  &nbsp;&nbsp; [<a href="empresa_funcionarios_obs.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>&cd_codigo=<%=cd_codigo%>&tela=1">Tela inteira</a>:<a href="empresa_funcionarios_obs.asp?mes_sel=<%=mes_sel%>&ano_sel=<%=ano_sel%>&cd_codigo=<%=cd_codigo%>&tela=0">Tela normal</a>]</td>
					</tr>
					<tr bgcolor=#cococo>
						<td align=center colspan="100%"><img src="imagens/blackdot.gif" width="100%" height=0></td>
					</tr>
...
%>
					<tr bgcolor="<%=str_cor_dia%>" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=strcor%>');" class="txt_">
						...
						<td align=left>&nbsp;<%=str_aviso%> <%=nm_obs%> <%'=total_mes(tempo_interv1)%> <%'=total_mes(tempo_interv2)%> <%'=carga_diaria%> <%=msg_1%><!--(<%=total_mes(strsun)%>) (<%=cd_carga_horaria%>) (<%=total_mes(saldo_dia)%>)--></td>
					<%If fechado = "1" Then%>
...
'***********************************************
		...
		Do while not rs3.EOF
			...
			msg_abono = "&nbsp;&nbsp;&nbsp;&nbsp; A&nbsp;&nbsp;&nbsp; B&nbsp;&nbsp;&nbsp; O&nbsp;&nbsp;&nbsp; N&nbsp;&nbsp;&nbsp; O&nbsp;&nbsp;&nbsp;&nbsp; "
			msg_3 = "+"&total_mes(dt_abono_tempo)&"hs"
		rs3.movenext
		loop
