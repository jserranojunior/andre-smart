					...
					<table border="0" cellspacing="0" cellpadding="2" align="center"><!-- OK 2-->
						...
						<%if strfoto = "" then%><%strfoto = "Vdlap.gif"%><%end if%>
							...
							<tr id="no_print">
								...
								<td rowspan="7" align="left" valign="top"><img src="foto/funcionarios/<%=strfoto%>" alt="" name="<%=strfoto%>" id="<%=strfoto%>" width="73" border="0"><br>								
								<font color="#7d7d7d"><a href="#" onClick="window.open('upload/formulario.asp?cd_codigo=<%=strcod%>','upload','width=400,height=150')" alt="Inserir/mudar foto">(Alterar foto)</a></font></td>
								<%'if 
								'end if%>
							...
							<tr>
								...
								<td class="txt_cinza" colspan="2" <%=tipo%>><a href="javascript:;" onclick="window.open('empresa/pop_up/janela_cargos.asp?cd_funcionario=<%=strcod%>&cd_funcao=<%=cd_funcao%>', 'principal', 'width=350, height=200'); return false;"><%=nm_funcao%>.</a>
								<%'rs_func.movenext
								'loop%>
							...
							<tr>
								...
								<%strsql ="SELECT * From View_funcionario_status where cd_funcionario = "&strcod&" order by cd_codigo"
									...
									rs_status.movenext
									loop%>
									<a href="javascript:;" onclick="window.open('empresa/pop_up/janela_status.asp?cd_funcionario=<%=strcod%>&cd_status=<%=cd_status%>', 'principal', 'width=350, height=200'); return false;"><%=nm_status%>.</a>
								<%end if%>
									</td>
									<td colspan="2">
									<%strsql ="TBL_unidades"
								...
								<%else%>
									...
									rs_unidade.movenext
									loop%>
									 <a href="javascript:;" onclick="window.open('empresa/pop_up/janela_unidade.asp?cd_funcionario=<%=strcod%>&cd_unidade=<%=cd_unidade%>', 'principal', 'width=350, height=200'); return false;"><%=nm_unidade%>. (<%=strunidades%>)</a>
									<%rs_uni.close
									Set rs_uni = nothing%>
								<%end if%></td>
								<td>&nbsp;</td>
							...
							<tr id="no_print">
								<td colspan="5" valign="middle"><A href="empresa_funcionarios_lista.asp"><img src="imagens/bt_lista.gif" alt="Listar" border="0" width="119" height="22" border="1"></a>
							...
							<tr id="no_print"><td><input onclick="javascript:JsDelete('<%=strcod%>')" type="button" value="Apagar" title="Apagar">
								&nbsp;<a href="javascript:;" onclick="window.open('empresa/pop_up/janela_dispensa.asp?cd_funcionario=<%=strcod%>','principal','width=350, height=200'); return false;">Dispensar?</a></td>
							</tr>
							</form>
							<%If strcod <> "" then%>
