...
'Strdt_demissao = FormatDateTime(now,2)
	...
	'Strdt_demissao = dt_ano & zero(dt_mes)
...
'		xsql_demissao = "UPDATE TBL_funcionario SET dt_desliga="&dt_desliga&" where cd_codigo="&strcod&""
	...
	strmsg = "Colaborador dispensado! "&dt_desliga&""
			...
			dbconn.execute(xsql)
		'Elseif strstatus <> 4 Then
		''xsql_demissao = "update TBL_funcionario SET dt_demissao=NULL, dt_desliga=NULL where cd_codigo="&strcod&""
		'xsql_demissao = "update TBL_funcionario SET dt_desliga=NULL where cd_codigo="&strcod&""
		'dbconn.execute(xsql_demissao)
	end if
...
response.redirect "../../empresa_funcionarios_lista.asp?msg="&strmsg&""
%>
