...
<table border="1" align="center" style="font-size:12px; font-family:arial;border-collapse:collapse;" bgcolor="#ffffff">
	...
	'else ' *** Aceita dados de outras area do sistema ****
	...
	<input type="hiddens" name="cd_origem" value="<%=str_origem%>">
	...
	<tr>
		...
		<%else%>
			...
			<%if cd_origem <> 6 Then%>
				...
				<input type="hiddens" name="cd_tipo_valor_atual" value="<%=cd_tipo_valor%>" size="3""><%'=cd_tipo_valor%>
			<%elseif cd_origem = 6 then	%>	
			...
			<td>
				...
				<span style="display:block;" class="mostra_categoria">
					...
					<%elseif cd_origem = 6 then%>
						<%if cd_categoria_valor = "" OR cd_categoria_valor = 1 then 
	...
	<tr>
		...
		<input type="<%if acao = "editar_valor" OR str_origem <> "" Then%>text<%else%>hidden<%end if%>" name="mes_vencimento" value="<%=zero(dt_mes)%>" size="2" maxlength="2" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);">
	...
	<tr bgcolor="#ebebeb">
		<td>Final</td>
		<td colspan="2"><input type="hidden" name="mes_final" value="<%=mes_final%>" id="mes_final" size="2">
						<input type="hidden" name="ano_final" value="<%=ano_final%>" size="2">
