...
filtro_ordens = request("filtro_ordens")
	...
	elseif year(now)&zero(month(now)) < dt_ano&zero(dt_mes) then
		...
		if filtro_ordens = "1" Then
			ordem = "nm_fornecedor, nm_descricao"
			subtotal = 0
...
<table class="no_print">
	<tr>
		...
		<td>
			...
			<a href="javascript:void(0);" return false;" onClick="window.open('../manutencao_2/rel_orc_aprov.asp?tipo=financ&ordem_sel=2&ano_sel=<%=year(now)%>&mes_sel=<%=month(now)%>','','location=0,status=0,width=800,height=700,scrollbars=1,resizable=1')">Relatório Manut. Reposi. 
		</td>
		<td><img src="../imagens/blackdot.gif" alt="" width="100" height="1" border="0"></td>
		<td>
			<table border="0" cellspacing="1" cellpadding="1" class="no_print" style="background-color:#b8d8da; border:black solid 2px;" align="center">
				...
				<tr>
					<td align="center" colspan="3">CONTAS A PAGAR - ver3</td>
				</tr>
				<tr>
					<td><b>M&ecirc;s</b></td>
					<td><b>Ano</b></td>
				...
				<tr>
					...
					<td colspan="2"><input type="text" name="ano_sel" maxlength="4" size="4" value="<%=dt_ano%>">&nbsp;<input type="submit" name="OK" value="OK" width="4" height="5"></td>
				</tr>
				</form>
				<tr>
					<td align="center" colspan="3"><a href="financeiro.asp?tipo=diario3&mes_sel=<%=mes_anterior%>&ano_sel=<%=ano_anterior%>&str_area=<%=str_area%>&filtro_ordens=<%=filtro_ordens%>"><< Anterior </a>&nbsp; &nbsp; &nbsp; <a href="financeiro.asp?tipo=diario3&mes_sel=<%=month(now)%>&ano_sel=<%=year(now)%>&str_area=<%=str_area%>&filtro_ordens=<%=filtro_ordens%>">Atual</a> &nbsp; &nbsp; &nbsp; <a href="financeiro.asp?tipo=diario3&mes_sel=<%=mes_posterior%>&ano_sel=<%=ano_posterior%>&str_area=<%=str_area%>&filtro_ordens=<%=filtro_ordens%>">Próximo>></a></td>
				<tr>
					<td colspan="3">&nbsp;</td>
				</tr>
				<tr>
					<td><%'if dt_ano&zero(dt_mes) >= year(now)&zero(month(now)) then%>
						<a href="javascript:void(0);" onClick="window.open('financeiro/diario_cad3.asp?mes_sel=<%=dt_mes%>&ano_sel=<%=dt_ano%>&acao=inserir','Cadastro','width=500,height=480,scrollbars=1,status=1')" return false;"><b>(+)</b> Incluir</a><%'end if%> &nbsp;&nbsp;&nbsp;&nbsp;
					</td>
					<td align="right" colspan="2">
						<%strsql = ""
						strsql ="sp_orcamento_s01 @funcao=SC"
						Set rs = dbconn.execute(strsql)
						
						if NOT rs.EOF then
							N_orcamentos_novos = rs("N_orcamentos_novos")
						end if
						if N_orcamentos_novos > 0 then%>
							<a href="javascript:void(0);" return false;" onClick="window.open('../manutencao_2/rel_orc_conta_lista.asp?tipo=conta_orcamentos','orc_novos','location=0,status=0,width=800,height=700,scrollbars=1,resizable=1')">
								<img src="../imagens/band_alerta.gif" alt="" width="15" height="12" border="0"> <%=N_orcamentos_novos%>
							</a>
						<%end if%>
					</td>
				</tr>	
		...
		<td>
			<table cellspacing="1" cellpadding="1" border="0" class="no_print" style="background-color:#b8d8da; border:black solid 2px;" align="center">
				<tr>
					<td align="center" colspan="5" style="color:white;"><b>LEGENDA</b><br>
					<img src="../imagens/blackdot.gif" alt="" width="200" height="1" border="0"></td>
				</tr>				
				<tr>
					<%cor_pos = "red"
					'cor_50neg = "#0057ae"%>
				...
				<tr>
					...
					<td>selecionado</td>
					<td>&nbsp;</td>
					<td><img src="../imagens/band_alerta.gif" alt="" width="15" height="12" border="0"></td>
					<td>Orçamento</td>
					<!--td bgcolor="<%=cor_neg%>"><img src="../imagens/px.gif" alt="" width="12" height="12" border="0"></td-->
					<!--td>< -7%</td-->
				</tr>
...
<table align="left" style="border-collapse:collapse; border:black solid 1px; height:20px; overflow:scroll;" cellspacing="1" >
		...
		<td colspan="2">
			<%xsql = "SELECT * FROM TBL_financeiro_area"
				...
				<td>ORDEM: <select name="filtro_ordens" onChange="filtro_ordem()"; return false;>
							<option value="0" <%if filtro_ordens = 0 then response.write("SELECTED")%>>Cronológica</option>
	...
	<tr style="background-color:gray; font-size:11px; color:white;">
		<td align="center">&nbsp;<br><img src="../imagens/px.gif" alt="" width="15" height="1" border="0"></td>
		<td align="center" valign="bottom"><b>Área</b><br><img src="../imagens/px.gif" alt="" width="40" height="1" border="0"></td>
		<td align="center" valign="bottom"><b>C.C</b><br><img src="../imagens/px.gif" alt="" width="35" height="1" border="0"></td>
		<td align="center" valign="bottom"><b>Conta</b><br><img src="../imagens/px.gif" alt="" width="45" height="1" border="0"></td>
		<td align="center" valign="bottom"><b>U.N.</b><br><img src="../imagens/px.gif" alt="" width="40" height="1" border="0"></td>
		<td align="center" valign="bottom"><b>Tipo</b><br><img src="../imagens/px.gif" alt="" width="30" height="1" border="0"></td>
		<%if mostra_data = 1 then%><td align="center" valign="bottom"><b>Data</b><br><img src="../imagens/px.gif" alt="" width="10" height="1" border="0"></td><%end if%>
		<td align="center" valign="bottom"><b>Pg</b><br><img src="../imagens/px.gif" alt="" width="20" height="1" border="0"></td>
		<td colspan="2" valign="bottom"><b>Favorecido - Histórico</b><br><img src="../imagens/px.gif" alt="" width="340" height="1" border="0"></td>
		<td align="center" valign="bottom"><b>Anterior</b><br><img src="../imagens/px.gif" alt="" width="60" height="1" border="0"></td>
		<td align="center" valign="bottom"><b>Venc</b><br><img src="../imagens/px.gif" alt="" width="35" height="1" border="0"></td>	
		<td align="center" valign="bottom"><b>Atual</b><br><img src="../imagens/px.gif" alt="" width="60" height="1" border="0"></td>
		<td align="center" valign="bottom"><b>Extra</b><br><img src="../imagens/px.gif" alt="" width="60" height="1" border="0"></td>
		<td align="center" valign="bottom"><b>Pend.</b><br><img src="../imagens/px.gif" alt="" width="60" height="1" border="0"></td>
		<td><b>&nbsp;</b><br><img src="../imagens/px.gif" alt="" width="10" height="3" border="0"></td>
	</tr>
	<!--tr>
		<td></td>
		<td></td>
	...
	xsql = "up_financeiro_diario3_lista_geral @dt_i='"&dt_mes&"/1/"&dt_ano&"', @dt_f='"&dt_mes&"/"&dia_final&"/"&dt_ano&"', @dt_i_a='"&mes_anterior&"/1/"&ano_anterior&"', @dt_f_a='"&mes_anterior&"/"&dia_final_anterior&"/"&ano_anterior&"', @ordem='"&ordem&"'"
		...
		while not rs.EOF
			...
			cd_categoria_valor = rs("cd_categoria_valor")
				...
				dt_pagto = rs("dt_pagto")
						...
						Set rs_pend = dbconn.execute(strsql_verpend)
							if not rs_pend.EOF then
								...
								soma_pendencias = soma_pendencias + int(mostra_pendencia)
								dt_inicio_pend = rs_pend("dt_inicio_pend")
								dt_fim_pend = rs_pend("dt_fim_pend")
							else
	...
	<%if abs(str_area) = "0" OR abs(str_area) = abs(cd_area)  Then%>
		...
		'*** calcula o subtotal com valores anteriores
			...
			if NOT ISNULL(nm_valor_atual) then
				soma_valores = abs(soma_valores) + nm_valor_atual
			end if
			soma_extra = abs(soma_extra) + nm_valor_extra
			'soma_extra = nm_valor_extra
			'soma_pendencias = nm_valor_pendente%>
	...
	<tr bgcolor="<%=cor_linha%>"  onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=cor_linha%>');">
		...
		<%if mostra_data = 1 then%><td align="left"><%=zero(day(dt_inicial))&"/"&zero(month(dt_inicial))&"/"&right(year(dt_inicial),2)%>-<%=dt_final%></td><%end if%>
		...
		<%elseif not IsNumeric(cd_valor) Then%>
			...
			<!--img src="../../imagens/check_proibido.gif" alt="Inclua o valor" width="12" height="12" border="0" return false;-->
		...
		<%else%>	
			<%if cd_tipo_valor = 3 AND nm_valor = 0 then%>
				<%'=cd_tipo_valor&" : "&nm_valor%>
				<img src="../../imagens/check_proibido.gif" alt="Inclua o valor" width="12" height="12" border="0" onClick="manipulacao_valor('<%=cd_diario%>','<%=dt_vencimento_anterior%>','<%=dt_mes%>','<%=dt_ano%>','<%=str_acao%>','<%=cd_user%>')"; return false;>
			<%else%>
				...
				<img src="../../imagens/check_ativo_2.gif" alt="Exclui" width="15" height="12" border="0" onClick="subtrai_pgto('<%=cd_valor%>',document.diario.total_pgto.value,'<%=check2%>','<%=check1%>')" id="<%=check2%>" name="<%=check2%>" <%'if ordem_var <> "" Then%>style="display:none;"<%'end if%>>
			<%end if%>
			</td>
		<%end if%>
		<td><span onClick="window.open('financeiro/diario_lista_pag3.asp?cd_diario=<%=cd_diario%>&mes_sel=<%=dt_mes%>&ano_sel=<%=dt_ano%>','Visualizar','width=500,height=<%if cd_tipo_valor=1 then%>240<%elseif cd_tipo_valor=2 then%>480<%elseif cd_tipo_valor=3 then%>460<%end if%>,scrollbars=1')" return false;" style="border-style:none;">
					...
					<%if nm_fornecedor <> "" and nm_pagamento <> ""  Then response.write(" - ")%>
						<%if trim(nm_cc) = "FOR" and trim(nm_tipo_abrv) = "FIX" then%><%else%><%=nm_descricao%><%end if%><%'=nm_pagamento%>&nbsp;</span>
						
						<%if trim(nm_cc) = "FOR" and trim(nm_tipo_abrv) = "FIX" then%>
						
						<%else%>

						<%if cd_qtd_parcelas > 1 then
								if cd_parcela = 0 then
									'response.write("<span style='color:red;'>Postergado</span>")
								else
									response.write("<span style='color:red;'><b>"&cd_parcela&"/"&cd_qtd_parcelas&"</b></span>")
								end if
							end if%>
							<%if cd_categoria_valor = 2 then response.write("¹")%>
							
							<%if nm_obs <> "" then%><img src="../imagens/ic_obs3.gif" alt="" width="10" height="10" border="0" onClick="window.open('financeiro/diario_lista_obs3.asp?cd_diario=<%=cd_diario%>&mes_sel=<%=dt_mes%>&ano_sel=<%=dt_ano%>','Visualizar','width=530,height=270,scrollbars=1')" return false;"><%end if%><%if cd_user = "46" then%><i>&nbsp;<%="("&cd_diario&"/"&cd_valor&")"%></i><%end if%>
							<%end if%>
							</td>
...
<!-- *************** ANTERIOR ***************-->		
		<td align="right" style="color:gray; background-color:<%=cor_linha2%>;">
							<%if nm_valor_anterior <> "0" then 
...
<!-- *************** VALOR FIXO ***************-->							
		<td align="right" <%if cd_tipo_valor <> 1 Then%> onClick="manipulacao_valor('<%=cd_diario%>','<%=dt_vencimento_anterior%>','<%=dt_mes%>','<%=dt_ano%>','<%=str_acao%>','<%=cd_user%>')"; return false;<%end if%>  style="border-left:thin solid silver;">
		<!--td align="right" <%if cd_tipo_valor = 4 Then%>onClick="manipulacao_pendencia('<%=cd_diario%>','<%=dt_vencimento_anterior%>','<%=dt_mes%>','<%=dt_ano%>','<%=str_acao%>','<%=cd_user%>')"; return false;<%'elseif cd_tipo_valor <> 1 Then%> onClick="manipulacao_valor('<%=cd_diario%>','<%=dt_vencimento_anterior%>','<%=dt_mes%>','<%=dt_ano%>','<%=str_acao%>','<%=cd_user%>')"; return false;<%end if%>-->
								...
								elseif ISNULL(nm_valor_atual) then 
									'response.write("x")
									
								elseif NOT ISNULL(nm_valor_atual) then 
									if nm_valor_atual > 0 then
										response.write(FormatNumber(nm_valor_atual,2))
									else
										if cd_tipo_valor <> 1 Then
											'response.write("x") '0 - Zero
												if cd_categoria_valor = 2 then '(2 = parcelado de valor variavel)
													total_aberto = total_aberto + nm_valor_anterior
													'response.write("._"&nm_valor_anterior&"-.")
													'nm_valor_anterior = "x"
												end if
										end if
									end if
...
<!-- *************** VALOR PENDENTE*************-->							
		<td align="right" bgcolor="silver" <%if cd_tipo_valor = 4 Then%>onClick="manipulacao_pendencia('<%=cd_diario%>','<%=dt_vencimento_anterior%>','<%=dt_mes%>','<%=dt_ano%>','<%=str_acao%>','<%=cd_user%>')"; return false;<%elseif cd_tipo_valor <> 1 Then%> onClick="manipulacao_valor('<%=cd_diario%>','<%=dt_vencimento_anterior%>','<%=dt_mes%>','<%=dt_ano%>','<%=str_acao%>','<%=cd_user%>')"; return false;<%end if%>>
								<%if cd_suspenso = 1 and cd_tipo_valor <> 1 then
									response.write("---")
								'elseif nm_valor_atual <> "0" then 
								'	response.write(FormatNumber(nm_valor_atual,2))
...
<!-- *******************************************-->
			...
			<%if cd_modo_pagamento > 0  then
				...
				'total_apagar = total_pago - nm_valor_atual + nm_valor_extra
			...
			elseif isnull(cd_modo_pagamento) OR cd_modo_pagamento < 1 then
				...
				total_a_pagar = total_a_pagar + nm_valor_atual + nm_valor_extra
					...
					if ISNULL(nm_valor) AND nm_valor_extra = 0 AND isnull(cd_suspenso) then
						total_aberto = total_aberto + nm_valor_anterior
					else
	...
	<tr>
		...
		<td align="right" bgcolor="#c0c0c0" style="color:red;"><b><%if soma_pendencias <> "" Then response.write(formatNumber(soma_pendencias,2))%></b></td>
		<td>&nbsp;</td>
		<!--td>&nbsp;<%%>Pagamento</td-->
	</tr>
	...
	<tr>
		...
		<td align="right" colspan="3" bgcolor="Silver" style="font-size:15px;">&nbsp;<b>TOTAL DO M&Ecirc;S</b></td>
	...
	<tr>
		<td colspan="7" bgcolor="#ffffff">&nbsp;</td>
		<td align="right" colspan="3">Necessidade de fluxo de caixa</td>
	...
	<tr>
		<td colspan="7" bgcolor="#ffffff">&nbsp;</td>
		<td align="right" colspan="3" bgcolor="Silver" style="font-size:15px;">&nbsp;<b>TOTAL ESTIMADO</b></td>
	...
	<tr>
		<td colspan="16">
			<%if cd_categoria_valor = 2 then response.write("¹ parcelamento com valor variável.")%>
		</td>
	</tr>
	</form>
