	...
	function manipulacao_valor(cd_diario,dt_vencimento_anterior,dt_mes,dt_ano,acao,user){
		...
		ano_hoje = hoje.getFullYear();
			...
			/*if (data_informada<data_hoje){
				...
				window.open('financeiro/diario_cad3.asp?cd_diario='+cd_diario+'&dia_sel='+dt_vencimento_anterior+'&mes_sel='+dt_mes+'&ano_sel='+dt_ano+'&acao='+acao+'','Cadastro','width=500,height=300,scrollbars=1');
				/*}*/
	}
	...
	<tr class="no_print">
		...
		<td>
			<table border="0" cellspacing="1" cellpadding="1" class="no_print" style="background-color:#b8d8da; border:black solid 2px;" align="center">
				...
				<tr>
					<td align="center" colspan="2"><a href="financeiro.asp?tipo=diario3&mes_sel=<%=mes_anterior%>&ano_sel=<%=ano_anterior%>&str_area=<%=str_area%>"><< Anterior </a>&nbsp; &nbsp; &nbsp; <a href="financeiro.asp?tipo=diario3&mes_sel=<%=month(now)%>&ano_sel=<%=year(now)%>&str_area=<%=str_area%>">Atual</a> &nbsp; &nbsp; &nbsp; <a href="financeiro.asp?tipo=diario3&mes_sel=<%=mes_posterior%>&ano_sel=<%=ano_posterior%>&str_area=<%=str_area%>">Próximo>></a></td>
				<tr>
					<td colspan="2">&nbsp;</td>
				</tr>
				<tr>
		...
		<td>
			<%xsql = "SELECT * FROM TBL_financeiro_area"
				...
				<td>ORDEM: <select name="filtro_ordens" onChange="filtro_ordem()"; return false;>
							...
							<option value="1" <%if filtro_ordens = 1 then response.write("SELECTED")%>>Ordem Alfabética</option>
							<!--option value="2" <%if filtro_ordens = 2 then response.write("SELECTED")%>>Ordem decrescente</option-->
						</select></td>
	</tr>
...
<!-- *************** XXXXXXXXXX ***************-->							
		<!--td align="right"-->
			...
			elseif isnull(cd_modo_pagamento) OR cd_modo_pagamento < 1 then
				...
				total_a_pagar = total_a_pagar + nm_valor + nm_valor_extra
					if nm_valor=0 AND nm_valor_extra=0 AND isnull(cd_suspenso) then
						...
						total_falta_pagto = total_falta_pagto + nm_valor + nm_valor_extra
