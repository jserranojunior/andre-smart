...
cd_tipo_valor = request("cd_tipo_valor")
	response.write(cd_tipo_valor)
cd_tipo_valor_atual = request("cd_tipo_valor_atual")
cd_categoria_valor = request("cd_categoria_valor")

nr_desconto = request("nr_desconto")
nm_text_desconto = request("nm_text_desconto")
...
if cd_cancela = 1 then '*** Somente em caso de cancelamento da conta a pagar
	...
	dt_cancela = mes_final&"/"&UltimoDiaMes(mes_final,ano_final)&"/"&ano_final
...
'		end if
'		dt_final = "'"&mes_final&"/"&UltimoDiaMes(mes_final,ano_final)&"/"&ano_final&"'"
'		response.write("<br>***AQUI: "&dt_final&"***<br>")
'	next
'end if
...
IF x = "" THEN
	...
	IF acao = "inserir" THEN
		...
		'*** Registra no diário ***
		response.write("up_financeiro_diario_insert3 @dt_inicial="&dt_inicial&", @dt_final="&dt_final&", @pre_ordem="&dia_vencimento&", @ordem_padrao="&dia_vencimento&", @cd_fornecedor='"&cd_fornecedor&"', @nm_descricao='"&nm_descricao&"', @cd_area='"&cd_area&"', @cd_centrocusto='"&cd_centrocusto&"', @cd_conta='"&cd_conta&"', @cd_unidade='"&cd_unidade&"', @cd_tipo_valor='"&cd_tipo_valor&"', @cd_categoria_valor="&cd_categoria_valor&", @cd_qtd_parcelas='"&cd_qtd_parcelas&"', @cd_user='"&cd_user&"', @data_atual='"&data_atual&"', @cd_origem='"&str_origem&"'")
				...
				Set rs_diario = dbconn.execute(strsql)
					...
					if not rs_diario.EOF then
						...
						for i_parc = 1 to cd_qtd_parcelas
								...
								if i_parc = 1 then '** parcela única ou primeira ***
									dt_vencimento = dt_vencimento
								
								elseif i_parc > 1 then '** demais parcelas ***
									mes_vencimento = mes_vencimento + 1
									ultimodia = ultimodiames(mes_vencimento,ano_vencimento)
										...
										'*** Se o dia for maior que o ultimo dia do mes ***
										if abs(dia_vencimento) > abs(ultimodia) then
									...
									'*** Se a parcela for a primeira, grava o valor.
									if cd_categoria_valor  = 1 then
										nm_valor = nm_valor
									else
										nm_valor = "0.00"
									end if
								...
								dbconn.execute(xsql)
									...
									if codigo_origem <> "" Then
										xsql = "up_orcamento_update_pagto @cd_codigo="&codigo_origem&", @cd_diario='"&cd_diario&"', @cd_pagto=1"
										dbconn.execute(xsql)
									end if
						next
					end if
	...
	response.write(acao&"<br>")
		 IF cd_cancela = 0 OR ISNULL(cd_cancela) then
		 '*** Altera os dados do pagamento ***
		response.write("@cd_diario='"&cd_diario&"', @nm_descricao='"&nm_descricao&"', @cd_fornecedor='"&cd_fornecedor&"', @cd_tipo_valor='"&cd_tipo_valor&"', @cd_categoria_valor="&cd_categoria_valor&", @ordem_padrao='"&dia_vencimento&"', @cd_qtd_parcelas='"&cd_qtd_parcelas&"', @cd_area='"&cd_area&"', @cd_centrocusto='"&cd_centrocusto&"', @cd_conta='"&cd_conta&"', @cd_unidade='"&cd_unidade&"', @dt_inicial='"&dt_inicial&"', @dt_final="&dt_final&", @cd_user='"&cd_user&"', @data_atual='"&data_atual&"'")
		xsql = "up_financeiro_diario_update3 @cd_diario='"&cd_diario&"', @nm_descricao='"&nm_descricao&"', @cd_fornecedor='"&cd_fornecedor&"', @cd_tipo_valor='"&cd_tipo_valor&"', @cd_categoria_valor="&cd_categoria_valor&", @ordem_padrao='"&dia_vencimento&"', @cd_qtd_parcelas='"&cd_qtd_parcelas&"', @cd_area='"&cd_area&"', @cd_centrocusto='"&cd_centrocusto&"', @cd_conta='"&cd_conta&"', @cd_unidade='"&cd_unidade&"', @dt_inicial='"&dt_inicial&"', @dt_final="&dt_final&", @cd_user='"&cd_user&"', @data_atual='"&data_atual&"'"
		'respose.write("***..."&cd_unidade&"...****")
		dbconn.execute(xsql)
		
		Elseif cd_cancela = 1 then
			'dt_cancela = "2015-07-31"
			'*** Altera a data final do pagamento - dt_vencimento
			strsql = "up_financeiro_diario_update_cancela3 @dt_final='"&dt_cancela&"', @cd_qtd_parcelas='"&cd_qtd_parcelas&"', @cd_diario="&cd_diario&", @cd_user="&cd_user&", @data_atual='"&data_atual&"' "
			dbconn.execute(strsql)
			response.write("cancelado:"&cd_cancela&". dt:"&dt_cancela&" parc:"&cd_qtd_parcelas&" cod:"&cd_diario&"...<br>")
		end if 
		
			if cd_origem = 6 then '*** Ao alterar a data de vencimento do orçamento, altera a data de vencimento do valor ***
				strsql = "UPDATE TBL_financeiro_valores3 SET dt_vencimento='"&ano_vencimento&"-"&mes_vencimento&"-"&dia_vencimento&"' WHERE cd_diario='"&cd_diario&"'"
				dbconn.execute(strsql)
			end if
	...
	response.write(acao&"<br>")
		...
		xsql = "up_financeiro_valor_insert3 @cd_diario="&cd_diario&", @nm_pagamento='"&nm_descricao&"', @nm_valor='"&nm_valor&"', @cd_parcela="&cd_parcela&", @dt_vencimento='"&dt_vencimento&"', @nm_obs='"&nm_obs&"', @cd_suspenso="&cd_suspenso&", @cd_user="&cd_user&", @data_atual='"&data_atual&"'"
		dbconn.execute(xsql)
	...
	response.write(acao&"<br>")
		...
		if dt_final <> "NULL" Then
			xsql = "up_financeiro_valor_update3_ok_f_date @cd_valor="&cd_valor&", @cd_diario='"&cd_diario&"', @cd_tipo_valor='"&cd_tipo_valor&"', @nm_pagamento='"&nm_descricao&"', @nm_valor='"&nm_valor&"', @cd_parcela="&cd_parcela&", @dt_vencimento='"&dt_vencimento&"', @nm_obs='"&nm_obs&"', @cd_suspenso="&cd_suspenso&", @dt_final="&dt_final&",@cd_user="&cd_user&", @data_atual='"&data_atual&"'"
		else
			xsql = "up_financeiro_valor_update3_no_f_date @cd_valor="&cd_valor&", @cd_diario='"&cd_diario&"', @cd_tipo_valor='"&cd_tipo_valor&"', @nm_pagamento='"&nm_descricao&"', @nm_valor='"&nm_valor&"', @cd_parcela="&cd_parcela&", @dt_vencimento='"&dt_vencimento&"', @nm_obs='"&nm_obs&"', @cd_suspenso="&cd_suspenso&", @dt_final=NULL,@cd_user="&cd_user&", @data_atual='"&data_atual&"'"
		end if
		dbconn.execute(xsql)
			...
			if cd_tipo_valor <> cd_tipo_valor_atual then
				if cd_tipo_valor = 3 then ' Altera para parcelado, calcula e insere as demais parcelas e datas
					'*** verifica as parcelas já existentes ***
						...
						'dbconn.execute(strsql)
						'response.write("cancelado:"&cd_cancela&".")
						end if
				...
				elseif cd_tipo_valor = 2 then 'Altera Para Fixo e se certifica que a data de témino esteja Null
					...
					if int(cd_tipo_valor_atual) <> int(cd_tipo_valor) then
						strsql = "UPDATE TBL_financeiro_diario3 SET dt_final=null, cd_qtd_parcelas=1 WHERE cd_codigo="&cd_diario&""
					else
						strsql = "UPDATE TBL_financeiro_diario3 SET dt_final=null, cd_qtd_parcelas=1 WHERE cd_codigo="&cd_diario&""
					end if
				...
				if cd_pendencia = 1 AND pend_existe = "" then
	...
	'******************************************************************
	'*** 		Insere o desconto para a fatura						***
	'******************************************************************
	ELSEIF acao = "fatura_desconto" THEN
	response.write(acao&"<br>")
		'*** verifica se a unidade já possui desconto para o mes selecionado ***
			'strsql_verdesc = "SELECT * FROM TBL_financeiro_fatura_desc WHERE (cd_unidade = "&cd_unidade&") AND (dt_data = '"&mes_sel&"/1/"&ano_sel&"')"
			strsql_verdesc = "SELECT * FROM TBL_financeiro_fatura_desc WHERE cd_unidade ="&cd_unidade&" AND dt_data='"&ano_sel&"-"&mes_sel&"-01'"
			'strsql_verdesc = "SELECT * FROM TBL_financeiro_fatura_desc WHERE cd_unidade ="&cd_unidade&" AND dt_data='2014-07-01'"
			Set rs = dbconn.execute(strsql_verdesc)
			if not rs.EOF then
				cd_desconto = rs("cd_codigo")
				desc_existe = 1
			end if
		'*** Insere o desconto do faturamento ***
		if desc_existe = 0 then
			response.write("<br>Desconto:"&desc_existe&"<br>")
			xsql = "up_financeiro_fatura_desc_insert @dt_data = '"&ano_sel&"-"&mes_sel&"-01', @cd_unidade="&cd_unidade&", @nr_desconto="&nr_desconto&", @nm_text_desconto='"&nm_text_desconto&"', @cd_user="&cd_user&",@dt_cad='"&data_hoje&"'" 
			dbconn.execute(xsql)
			' response.write("<br>***<br>@dt_data = '"&ano_sel&"-"&mes_sel&"-01', @cd_unidade="&cd_unidade&", @nr_desconto="&nr_desconto&", @nm_text_desconto='"&nm_text_desconto&"', @cd_user="&cd_user&",@dt_cad='"&data_hoje&"'<br>***<br>")
			response.redirect("../fatura3.asp?tipo=fatura&mes_sel="&mes_sel&"&ano_sel="&ano_sel&"&cd_unidade="&cd_unidade&"")
		'*** Altera o desconto do faturamento ***
		else
			if isnumeric(nr_desconto) then
				response.write("<br>Desconto:"&desc_existe&"<br>")
				xsql = "up_financeiro_fatura_desc_update @dt_data='"&ano_sel&"-"&mes_sel&"-01', @cd_unidade="&cd_unidade&", @nr_desconto="&nr_desconto&", @nm_text_desconto='"&nm_text_desconto&"', @cd_user="&cd_user&", @dt_cad='"&data_hoje&"'" 
				dbconn.execute(xsql)
				response.redirect("../fatura3.asp?tipo=fatura&mes_sel="&mes_sel&"&ano_sel="&ano_sel&"&cd_unidade="&cd_unidade&"")
			else
				xsql = "up_financeiro_fatura_desc_delete @cd_desconto='"&cd_desconto&"'" 
				dbconn.execute(xsql)
				response.redirect("../fatura3.asp?tipo=fatura&mes_sel="&mes_sel&"&ano_sel="&ano_sel&"&cd_unidade="&cd_unidade&"")
			end if
		end if
	...
	ELSEIF acao = "apaga_cheque" THEN
	'response.write(acao&"-"&cd_cheque)
	
		xsql = "up_financeiro_cheque_delete3 @cd_cheque = "&cd_cheque
		dbconn.execute(xsql)
...
@cd_categoria_valor=<%=cd_categoria_valor%><br>
...
@dt_inicial=<%=dt_inicial%><br>
@dt_final=<%=dt_final%><br>
@cd_user=<%=cd_user%><br>
...
cd_categoria_valor = <%=cd_categoria_valor%><br><br>
...
@cd_diario="&<%=cd_diario%>&", <br>
@nm_valor='"&<%=nm_valor%>&"', <br>
@cd_parcela:<%=cd_parcela%><br> 

@dt_inicio_pendencia='"&<%=mes_vencimento&"/1/"&ano_vencimento%>&"', <br>
@cd_user="&<%=cd_user%>&", <br>
@dt_cad='"&<%=data_atual%>&"'"<br>
<br>
<br>
'up_financeiro_diario_update3 
		@cd_diario=<%=cd_diario%><br>
		@nm_descricao=<%=nm_descricao%> <br>
		@cd_fornecedor=<%=cd_fornecedor%> <br>
		@cd_tipo_valor=<%=cd_tipo_valor%> <br>
