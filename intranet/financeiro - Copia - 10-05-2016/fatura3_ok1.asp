...
data_atual = month(now)&"/"&day(now)&"/"&year(now)&" "&hour(now)&":"&minute(now)%>
	...
	<%if cd_unidade = "0" OR cd_unidade = "" Then
		...
		dia_vencimento2 = rs("dia_vencimento2")
		...
		cd_prazo_faturamento = rs("cd_prazo_faturamento")
			...
			'if cd_prazo_faturamento <> "" Then
				mes_faturamento = dt_mes - 2'int(cd_prazo_faturamento)
			...
			'response.write(cd_prazo_faturamento&"***")
			...
			nm_empresa = rs("nm_empresa")
			cd_cnpj_emp = rs("cd_cnpj")
			nm_endereco_emp = rs("nm_endereco_emp")
			nm_numero_emp = rs("nm_numero")
			nm_complemento_emp = rs("nm_complemento")
			nm_bairro_emp = rs("nm_bairro")
			nm_cidade_emp = rs("nm_cidade")
			nm_estado_emp = rs("nm_estado")
			nm_cep_emp = rs("nm_cep")
			telefone_emp = rs("telefone")
			nm_email_emp = rs("nm_email")
			nm_site_emp = rs("nm_site")
			
			nm_banco_emp = rs("nm_banco")
			nm_agencia_emp = rs("nm_agencia")
			nm_contacorrente_emp = rs("nm_contacorrente")
	...
	**********************************************-->
			<table align="center" width="500" border="0" style="border-collapse:collapse;">
				...
				<tr>
					<td>&nbsp;<!-- limitador de linha --></td>
					<td colspan="2" valign="top" style="font-family:arial; font-size:18px; text-align:justify; text-justify:inter-word;">Recebi da <b><%=nm_empresa%></b>, o relatório dos procedimentos realizados por videocirurgia no mês de <%=mes_selecionado(mes_faturamento)&" de "&ano_faturamento%>, no <b><%=nm_unidade%></b>, junto com o recibo de pagamento da locação dos equipamentos de videocirurgia relativo ao mês com vencimento em <b><%=dia_vencimento%> <%if dia_vencimento2 <> "" Then response.write(" e "&dia_vencimento2)%> de <%=mes_selecionado(dt_mes)&" de "&dt_ano%></b>.</td>
				</tr>
						...
						<img src="../imagens/px.gif" alt="" width="1" height="40" border="0"><br>
							...
							<%else%>
								<table border="0" align="center" style="font-family:arial; font-size:15px; border-collapse:collapse;padding:50px 40px; background:transparent;">
										...
										<%else
										valor_total_procedimentos_div = valor_total_procedimentos/2%>
										<tr>
											<td align="left" style="padding:1px 15px;"><b>Vencimento</b></td>
											<td align="left" style="padding:1px 5px;"><b><%=zero(dia_vencimento)&"/"&zero(dt_mes)&"/"&dt_ano&" - "&formatCurrency(valor_total_procedimentos_div)%></b></td>
										</tr>
										<tr>
											<td align="left" style="padding:1px 15px;">&nbsp;</td>
											<td align="left" style="padding:1px 5px;"><b><%=zero(dia_vencimento2)&"/"&zero(dt_mes)&"/"&dt_ano&" - "&formatCurrency(valor_total_procedimentos_div)%></b></td>
										</tr>
										<%end if%>
										<tr>
