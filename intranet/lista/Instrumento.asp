...
<body>
<table border="1">
	<tr style="background-color:gray; color:white;"><td colspan="3"> &nbsp; Lista dos nomes da tabela de equipamentos/<b>instrumentos</b>/óticas/materiais &nbsp; </td></tr>
	<tr style="background-color:silver;"><td>Nº</td><td>Status</td><td>Item</td><td align="center">M</td></tr>
	<%num_linha = 0
	num_linha_pag = 0
	xsql_eqp = "Select * From TBL_equipamento where cd_status=1 and nm_equipamento_novo is not Null and cd_tipo = 3 order by nm_equipamento_novo"
		...
		<%if num_linha_pag = 40 then
			...
			cabeca = 0%>
				...
				<tr style="background-color:silver;" id="ok_print"><td>Nº</td><td>Status</td><td>Item</td><td align="center">M</td></tr>
		<%end if
		rs_eqp.movenext
		selected = ""
		loop%>
