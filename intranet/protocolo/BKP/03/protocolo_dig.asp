		...
		<tr bgcolor="#808080">
			<td align="center" colspan="100%"><font color="white"><b>&nbsp;Identificação do Paciente - Etiqueta<%'=acao%></b></font></td>
		</tr>
		<tr bgcolor="#e0e0e0">
		...
		<tr  bgcolor="#f7f7f7">
			...
			<td><b>
				<select name="nm_agenda_cirurgia" class="inputs" onFocus="nextfield ='cd_co';">
					<option value=""></option>
			...
			<td><input type="checkbox" name="cd_co" value="1" <%=co_ck%> onFocus="nextfield ='dt_hora_agenda';">&nbsp;Sim</td>
		</tr>
		<tr bgcolor="#f7f7f7"><td colspan="100%">&nbsp;<!--Pula linha--></td></tr>
		<tr bgcolor="#e0e0e0">
			<td>&nbsp;Hora Agendada</td>
		...
		<tr bgcolor="#f7f7f7">
			...
			<td colspan="1">&nbsp;<b><input type="text" name="cd_especialidade" size="3" class="inputs" value="<%=cd_especialidade%>" onKeyup="mostra_espec();" onFocus="nextfield ='cd_procedimento_busca';">
			...
			cd_procedimento = rs_proc("a058_codpro")
				...
				if num_cor = 1 then cor_linha = "#e5e5e5" else cor_linha = "#ffffff" end if%>
					<tr>
						...
						<td bgcolor="<%=cor_linha%>"><img src="imagens/ic_del.gif" onclick="javascript:JsProcDelete('<%=rs_proc("cd_codigo")%>','<%=cd_protocolo%>','<%=cd_unidade%>','<%=cd_digito%>')" type="button" value="A" title="Apagar"></td>
					</tr>				
				<%if num_cor < 2 then
					num_cor = num_cor + 1
				else
			...
			cd_material = rs_mat("a061_codmat")
				...
				<%Do while not rs_material.EOF%>
					...
					<tr>
						...
						<td bgcolor="<%=cor_linha%>"><img src="imagens/ic_del.gif" onclick="javascript:JsMatDelete('<%=rs_mat("cd_codigo")%>','<%=cd_protocolo%>','<%=cd_unidade%>','<%=cd_digito%>')" type="button" value="A" title="Apagar"></td>
					</tr>				
				<%if num_cor < 2 then
					num_cor = num_cor + 1
				else
...
<%if oticas = xx Then%>
		...
		***** NOVA ÁREA DE INCLUSÃO DE ÓTICAS*****-->		
		...
		<tr bgcolor="#f7f7f7">
			<td colspan="100%" bgcolor="#f5f5f5"><%'***** Substitui a vírgula pela sentença de busca *****
		if not cd_protocolo = "" Then
			strsql ="Select * from VIEW_protocolo_patrimonio_lista where cd_protocolo="&cd_protocolo&" and cd_unidade = "&cd_unidade&""
			Set rs_pat = dbconn.execute(strsql)
			
			%><table border="0">
				<tr>
					<td bgcolor="#f5f5f5"><img src="../imagens/px.gif" alt="" width="50" height="1" border="0"></td>
					<td align="right" valign="top" style="width:100px; color:white;" bgcolor="#d1d1d1">Cód.</td>
			...
			cd_patrimonio = rs_pat("cd_patrimonio")
				'cd_patrimonio = replace(cd_patrimonio,","," OR a058_codpro=")%>
			
				<%strsql ="Select * from View_patrimonio_lista where cd_patrimonio="&cd_patrimonio&""
				Set rs_patrimonio = dbconn.execute(strsql)%>
				<%Do while not rs_patrimonio.EOF
				if num_cor = 1 then cor_linha = "#e5e5e5" else cor_linha = "#ffffff" end if%>
					<tr>
						<td bgcolor="#f5f5f5"><img src="../imagens/px.gif" alt="" width="50" height="1" border="0"></td>
						<td align="right" valign="top" style="width:100px" bgcolor="<%=cor_linha%>"><%=rs_patrimonio("nm_tipo")%><%=rs_patrimonio("cd_patrimonio")%>&nbsp;</td>
						<td style="width:450px" bgcolor="<%=cor_linha%>">&nbsp; <%=left(rs_patrimonio("nm_equipamento"),80)&"<br>"%></td>
						<td bgcolor="<%=cor_linha%>"><img src="imagens/ic_del.gif" onclick="javascript:JsPatDelete('<%=rs_pat("cd_codigo")%>','<%=cd_protocolo%>','<%=cd_unidade%>','<%=cd_digito%>')" type="button" value="A" title="Apagar"></td>
					</tr>				
				<%if num_cor < 2 then
					num_cor = num_cor + 1
				else
...
<SCRIPT LANGUAGE="javascript">
	function JsProcDelete(cod1,cod2,cod3,cod4)
	{
	  if (confirm ("Tem certeza que deseja excluir o procedimento?"))
		  {
			document.location.href('protocolo/acoes/protocolo_acao.asp?cd_codigo='+cod1+'&cd_protocolo='+cod2+'&cd_unidade='+cod3+'&cd_digito='+cod4+'&acao=excluir&obj_excl=6');
		  }
	}
	...
	function JsMatDelete(cod1,cod2,cod3,cod4)
	{
	  if (confirm ("Tem certeza que deseja excluir o material de empréstimo?"))
		  {
			document.location.href('protocolo/acoes/protocolo_acao.asp?cd_material='+cod1+'&cd_protocolo='+cod2+'&cd_unidade='+cod3+'&cd_digito='+cod4+'&obj_excl=3&acao=excluir');
		  }
	}
	...
	function JsPatDelete(cod1,cod2,cod3,cod4)
	{
	  if (confirm ("Tem certeza que deseja excluir esta ótica?"))
		  {
			document.location.href('protocolo/acoes/protocolo_acao.asp?cd_codigo='+cod1+'&cd_protocolo='+cod2+'&cd_unidade='+cod3+'&cd_digito='+cod4+'&obj_excl=7&acao=excluir');
		  }
	}
