...
cd_categoria = request("cd_categoria")
	...
	'	txt_coluna = "RACK"
	'end if
'cd_detalhes = request("cd_detalhes")
	elseif cd_categoria = 5 then
		campos = ", a057_codesp, nm_especialidade, a055_crmmed, a055_desmed"
		txt_cabecalho = "MÉDICOS CIRURGIÕES X HISTEROSCOPIA"
		txt_coluna = "MÉDICO/CIRURGIÃO"
		detalhe = " AND (A057_codesp = 3) "
		'detalhes = 
	elseif cd_categoria = 6 then
		campos = ", a057_codesp, nm_especialidade, a055_crmmed, a055_desmed"
		txt_cabecalho = "MÉDICOS CIRURGIÕES X GINECOLOGIA"
		txt_coluna = "MÉDICO/CIRURGIÃO"
		detalhe = " AND (A057_codesp = 33) "
		'detalhes = 
	end if
%>
...
<table align="center" class="no_print">	
	<%if session("cd_codigo") = 46 then%>
		<tr><td colspan="100" align="center"><span style="font-size=8px;color:red;">protocolo_relatorio_top10.asp</span></td></tr>
	<%end if%>
	<tr>
	...
	<tr>
		<td>Período</td>
		<td><input type="text" name="dia_i" value="<%=dia_i%>" size="2" maxlength="2" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);">/<input type="text" name="mes_i" value="<%=mes_i%>" size="2" maxlength="2" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);">/<input type="text" name="ano_i" value="<%=ano_i%>" size="4" maxlength="4" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 4)" onFocus="PararTAB(this);"> &nbsp; à &nbsp; <input type="text" name="dia_f" value="<%=dia_f%>" size="2" maxlength="2" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);">/<input type="text" name="mes_f" value="<%=mes_f%>" size="2" maxlength="2" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);">/<input type="text" name="ano_f" value="<%=ano_f%>" size="4" maxlength="4" onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 4)" onFocus="PararTAB(this);"></td>
	</tr>
	<tr>
		<td>Unidade</td>
		<td>
	...
	<tr>
		...
		<td><select name="cd_categoria">
				...
				<option value="5" <%if cd_categoria = 5 then%>Selected<%end if%>>Médicos X Histeroscopia</option>
				<option value="6" <%if cd_categoria = 6 then%>Selected<%end if%>>Médicos X Ginecologia</option>
			</select>
		</td>
	</tr>
...
<table align="center">	
	...
	xsql = "SELECT TOP "&top_busca&" COUNT (a002_numpro) AS conta_previa FROM VIEW_protocolo_lista Where A002_datpro BETWEEN '"&mes_i&"/"&dia_i&"/"&ano_i&"' AND '"&mes_f&"/"&dia_f&"/"&ano_f&"' "&str_unidade&" "&contar_emprestimos&" "&detalhe&" ORDER BY conta_previa DESC"
	...
	'*** Conta os registros ***
	xsql = "SELECT top "&top_busca&" COUNT (a002_numpro) AS conta, a053_codung, nm_sigla "&campos&" FROM VIEW_protocolo_lista Where A002_datpro BETWEEN '"&mes_i&"/"&dia_i&"/"&ano_i&"' AND '"&mes_f&"/"&dia_f&"/"&ano_f&"' "&str_unidade&" "&contar_emprestimos&" "&detalhe&" Group by a053_codung, nm_sigla "&campos&" ORDER BY conta DESC"
	Set rs = dbconn.execute(xsql)
	...
	while not rs.EOF
		...
		elseif cd_categoria = 5 OR cd_categoria = 6 then
			variavel = rs("a055_desmed")
		end if
	...
	<%if cabeca = 0 Then%>
	<tr>
		<td class="ok_print"><img src="../../imagens/px.gif" alt="" width="80" height="1" border="0"></td>
		<td align="right" colspan="100%"><br>&nbsp;<img src="../imagens/logotipo_vdlap.gif" alt="" width="91" height="51" border="0"><br>&nbsp;</td>
	</tr>
	<tr><td class="ok_print"><img src="../../imagens/px.gif" alt="" width="80" height="1" border="0"></td>
		<td colspan="100%" align="center" style="background-color:gray; color:white; font-size:14px;">&nbsp;<br>TOP <%=top_busca%> - <%=txt_cabecalho%><br>CIRURGIAS DE <%=ucase(left(mes_selecionado(int(mes_i)),3))&"/"&ano_i%> À <%=ucase(left(mes_selecionado(int(mes_f)),3))&"/"&ano_f%><br><%=unidade%><%'=ucase(unidade)%><br>&nbsp;</td></tr>
	<tr>
		<td class="ok_print"><img src="../../imagens/px.gif" alt="" width="80" height="1" border="0"></td>
	...
	<tr style="font-size:12px;">
		...
		<td colspan="2" class="ok_print">TOTAL</td>
	...
	<tr style="font-size:12px;">
		...
		<td colspan="2" class="ok_print">OUTROS</td>
	...
	<tr>
		<td class="ok_print"><img src="../../imagens/px.gif" alt="" width="80" height="16" border="0"></td>
		<td colspan="100%"><img src="../../imagens/blackdot.gif" alt="" width="100%" height="1" border="0"></td>
	</tr>
	<tr style="font-size:12px;">
		<td>&nbsp;<img src="../../imagens/px.gif" alt="" width="80" height="14" border="0"></td>	
		<td align="left" colspan="2" class="ok_print">TOTAL DE CIRURGIAS NO PERÍODO</td>
		<td align="right">&nbsp;<%=conta_previa%> &nbsp; </td>
		<td align="right">&nbsp;<%'=porcentagem_geral%></td>
	</tr>
	<%num_linha = num_linha + 5
		while not num_linha = qtd_linhas_pag%>
		<tr><td class="ok_print"><img src="../../imagens/px.gif" alt="" width="80" height="14" border="0"></td>
	...
	<%if session_cd_usuario = 46 then%>
		<tr class="no_print">
			...
			SELECT top <%=top_busca%> COUNT (a002_numpro) AS conta, a053_codung, nm_sigla <%=campos%> <br>
			FROM VIEW_protocolo_lista 
			WHERE A002_datpro BETWEEN <%=mes_i&"/"&dia_i&"/"&ano_i%> AND <%=mes_f&"/"&dia_f&"/"&ano_f%> <%=str_unidade%> <%=contar_emprestimos%> <%=detalhe%><br>
			Group by a053_codung, nm_sigla <%=campos%><br>
			ORDER BY conta DESC
			</td>
		</tr>
	<%end if%>
