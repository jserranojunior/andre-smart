...
cd_cancelado = request("cd_cancelado")
...
'-------------------- Protocolo --------------------
	...
	if cd_cancelado = 1 then
		cancelado = "AND cd_cancelado = 1 "
	else
		cancelado = "AND cd_cancelado is null "
	end if
...
'**********************************************
'************** Condições *********************
condicao = "Where "&periodo&" "&unidade&" "&nome_unidade&" "&protocolo&" "&paciente&" "&idade&" "&sexo&" "&convenio&" "&medico&" "&agenda&" "&tipoagenda&" "&rack&" "&especialidade&" "&procedimento&" "&material&" "&patrimonio&" "&cancelado&" "
'**********************************************
'************** Agrupamento *********************
if campos <> "" Then
...
<body>
			...
			<table border="1" bordercolor="#C0C0C0" cellpadding="0" cellspacing="0" bordercolordark="#FFFFFF" class="no_print" width="500">
				...
				<tr>
					<td colspan="1"><img src="imagens/blackdot.gif" width="10" height="1"></td>
					<td colspan="1"><img src="imagens/blackdot.gif" width="40" height="1"></td>
					<td colspan="3"><img src="imagens/blackdot.gif" width="130" height="1"></td>
					<td colspan="4"><img src="imagens/blackdot.gif" width="560" height="1"></td>
					<td><img src="imagens/blackdot.gif" width="70" height="1"></td>
				</tr>
...
<!-- *** Data *** -->
				<tr bgcolor="#eaeaea" onmouseover="mOvr(this,'#ccffff');" onclick="javascript:;" onmouseout="mOut(this,'#eaeaea');">
					<td>&nbsp;</td>					
					<td colspan="3">
						<%if data <> 0  then%>
					<%ck_data = "checked"%>
...
<!-- *** Quantidade *** -->
				<tr bgcolor="#f7f7f7" onmouseover="mOvr(this,'#ccffff');" onclick="javascript:;" onmouseout="mOut(this,'');">
					<td>&nbsp;</td>
...
<!-- *** Nº Protocolo *** -->
				<tr bgcolor="#f7f7f7" onmouseover="mOvr(this,'#ccffff');" onclick="javascript:;" onmouseout="mOut(this,'');">
					...
					<td colspan="4"><!--input type="text" name="nm_und" size="2" maxlength="3" class="inputs" value="<%'=nm_und%>" class="inputs"-->
						<input type="text" name="nm_protocolo" size="10" maxlength="50" class="inputs" value="<%=mm_protocolo%>" class="inputs">
						&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Mostrar somente cancelados: <input type="radio" name="cd_cancelado" value="1" <%if cd_cancelado="1" then response.write("checked")%>> SIM / <input type="radio" name="cd_cancelado" value="0" <%if cd_cancelado="0" then response.write("checked")%>> NÃO
...
<!-- *** Tempo de cirurgia *** -->
				<tr bgcolor="#f7f7f7" onmouseover="mOvr(this,'#ccffff');"  onclick="javascript:;" onmouseout="mOut(this,'');">
					...
					<td colspan="3">
						...
						<input type="checkbox" name="campos" value=",a002_horini,a002_horfin" <%=ck_campo_10_2%>>&nbsp;Tempo da cirurgia
					</td>
					<td colspan="4">&nbsp;<%if atraso > 0 then%><%ck_atraso ="checked"%><%end if%><input type="checkbox" name="atraso" value="1"> &nbsp; Atraso &nbsp;&nbsp;&nbsp;
									&nbsp;<%if tempo > 0 then%><%ck_tempo ="checked"%><%end if%><input type="checkbox" name="tempo" value="1"> &nbsp; Tempo de cirurgia
					<!--	<input type="text" name="hora_i" value="<%=hora_i%>" size="2" maxlength="2">:<input type="text" name="minuto_i" value="<%=minuto_i%>" size="2" maxlength="2"> Até
						<input type="text" name="hora_f" value="<%=hora_f%>" size="2" maxlength="2">:<input type="text" name="minuto_f" value="<%=minuto_f%>" size="2" maxlength="2">
					-->
					</td>					
...
'ok = ""
	...
	if ok <> "" Then 'Inicio do resultado da pesquisa
			...
			'******************************	%>
				<tr>					
					<td bgcolor="#C0C0C0" align="center" colspan="100%">  <b>RELATÓRIOS - <font color="red">Protocolos<%if cd_cancelado = 1 then response.write(" cancelados")%></font></b></td>
				</tr>				
...
<%else%>
	...
	<%	
			...
			<%end if%>	
				...
				<%if data > 0 then%>
					<td valign="top"><%=A002_datpro%> <%'=compara%><%'=grava_info%></td><%end if%>
				<%if campo_10_1 > 0 then%>
