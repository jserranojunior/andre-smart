		...
		<span id='divMed'><select name="cd_crm_1" class="inputs" onChange="alimenta_medico(this.value);" onBlur="alimenta_medico(this.value);" onFocus="nextfield ='cd_rack';" onkeypress="detecta_enter(event,'cd_rack','<%=cd_user%>')">
								...
								strsql ="Select * from TBL_medicos where a055_status = 1 AND A055_codmed='"&cd_med&"' order by A055_desmed"
								Set rs_med = dbconn.execute(strsql)
		...
		<tr>
			...
			<b><input type="text" name="cd_procedimento_busca"  value=""  id="cd_procedimento_busca" size="10" maxlength="30" class="inputs"  onKeyup="mostra_procedimento();" onkeypress="detecta_enter(event,'somar_proc','<%=cd_user%>')">
