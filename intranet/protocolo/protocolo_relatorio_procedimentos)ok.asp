...
if cd_empresa = 1 or cd_empresa = "" then
	nome_empresa = "Vd Lap Cirúrgica LTDA"
	nm_logotipo = "Vdlap2p"
elseif cd_empresa = 2 then
	nome_empresa = "CMI Cirúrgica Ltda"
	nm_logotipo = "cmi2p"
elseif cd_empresa = 3 then
	nome_empresa = "LOC LAP Cirúrgica Ltda" 
	nm_logotipo = "loc2p"
end if%>
...
<table align="center" style="border:1px solid gray; font-size: 13px; border-collapse:collapse;" class="no_print">	
	<%if session("cd_codigo") = 46 then%>
		<tr><td colspan="100" align="center"><span style="font-size=8px;color:red;">protocolo_relatorio_procedimentos.asp</span></td></tr>
	<%end if%>
	<tr>
	...
	<tr bgcolor="#dadada">
		...
		<td><select name="cd_empresa">
			<option value="1" <%if cd_empresa = 1 Then%>SELECTED<%end if%>>Vd Lap</option>
			<option value="2" <%if cd_empresa = 2 Then%>SELECTED<%end if%>>CMI</option>
			<option value="3" <%if cd_empresa = 3 Then%>SELECTED<%end if%>>LOC LAP</option>
</select></td>
	</tr>
	<tr bgcolor="#dadada">
		<td>&nbsp;Unidade</td>
		...
		<td>
			<input type="text" name="dt_mes" size="2" maxlength="2" style="background-color:#ffffff" value="<%=dt_mes%>">
				<select name="ano_sel">
						...
						<option value="<%=ano_inicio%>" 
							<%if int(ano_inicio) = int(ano_sel) then%>
								<%response.write("selected")%>
							<%elseif ano_sel = "" Then%>
								<%response.write("selected")%>
							<%end if%>><%=ano_inicio%></option>
...
'******************************************************************************************************
	...
	'*************************************************************%>
		<tr>
			<td colspan="3" align="left">&nbsp;<img src="../imagens/<%=nm_logotipo%>.gif" alt="" width="75" height="43" border="0"></td>
			<td colspan="4" align="center"><b><%=nome_empresa%> _<%=pre_conta%></b></td>
		</tr>
		<tr>		
		...
		<tr style="font-size:11px;">
			<td style="border:1px; solid red;" colspan="100%" align="center" >
				...
				<b><%=nm_unidade%></b><br>
				<b><%=mes_selecionado(int(dt_mes))%>/<%=ano_sel%></b> <%'=procedimentos%> <%'=rack_num%></td>
		</tr>
		<tr><td colspan="100%"><img src="../imagens/blackdot.gif" alt="" width="100%" height="1" border="0"></td></tr>
			...
			'***********************************
				if rack_numerar = rack_num OR procedimentos = linha_numerar then
					...
					<%if proc_co = "" and cd_unidade <> 20 Then%>						
						<%conta_linhas = conta_linhas +1%>					
						<tr bgcolor="dadada" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#dadada');" style="font-size:8px; height:20px;" valign="middle"><td colspan="8"><!-- salto de linha para ajuste do rodapé -->&nbsp;<%'=conta_linhas%> <b>TOTAL DE PROCEDIMENTOS REALIZADOS:</b> <%if cd_unidade = 20 then%><%=rack_num%><%else%><%=procedimentos%><%end if%></td></tr>
					<%Else
						conta_linhas = conta_linhas + 1%>					
						<tr bgcolor="ffffff" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#ffffff');" valign="middle"><td colspan="8" style="font-size:8px; height:20px; border-top: 1px solid black; border-bottom: 1px solid black;"><!-- salto de linha para ajuste do rodapé -->&nbsp;<%'=conta_linhas%> <b>Total parcial de procedimentos:</b> <%if cd_unidade = 20 then%><%=rack_num%><%else%><%=procedimentos%><%end if%></td></tr>
					<%end if%>
				...
				if cd_unidade = 20 AND rack_numerar = rack_num AND int(procedimentos) = int(rack_total) then
					...
					<tr bgcolor="#dadada" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#dadada');" valign="middle"><td colspan="8" style="font-size:8px; height:20px; <%=style_border%>"><%'=conta_linhas%><%'="("&parciais_rack&"+"&conta_racks&")"%><!-- salto de linha para ajuste do rodapé -->&nbsp;<b>Procedimentos realizados com <%=rack_item%></b></td></tr>
...
<!--**********************************************************************************************************************************************************-->				
	...
	<%if mostra_cabecalho = 0 then%>
		<tr>
			<td colspan="3" align="left">&nbsp;<img src="../imagens/<%=nm_logotipo%>.gif" alt="" width="75" height="43" border="0"></td>
			<td colspan="4" align="center"><b><%=nome_empresa%></b></td>
		</tr>
		<tr>		
			<td><img src="../imagens/px.gif" alt="" width="20" height="1" border="0"></td>
			<td><img src="../imagens/px.gif" alt="" width="45" height="1" border="0"></td>
	...
	<%end if%>
		...
		'*** Esconde o cabeçalho no meio do formulario***
			mostra_cabecalho = 1
		'************************************************
		'Caso os registros acabem na mesma qtd. de linhas, mostra cabeçalho e pula página
		'*********************************************************************************
		...
		<tr id="print_ok"><td colspan="8"><img src="../imagens/blackdot.gif" alt="" width="100%" height="1" border="0"></td></tr>
			<tr style="font-size:10px;" >
				...
				<%'*** cabeçalho***%>
					<tr>
						<td colspan="3" align="left">&nbsp;<img src="../imagens/<%=nm_logotipo%>.gif" alt="" width="75" height="43" border="0"></td>
						<td colspan="2" align="center"><b><%=nome_empresa%></b></td>
					</tr>
					<tr>		
						<td><img src="../imagens/px.gif" alt="" width="20" height="1" border="0"></td>
						<td><img src="../imagens/px.gif" alt="" width="45" height="1" border="0"></td>
		...
		'*** fim do cabeçalho para adequação do conteúdo ***
		...
		if conta_linhas = qtd_linhas_pag OR proc_co = linha_numerar OR rack_numerar = rack_num then
			...
			'**********************************
				...
				total_procedimentos = total_procedimentos + proc_co
					conta_linhas = conta_linhas + 1%>
					<tr><td colspan="100%">&nbsp;<%'=conta_linhas&"="&qtd_linhas_pag&" ou "&proc_co&"="&linha_numerar&" ou "&rack_numerar&"="&rack_num%></td></tr>
			...
			'**********************************
				if linha_numerar_e = emprestimos then
					...
					<tr bgcolor="dadada" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#dadada');" style="font-size:8px; height:20px;" valign="middle">
						<td align="right" colspan="4" style="font-size:8px; height:20px; border-bottom: 1px solid black; border-top: 1px solid black;">&nbsp;<b>TOTAL DE PROCEDIMENTOS DE EMPRESTIMO:</b> <%=rack_num_e%></td>
						<td align="left" style="border-bottom: 1px solid black; border-top: 1px solid black;"> &nbsp; &nbsp; <b><%=zero(emprestimos)%></b></td>
						<td align="right" style="border-bottom: 1px solid black; border-top: 1px solid black;"></td>
						<td align="right" style="border-bottom: 1px solid black; border-top: 1px solid black;">ITENS EMPRESTADOS: <%=rack_num_e%></td>
						<td colspan="2" style="border-bottom: 1px solid black; border-top: 1px solid black;"> &nbsp; &nbsp; <%=parcial_emprestimos%></td>
					</tr>					
				<%end if%>				
			<%'***********************************************************************
