		...
		Set rs = dbconn.execute(xsql)
			...
			response.write("<br>ok:"&no_protocolo&"<br>")
				...
				if cod_exist_protocolo <> "" and acao = "inserir" Then
					acao = "editar"	
				elseif cod_exist_protocolo <> "" and acao = "cancelar" Then
					mensagem = "O protocolo já encontra-se cancelado"
					response.redirect("../../protocolo.asp?tipo=digitacao&modalidade=cancelar&no_protocolo="&no_protocolo&"&cd_unidade="&zero(cd_unidade)&"&no_digito=o&mensagem="&mensagem&"")
				end if
...
x = int("100")
if x = int("100") then	'evita que os dados sejam gravados no Banco de dados, fins de desenvolvimento
	...
	mensagem = "Ok, protocolo alterado com sucesso"
		...
		if IsNumeric(proc_item) then
			...
			Set rs_proc = dbconn.execute(xsql)
				...
				codigo_do_protocolo = rs_proc("cd_protocolo")
				response.write("<br>>>>"&codigo_do_protocolo&"<<<<br>")
				
				'if not rs_proc.EOF then
				if codigo_do_protocolo <> "" Then
				response.write("COD:"&codigo_do_protocolo&"<br>")
				...
				Else
					...
					mensagem2 = "Procedimento inserido."
				'elseif 
				'	response.write("nada a declarar")
				End if
		end if
...
'*************************************************************************************************************************************************************
	...
	'Cancela protocolo
	xsql ="up_protocolo_cancelar @no_protocolo="&no_protocolo&", @cd_unidade="&cd_unidade&",@cd_user="&cd_user&",@data_atual='"&day(data_atual)&"/"&month(data_atual)&"/"&year(data_atual)&"'"
	dbconn.execute(xsql)
...
'	
	...
	End if
'Else
	if int(cd_user) <> desenvolvedor then
