...
function limpa_ordem(obj){
	...
	document.form.ordem_11_2.value='';
	...
	document.ordem_11_2.style.display='inline';
...
'**********************************************
		...
		campo_10_2 = Instr(1,campos,"inicio_cir,tempo_cir,atraso,a002_horini,a002_horfin",0)
...
', { fn HOUR(dbo.TBL_Protocolo.A002_horini)                       } & { fn MINUTE(dbo.TBL_Protocolo.A002_horini) } AS MINuto,
'-------------------- Paciente --------------------
	if nm_paciente <> "" then
...
<%ordem_total = request("ordem_total")
	ver_string_ordem = instr(ordem_total,",")
		if campo_9_0 > 0 then 'Ano
			ordem_total = replace(ordem_total,"a002_datpro","dt_ano")
			ordem_total = replace(ordem_total,"dt_ano,dt_mes","dt_ano")
		elseif campo_9_1 > 0 then 'Ano e mês
			ordem_total = replace(ordem_total,"dt_ano","dt_ano,dt_mes")
				ordem_total = replace(ordem_total,"dt_ano,dt_mes,dt_mes","dt_ano,dt_mes")
			ordem_total = replace(ordem_total,"a002_datpro","dt_ano,dt_mes")
		elseif campo_9_2 > 0 then 'Ano, mes e dia
			ordem_total = replace(ordem_total,"dt_ano,dt_mes","a002_datpro")
			ordem_total = replace(ordem_total,"dt_ano","a002_datpro")
		end if
		
	response.write(ordem_total)
			...
			if ordem_total <> "" Then
				...
				ordem_primaria = instr(ordem_total,",")
					if ordem_primaria = 0 then
						...
						ordem_primaria = ordem_primaria-1'len(ordem_total)-1
					end if
...
<body>
			...
			<table border="1" bordercolor="#C0C0C0" cellpadding="0" cellspacing="0" bordercolordark="#FFFFFF" id="no_print" width="500">
				...
				<tr id="no_print">
					<td>&nbsp;</td>
					<td bgcolor="#C0C0C0" align="center" colspan="100%"><b>BUSCA AVANÇADA - <font color="red">Protocolos</font></b></td>
				</tr>
				<tr id="no_print">
...
<!-- *** Nº Protocolo *** -->
				<tr bgcolor="#f7f7f7" id="no_print" onmouseover="mOvr(this,'#ccffff');" onclick="javascript:;" onmouseout="mOut(this,'');">
					...
					<td colspan="4"><!--input type="text" name="nm_und" size="2" maxlength="3" class="inputs" value="<%'=nm_und%>" class="inputs"--><input type="text" name="nm_protocolo" size="10" maxlength="50" class="inputs" value="<%=nm_protocolo%>" class="inputs">
...
<!-- *** Convenio *** -->
				<tr bgcolor="#f7f7f7" id="no_print" onmouseover="mOvr(this,'#ccffff');"  onclick="javascript:;" onmouseout="mOut(this,'');">
					...
					<td colspan="3"><%if campo_6_1 <> 0 then%><%ck_campo_6 = "checked"%><%end if%>
						<input type="checkbox" name="campos" value=",a054_codcon, nm_convenio" <%=ck_campo_6%>>&nbsp;Convênio 
					</td>
...
<!-- *** Realização da cirurgia *** -->
				<tr bgcolor="#f7f7f7" id="no_print" onmouseover="mOvr(this,'#ccffff');"  onclick="javascript:;" onmouseout="mOut(this,'');">
					...
					<td colspan="3" valign="top">
						<%if campo_10_2 > 0 then%><%ck_campo_10_2 ="checked"%><%end if%>
						<input type="checkbox" name="campos" value=",inicio_cir,tempo_cir,atraso,a002_horini,a002_horfin" <%=ck_campo_10_2%>>&nbsp;Hora realização
					</td>
					<td colspan="4">&nbsp;<br>
									&nbsp;<input type="checkbox" name="tempo" value="1" <%if tempo = 1 then%>checked<%end if%>> &nbsp; Tempo de cirurgia &nbsp;&nbsp;&nbsp;<br>
									&nbsp;<input type="checkbox" name="atraso" value="1" <%if atraso = 1 then%>checked<%end if%>> &nbsp; Atraso
					<!--	<input type="text" name="hora_i" value="<%=hora_i%>" size="2" maxlength="2">:<input type="text" name="minuto_i" value="<%=minuto_i%>" size="2" maxlength="2"> Até
						<input type="text" name="hora_f" value="<%=hora_f%>" size="2" maxlength="2">:<input type="text" name="minuto_f" value="<%=minuto_f%>" size="2" maxlength="2">
					-->
					</td>					
					...
					ordem_num="ordem_11"
					ordem_campo = "inicio_cir,a002_horini"%>
					<img src="../../imagens/blackdot.gif" alt="Inclui" width="15" height="15" border="0" onClick="ordem('<%=ordem_campo%>',document.form.ordem_total.value,'<%=ordem_num%>')" id="<%=ordem_num%>" name="<%=ordem_num%>" <%if ordem_var <> "" Then%>style="display:none;"<%end if%>><input type="text" name="<%=ordem_num%>" size="2" maxlength="2" readonly="1" style="border: 1px solid none;" value="<%=ordem_var%>">
					<%ordem_var = ordem_11_1
					ordem_num="ordem_11_1"
					ordem_campo = "tempo_cir"%>
					<img src="../../imagens/blackdot.gif" alt="Inclui" width="15" height="15" border="0" onClick="ordem('<%=ordem_campo%>',document.form.ordem_total.value,'<%=ordem_num%>')" id="<%=ordem_num%>" name="<%=ordem_num%>" <%if ordem_var <> "" Then%>style="display:none;"<%end if%>><input type="text" name="<%=ordem_num%>" size="2" maxlength="2" readonly="1" style="border: 1px solid none;" value="<%=ordem_var%>">
					<%ordem_var = ordem_11_2
					ordem_num="ordem_11_2"
					ordem_campo = "atraso"%>
					<img src="../../imagens/blackdot.gif" alt="Inclui" width="15" height="15" border="0" onClick="ordem('<%=ordem_campo%>',document.form.ordem_total.value,'<%=ordem_num%>')" id="<%=ordem_num%>" name="<%=ordem_num%>" <%if ordem_var <> "" Then%>style="display:none;"<%end if%>><input type="text" name="<%=ordem_num%>" size="2" maxlength="2" readonly="1" style="border: 1px solid none;" value="<%=ordem_var%>"></td>
				</tr>
<!-- *** Tipo de agenda *** -->
...
<!-- *** Procedimento *** -->
				<tr bgcolor="#f7f7f7" id="no_print" onmouseover="mOvr(this,'#ccffff');"  onclick="javascript:;" onmouseout="mOut(this,'');">
					...
					<td colspan="4"><input type="text" name="nm_procedimento" size="50" maxlength="50" class="inputs" value="<%=str_procedimento%>">&nbsp;
					<input type="checkbox" name="proc_todos" value="1" <%if proc_todos = 1 then%>checked<%end if%>> Todos os procedimentos</td>
...
<!-- *** Patrimonio *** -->
				<tr bgcolor="#f7f7f7" id="no_print" onmouseover="mOvr(this,'#ccffff');"  onclick="javascript:;" onmouseout="mOut(this,'');">
					...
					<td colspan="3"><%if campo_16_1 <> 0 then%><%ck_campo_16 = "checked"%><%end if%>
						<input type="checkbox" name="patrimonios" value="patrimonios,a002_numpro" <%=ck_campo_16%>>&nbsp;Óticas
					</td>
...
'ok = ""
	...
	if ok <> "" Then 'Inicio do resultado da pesquisa
			...
			'******************************	%>
				...
				<tr bgcolor="#c0c0c0">
					<td>Nº<br><img src="../imagens/px.gif" alt="" width="20" height="1" border="0"></td>
					...
					<%if campo_10_2 > 0 then%><td>Inicio<br><img src="../imagens/px.gif" alt="" width="45" height="1" border="0"></td>
						...
						<%if tempo = "1" Then%><td>Tempo<br><img src="../imagens/px.gif" alt="" width="45" height="1" border="0"></td><%end if%>
						<%if atraso = "1" Then%><td>Atrasos<br><img src="../imagens/px.gif" alt="" width="45" height="1" border="0"></td><%end if%>
					<%end if%>
...
<%else%>
	...
	<%	
			...
			<%end if%>	
				...
				<tr><td bgcolor="#ebebeb">&nbsp;</td><td bgcolor="#ebebeb" colspan="100%"><b><i><%=zero(sub_conta)%></i></b></td></tr><tr><td>&nbsp;</td></tr>
				<%sub_conta = 0
				end if%>
						...
						'--------------------------------------------------------------------------------------
							...
							if atraso = 1 then
								 if hrinicio > hragenda then
									...
									cor_atrasos = "red"
								elseif hrinicio < hragenda then
									cirurgia_atraso = "&nbsp;"&formatahora(hragenda - hrinicio) 'ANTECIPADAS
									cor_atrasos = "green"
								else
									cirurgia_atraso = "&nbsp;"&formatahora(hragenda - hrinicio) 'NO HORÁRIO
									cor_atrasos = "black"
								end if
							end if
					end if%>
						
						<%if tempo = "1" Then%><td valign="top" align="right"><b><%=cirurgia_tempo%></b><%'=" - "&tempo_cir%></td><%end if%>
						<%if atraso = "1" Then%><td valign="top" align="right" style="color:<%=cor_atrasos%>;"><%if campo_10_1 = 0 then%>Selecione<br> "hora &nbsp; &nbsp; <br>agenda"&nbsp;<%else%><%=cirurgia_atraso%><%end if%></td><%end if%>
				<%end if%>
			...
			<%if subtotais = 1 then%>
				<tr><td bgcolor="#ebebeb">&nbsp;</td><td bgcolor="#ebebeb" colspan="95%"><b><i><%=zero(sub_conta)%></i></b></td></tr><tr><td>&nbsp;</td></tr>
			<%end if%>
				<tr><td colspan="100%"><img src="imagens/blackdot.gif" width="100%" height="1"></td></tr>					
				<tr bgcolor="#e1e1e1">
