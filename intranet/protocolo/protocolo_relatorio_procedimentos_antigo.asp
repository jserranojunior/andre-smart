...
<table align="center" style="border:1px solid gray; font-size: 13px; border-collapse:collapse;" class="no_print">	
	<tr>
		<td align="center" style="border:1px solid gray; font-size: 15px; border-collapse:collapse;"><b>Relatorio de Cirurgia</b></td>
	</tr>
	<tr bgcolor="#dadada">
...
'******************************************************************************************************
	...
	<%'******************************************************
	'*** ------------------------------------------------ ***
	'*** INICIO DO RELATORIO DE CIRURGIAS E PROCEDIMENTOS ***
	'*** ------------------------------------------------ ***
	'********************************************************%>	
		<!-- mostrava o cabe�alho -->	
...
do dia 1 at� o dia <%=ultimodia%>
		...
		<tr><td colspan="100%"><img src="../imagens/blackdot.gif" alt="" width="100%" height="1" border="0"></td></tr>
		<%'*** Cabe�alho de separa��o de racks ***
	...
	<%end if%>
		...
		'*** Esconde o cabe�alho no meio do formulario***
			mostra_cabecalho = 1
		'**************************************************
		
		'if reg_num = qtd_linhas_pag OR rack_numerar = rack_num OR procedimentos = reg_num OR procedimentos = linha_numerar then 'Salto de p�gina caso alcance 46 registros ou se o alcan�ar a quantidade de registros por rack
			...
			'***********************************
				if rack_numerar = rack_num OR procedimentos = linha_numerar then
					...
					<!--tr style="height:20px;"><td colspan="8" valign="bottom"><img src="../imagens/blackdot.gif" alt="" width="100%" height="1" border="0"></td></tr-->
						<%if conta_linhas < qtd_linhas_pag-1 Then ' *** Pula uma linha caso haja espa�o ***
							conta_linhas = conta_linhas +1%>
							<tr><td>&nbsp;<%'=conta_linhas%></td></tr>
						<%end if%>
					<%if proc_co = "" and cd_unidade <> 20 and cd_unidade <> 14 Then%>						
						<%conta_linhas = conta_linhas +1%>					
						<tr bgcolor="dadada" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#dadada');" style="font-size:8px; height:20px;" valign="middle"><td colspan="8"><!-- salto de linha para ajuste do rodap� -->&nbsp;<%'=conta_linhas%> <b>TOTAL DE PROCEDIMENTOS REALIZADOS:</b> <%if cd_unidade = 20 or cd_unidade = 14 then%><%=rack_num%><%else%><%=procedimentos%><%end if%></td></tr>
					<%Else
						conta_linhas = conta_linhas + 1%>					
						<tr bgcolor="ffffff" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#ffffff');" valign="middle"><td colspan="8" style="font-size:8px; height:20px; border-top: 1px solid black; border-bottom: 1px solid black;"><!-- salto de linha para ajuste do rodap� -->&nbsp;<%'=conta_linhas%> <b>Total parcial de procedimentos:</b> <%if cd_unidade = 20 or cd_unidade = 14 then%><%=procedimentos%><%else%><%=procedimentos%><%end if%></td></tr>
					<%end if%>
				...
				if cd_unidade = 20 AND rack_numerar = rack_num AND int(procedimentos) = int(rack_total) or cd_unidade = 14  then
					...
					next%>
						...
						<%end if
					conta_linhas = conta_linhas + 1%>
					<tr bgcolor="#ffffff" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#ffffff');" style="font-size:8px; height:20px; border: 1px solid black;" valign="middle"><td colspan="8"><%'=conta_linhas%><!-- salto de linha para ajuste do rodap� -->&nbsp;<b>TOTAL DE PROCEDIMENTOS REALIZADOS: <%=procedimentos%></b></td></tr>					
				<%end if%>
				<%'**************************
				...
				if proc_co = "" AND emprestimos = "" AND procedimentos = linha_numerar then
					reg_num = reg_num + 3%>
					<tr style="height:20px;"><td colspan="8" valign="bottom"><img src="../imagens/blackdot.gif" alt="" width="100%" height="1" border="0"></td></tr>
					<tr bgcolor="#dadada" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#dadada');" style="font-size:8px; height:20px;" valign="middle"><td colspan="8"><!-- salto de linha para ajuste do rodap� -->&nbsp;<b>TOTAL DE PROCEDIMENTOS REALIZADOS: <%=procedimentos%></b></td></tr>
					<tr style="height:20px;"><td colspan="8" valign="top"><img src="../imagens/blackdot.gif" alt="" width="100%" height="1" border="0"></td></tr>
				<%end if%>
			<%while conta_linhas + 1 <= qtd_linhas_pag%>
				<tr style="font-size:8px; height:20px;" valign="middle"><td colspan="8"><%'=conta_linhas + 1%><!-- salto de linha para ajuste do rodap� -->&nbsp;<%'=reg_num&"__ "&qtd_linhas_pag%></td></tr>
				<%conta_linhas = conta_linhas + 1 
					
					
			wend
			'****************************************
			'*** RODAP� DOS PROCEDIMENTOS NORMAIS ***
			'****************************************%>
		...
		<%'Numera��o e mudan�a das cores das linhas
...
<!--**********************************************************************************************************************************************************-->				
	...
	<%if mostra_cabecalho = 0 then%>
		...
		<tr style="font-size:11px;">
			<td style="border:1px; solid red;" colspan="100%" align="center" >
				<b>RELATORIO DE PROCEDIMENTOS REALIZADOS NO C.O.</b> <br>
				<b><%=nm_unidade%></b><br>
	...
	<%end if%>
		...
		'if reg_num_co = qtd_linhas_pag OR rack_numerar_co = rack_num_co OR proc_co = reg_num_co then 'Salto de p�gina caso alcance 46 registros ou se o alcan�ar a quantidade de registros por rack
		if conta_linhas = qtd_linhas_pag OR proc_co = linha_numerar OR rack_numerar = rack_num then	
			...
			'**********************************
				if rack_numerar = rack_num OR proc_co = linha_numerar then%>
						...
						<%end if
					conta_linhas = conta_linhas +1%>
					<tr bgcolor="ffffff" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#ffffff');"  valign="middle"><td colspan="8" style="font-size:8px; height:20px; border-bottom: 1px solid black; border-top: 1px solid black;"><%'=conta_linhas%><!-- salto de linha para ajuste do rodap� -->&nbsp;<b>Total parcial de procedimentos realizados no CO:</b>  <%=proc_co%></td></tr>
				<%end if%>
				...
				total_procedimentos = total_procedimentos + proc_co
					conta_linhas = conta_linhas + 1%>
					<tr><td>&nbsp;<%'=conta_linhas%></td></tr>
					<%conta_linhas = conta_linhas +1%><tr bgcolor="#dadada" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#dadada');" valign="middle"><td colspan="8" style="font-size:8px; height:20px; border-top: 1px solid black;"><%'=conta_linhas%><!-- salto de linha para ajuste do rodap� -->&nbsp;<b>Total parcial de procedimentos: <%=procedimentos%></b></td></tr>
					<%conta_linhas = conta_linhas +1%><tr bgcolor="#dadada" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#dadada');" valign="middle"><td colspan="8" style="font-size:8px; height:20px;"><%'=conta_linhas%><!-- salto de linha para ajuste do rodap� -->&nbsp;<b>Total parcial de procedimentos no CO: <%=proc_co%></b></td></tr>
					<%conta_linhas = conta_linhas +1%><tr bgcolor="#dadada" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#dadada');" valign="middle"><td colspan="8" style="font-size:8px; height:20px;border-bottom: 1px solid black;"><%'=conta_linhas%><!-- salto de linha para ajuste do rodap� -->&nbsp;<b>TOTAL DE PROCEDIMENTOS REALIZADOS: <%=total_procedimentos%></b></td></tr>
					
				<%end if%>
			<%while conta_linhas + 1  <= qtd_linhas_pag
			conta_linhas = conta_linhas + 1%>
				<tr style="font-size:8px; height:20px;" valign="middle"><td colspan="8"><%'=conta_linhas%><!-- salto de linha para ajuste do rodap� -->&nbsp;<%'=reg_num&"__ "&qtd_linhas_pag%></td></tr>
			<%wend
			'***********************************************************************%>
...
<%'******************************************************************************************************
'********************************************************************************************************
	'C.1 - Mostra os procedimentos - Empr�stimos
'********************************************************************************************************
'********************************************************************************************************
	...
	for i_e = 1 to emprestimos
		...
		<%mostra_registro = 0%>		
			...
			<tr style="border:0px; solid black; font-size:11px;">
				...
				<td align="right"><b>Reg</b>&nbsp;</td>
				<td align="left">&nbsp;<b>Cirurgia</b></td>
				<td align="left" colspan=2><b>Equipe M�dica</b></td>
			...
			'**********************************
				if linha_numerar_e = emprestimos then
					...
					<tr bgcolor="dadada" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'#dadada');" style="font-size:8px; height:20px;" valign="middle">
						<td colspan="4" style="font-size:8px; height:20px; border-bottom: 1px solid black; border-top: 1px solid black;"><%'=conta_linhas%><!-- salto de linha para ajuste do rodap� -->&nbsp;<b>TOTAL DE PROCEDIMENTOS DE EMPRESTIMO:</b> <%=rack_num_e%><%=emprestimos%></td>
						<td align="right" style="border-bottom: 1px solid black; border-top: 1px solid black;"><!-- salto de linha para ajuste do rodap� -->&nbsp;<b>TOTAL DE ITENS EMPRESTADOS:</b> <%=rack_num_e%></td>
			...
			if conta_linhas = qtd_linhas_pag then
				...
				conta_linhas = conta_linhas + 1%>
					<tr style="font-size:8px; height:20px;" valign="middle">
						<td colspan="8"><%'=conta_linhas%><!-- salto de linha para ajuste do rodap� -->&nbsp;<%'=reg_num&"__ "&qtd_linhas_pag%></td></tr>
				<%wend%>
			
			<tr id="print_ok"><td colspan="8"><img src="../imagens/blackdot.gif" alt="" width="100%" height="1" border="0"></td></tr>
			<tr style="font-size:10px;" >
				<td colspan="2">&nbsp;</td>
				<td colspan="5" style="page-break-after:always;" align="center">Data de emiss�o: Z <b><%=zero(day(now))&"/"&mesdoano(month(now))&"/"&right(year(now),2)&" "&hour(now)&":"&minute(now)%></b></td>
				<td align="right">&nbsp;P�g.:<%=num_pag%>&nbsp;&nbsp;&nbsp;</td>
			</tr>
			<%mostra_cabecalho = 0
			conta_linhas = 0
	...
	next%>
		...
		conta_linhas = conta_linhas + 1
			while conta_linhas <= qtd_linhas_pag%>
				<tr bgcolor="ffffff" onmouseover="mOvr(this,'#ffff99');"  onclick="javascript:;" onmouseout="mOut(this,'ffffff');" style="font-size:8px; height:20px;" valign="middle">
					<td colspan="8"><%'=conta_linhas%><!-- salto de linha para ajuste do rodap� -->&nbsp;<%'=reg_num_e&"__ "&qtd_linhas_pag&"=="&emprestimos &"__ "& linha_numerar_e%></td></tr>
				<%reg_num_e = reg_num_e +1
				conta_linhas = conta_linhas +1
			wend
		'***********************************************************************
		...
		'if reg_num_e = qtd_linhas_pag OR emprestimos = reg_num_e then 'Salto de p�gina caso alcance 46 registros ou se o alcan�ar a quantidade de registros por rack
			reg_num_co = 0	'Reinicia a contagem de registros por p�gina.
			...
			<tr style="font-size:10px;">
				<td colspan="2">&nbsp;</td>
				<td colspan="5" style="page-break-after:always;" align="center">Data de emiss�o: Y <b><%=zero(day(now))&"/"&mesdoano(month(now))&"/"&right(year(now),2)&" "&hour(now)&":"&minute(now)%></b></td>
				<td align="right">&nbsp;P�g.:<%=num_pag%>&nbsp;&nbsp;&nbsp;</td>
			</tr>
		<%num_pag = num_pag + 1
...
*********************************************************************************************************************************************************-->
	...
	<%if nao_mostra_ultimo_rodape = "" Then%>
		...
		<tr style="font-size:10px;">
			...
			<td colspan="5" align="center">Data de emiss�o: X <b><%=zero(day(now))&"/"&mesdoano(month(now))&"/"&right(year(now),2)&" "&hour(now)&":"&minute(now)%></b></td>
			<td align="right">&nbsp;P�g.:<%=num_pag%>&nbsp;&nbsp;&nbsp;</td>
		</tr>
	<%end if%>
<%'**************************************************************************************************
