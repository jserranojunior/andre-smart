...
<table class="txt" border="0" width="600" align="center" style="border-collapse:collapse;>	
	...
	<%if i = 1 then%>
		...
		<%strsql ="SELECT top 1 * FROM TBL_patrimonio_manutencoes WHERE cd_patrimonio="&cd_pat_codigo&" and cd_suspenso <> 1 AND tipo_manut = "&i&" AND (dt_plan_prev <= '12/31/"&ano_sel&"') order by dt_plan_prev desc"
			Set rs_prev = dbconn.execute(strsql)
				...
				if int(ano_sel) = dt_plan_prev_ano then
					'mostra a preventiva e soma o mes ao periodo de cada prev. e mostra se o período for menor que dezembro
				elseif int(ano_sel) > dt_plan_prev_mes then
					'soma o mes ao período de cada prev., caso o resultado for maior que 12 então soma 1 ao ano e ...
						
						prox_mes_prev = dt_plan_prev_mes + dt_periodicidade_prev
						somar_ao_ano = int(prox_mes_prev/12)
							
							if int(somar_ao_ano) >= int(1) Then
								prox_ano_prev = dt_plan_prev_ano + somar_ao_ano
								prox_mes_prev = dt_plan_prev_mes - dt_periodicidade_prev
							else
								prox_ano_prev = dt_plan_prev_ano' + somar_ao_ano
								prox_mes_prev = dt_plan_prev_mes' - dt_periodicidade_prev
							...
							'prox_mes_prev = prox_mes_prev - dt_periodicidade_prev
				end if
		...
		for imes = 1 to 12
			...
			'		if int(ano_sel) = year(dt_plan_prev) then
			'			if int(dt_plan_prev_mes) = imes then
			...
			'			end if
			'		
			'		elseif int(ano_sel) > year(dt_plan_prev) then
			'			marca = "X"&ano_sel&"-"&year(dt_plan_prev)
			'		else 
			'			marca = "<a href=javascript:viod(0); onclick=window.open('patrimonio/patrimonio_cad_manutencoes.asp?tipo=cadastro&cd_plan_prev="&cd_plan_prev&"','nome','width=400,height=250'); return false;><img src='../imagens/x0.gif' height=15 border='0'></a>"
			'		end if%>					
				<%'end if%>
		<td style="<%=estilo_corpo%>"><%=prox_mes_prev&"/"&prox_ano_prev&"("&somar_ao_ano%></td>
		<%marca = ""
		next%>
