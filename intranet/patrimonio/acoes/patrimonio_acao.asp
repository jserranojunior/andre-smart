...
unid_ori = request("unid_ori")
unid_des = request("unid_des")
rack_des = request("rack_des")

dia_mov = request("dia_mov")
mes_mov = request("mes_mov")
ano_mov = request("ano_mov")
dt_movimento = ano_mov&"-"&mes_mov&"-"&dia_mov

tipo_mov = request("tipo_mov")
id_movimento = request("id_movimento")
...
'******************************		
	ElseIf acao = "editar" AND cd_tipo = "patrimonio" Then
		...
		xsql = "up_patrimonio_update2 @cd_pat_codigo="&cd_pat_codigo&", @cd_equipamento='"&cd_equipamento&"', @cd_marca='"&cd_marca&"', @cd_pn='"&cd_pn&"', @cd_ns='"&cd_ns&"', @no_patrimonio='"&no_patrimonio&"', @nm_tipo='"&nm_tipo&"', @cd_especialidade='"&cd_especialidade&"', @cd_unidade='"&cd_unidade&"', @cd_rack='"&cd_rack&"', @cd_unidade_comodato='"&cd_unidade_comodato&"', @cd_rack_comodato='"&cd_rack_comodato&"', @cd_estado='"&cd_estado&"', @cd_categoria='"&cd_categoria&"' ,@dt_data='"&dt_data&"', @cd_nf='"&cd_nf&"',@cd_preventiva='"&cd_preventiva&"', @dt_periodicidade_prev="&dt_periodicidade_prev&", @cd_fornecedor_prev="&cd_fornecedor_prev&", @cd_calibracao="&cd_calibracao&", @dt_periodicidade_cal="&dt_periodicidade_cal&", @cd_fornecedor_cal="&cd_fornecedor_cal&", @cd_seg_elet="&cd_seg_elet&", @dt_periodicidade_elet="&dt_periodicidade_elet&", @cd_fornecedor_elet="&cd_fornecedor_elet&", @nao_constar="&nao_constar&""
		dbconn.execute(xsql)
		'response.write xsql
			...
			set rs = dbconn.execute(strsql)
				if not rs.EOF then
					...
					if existe_mov < 1 then '*** Grava o primeiro movimento = Empresa vinculada à unidade ***
						...
						id_movimento = 1 '*** indica o primeiro movimento do recém cadastrado patrimônio ***
						xsql = "up_patrimonio_movimentacao_insert @cd_patrimonio='"&cd_pat_codigo&"', @cd_unidade_origem='"&cd_empresa&"', @cd_unidade_destino='"&cd_empresa&"', @cd_rack_origem='"&cd_rack_origem&"', @cd_tipo_movimentacao=3, @dt_ida='"&dt_data&"', @dt_retorno='"&dt_data&"', @id_movimento="&id_movimento&", @cd_user="&cd_user&", @dt_cadastro='"&dt_hoje&"'"
						response.write("up_patrimonio_movimentacao_insert @cd_patrimonio='"&cd_pat_codigo&"', @cd_unidade_origem='"&cd_empresa&"', @cd_unidade_destino='"&cd_empresa&"', @cd_rack_origem='"&cd_rack_origem&"', @cd_tipo_movimentacao=3, @dt_ida='"&dt_data&"', @dt_retorno='"&dt_data&"', @id_movimento="&id_movimento&", @cd_user="&cd_user&", @dt_cadastro='"&dt_hoje&"'")
						'dbconn.execute(xsql)
					end if
			
			'*** Verifica se há solicitação de movimentação 
			if unid_des <> "" OR rack_des <> "" Then
				id_movimento = id_movimento + 1
				'response.write("unidade ou rack ok")
				xsql = "up_patrimonio_movimentacao_insert @cd_patrimonio='"&cd_pat_codigo&"', @cd_unidade_origem='"&unid_ori&"', @cd_rack_origem='"&rack_ori&"', @cd_unidade_destino='"&unid_des&"', @cd_rack_destino='"&rack_des&"', @cd_tipo_movimentacao="&tipo_mov&", @dt_ida='"&dt_movimento&"', @dt_retorno=null, @id_movimento='"&id_movimento&"', @cd_user="&cd_user&",@dt_cadastro='"&dt_hoje&"'"
				'response.write("<br><br>xsql = up_patrimonio_movimentacao_insert <br>@cd_patrimonio='"&cd_pat_codigo&"', <br>@cd_unidade_origem='"&unid_ori&"', <br>@cd_rack_origem='"&rack_ori&"', <br>@cd_unidade_destino='"&unid_des&"', <br>@cd_rack_destino='"&rack_des&"', <br>@cd_tipo_movimentacao="&tipo_mov&", <br>@dt_ida='"&dt_descarte&"', <br>@dt_retorno=null, <br>@id_movimento='"&ultimo_mov&"', <br>@cd_user="&cd_user&",<br>@dt_cadastro='"&dt_hoje&"'<br><br>")
				dbconn.execute(xsql)
				
			else
				response.write("nada a movimentar")
			end if
