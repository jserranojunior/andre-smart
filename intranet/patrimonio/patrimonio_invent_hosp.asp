...
<table class="txt" width="600" align="center" style="border-collapse:collapse; border:0px solid white;">
	<%num_lista = 0
	conta_registro = 1
	if rel_mod = 2 AND strcd_unidade = 2 or rel_mod = 2 AND strcd_unidade = 3 or rel_mod = 2 AND strcd_unidade = 15 or rel_mod = 2 AND strcd_unidade = 38 then
		qtd_linhas = 37
	else
		qtd_linhas = 40
	end if
				...
				ordem = " "&ordem&" "
			'	response.write strcd_unidade
				if strcd_unidade = 29 then
					strsql = "SELECT * FROM VIEW_patrimonio_lista WHERE cd_unidade_comodato = "&strcd_unidade&" AND nm_tipo = 'E' AND nao_constar between 1 and 2  AND dt_descarte > '"&dt_hoje&"' OR cd_unidade_comodato = "&strcd_unidade&" AND nm_tipo = 'E' AND nao_constar between 1 and 2  AND dt_descarte is null order by cd_posicao, nm_equipamento_novo"
				 elseif strcd_unidade = 33 then
					strsql = "SELECT * FROM VIEW_patrimonio_lista WHERE cd_unidade_comodato = "&strcd_unidade&" AND nao_constar between 1 and 2  AND dt_descarte > '"&dt_hoje&"' OR cd_unidade_comodato = "&strcd_unidade&" AND nao_constar between 1 and 2  AND dt_descarte is null order by cd_posicao, nm_equipamento_novo"
				 else
				' strsql = "SELECT * FROM VIEW_patrimonio_lista WHERE cd_unidade_comodato = "&strcd_unidade&" AND nm_tipo = 'E' AND nao_constar <> 2 AND dt_descarte > '"&dt_hoje&"' OR cd_unidade_comodato = "&strcd_unidade&" AND nm_tipo = 'E' AND nao_constar <> 2 AND dt_descarte is null order by cd_posicao, nm_equipamento_novo"
				 	 strsql = "SELECT * FROM VIEW_patrimonio_lista WHERE cd_unidade_comodato = "&strcd_unidade&" AND  nm_tipo = 'E' AND nao_constar = 1 AND dt_descarte > '"&dt_hoje&"' OR cd_unidade_comodato = "&strcd_unidade&"  AND nm_tipo = 'E' AND nao_constar = 1 AND dt_descarte is null order by cd_posicao, nm_equipamento_novo"
			  end if
	...
	<%if rel_mod = 2 AND cd_unidade = 2 or rel_mod = 2 AND cd_unidade = 3 or rel_mod = 2 AND cd_unidade = 15 or rel_mod = 2 AND strcd_unidade = 38 then
		colspan_tit = "6"
		colspan_rod = "9"
	else
		colspan_tit = "5"
	...
	<tr>
		...
		<%else%>
			...
			<img src="../imagens/px.gif" alt="" width="88" height="43" border="0">
		<%end if%></td>			
	</tr>
	...
	<%if rel_mod = 2 AND cd_unidade = 2 or rel_mod = 2 AND cd_unidade = 3 or rel_mod = 2 AND cd_unidade = 15 or rel_mod = 2 AND cd_unidade = 38 then%>
	...
	<%if rel_mod = 2 AND cd_unidade = 2 or rel_mod = 2 AND cd_unidade = 3 or rel_mod = 2 AND cd_unidade = 15  or rel_mod = 2AND strcd_unidade = 38 then%>
	...
	<tr>
		<td>&nbsp;</td>
		<td colspan="5"><b style="font-family:arial; font-size:10px;"><!--ITA20211.PQ.006-->
		ANF20211.PQ003 - ANEXO 2 - INVENTÁRIO DE EQUIPAMENTOS MÉDICOS - Vd Lap</b></td>		
	</tr>
	<%elseif cd_unidade = 38 then%>
	<%elseif cd_unidade = 3 then%>
	<tr>
		<td>&nbsp;</td>
		<td colspan="5"><b style="font-family:arial; font-size:10px;"><!--ITA20211.PQ.006-->
		MOR20211.PQ008 - ANEXO 2 - INVENTÁRIO DE EQUIPAMENTOS MÉDICOS - Vd Lap</b></td>		
	</tr>
	<%end if%>
	<tr><td>&nbsp;</td></tr>
	<tr>		
