...
<br class="no_print">	
<br class="no_print">
<table align="center" class="no_print" style="border:1 solid orange; page-break-after:always;">
...
<table class="txt" border="0" width="600" align="center" cellpadding="0" cellspacing="0" bgcolor="#C0C0C0" >
	...
	'rack_anterior = 0
				...
				strsql = "SELECT * FROM VIEW_patrimonio_lista WHERE cd_unidade_comodato = "&strcd_unidade&" AND nm_tipo = 'E' AND dt_descarte > '"&dt_hoje&"' OR cd_unidade_comodato = "&strcd_unidade&" AND nm_tipo = 'E' AND dt_descarte IS NULL order by nm_rack_comodato, cd_posicao, nm_equipamento_novo"
	...
	<%if saltar_pg = 1 then
		...
		<%'wend%>
		<tr>
			<td colspan="6" align="right"><p id="quebrapagina"><%=zero(day(now))&"/"&zero(month(now))&"/"&year(now)%></b><%'=linha&"-"&qtd_linhas_pg%><%'=linhas_dif%></td>
		</tr>
	...
	estilo_corpo = "border:1px solid gray; border-collapse:collapse; font-size:12px;"%>
		<tr style="border:0px solid gray; border-collapse:collapse; font-size:14px; color:black;">			
			<td colspan="4" align="center" valign="middle" style="font-size:10 px;">
	...
	if rack_anterior = "" OR rack_anterior <> cd_rack  Then%>				
		...
		linha_bloc = 0%>
	<tr><td>&nbsp;<%'linha=linha-1%><%'=linha%><br><img src="../imagens/px.gif" alt="" width="1" height="1" border="0"><%'linha=linha-1%><%'=linha%></td></tr>
	<tr bgcolor="#969696">
		...
		<!--"td align="center" style="<%=estilo_titulo%>"><b>Estado</b></td--->
	</tr>
	<tr>
		<td style="<%=estilo_titulo%>"><img src="../imagens/px.gif" alt="" width="1" height="1" border="0"><%'linha=linha-1%><%'=linha%></td>
		<td style="<%=estilo_titulo%>"><img src="../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
		<td style="<%=estilo_titulo%>"><img src="../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
		<td style="<%=estilo_titulo%>"><img src="../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
		<td style="<%=estilo_titulo%>"><img src="../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
		<td align="center" style="<%=estilo_titulo%>;font-size:9px;" bgcolor="#969696"><b>VD LAP</b></td>
		<!--td style="<%=estilo_titulo%>"><img src="../imagens/px.gif" alt="" width="1" height="1" border="0"></td-->
	</tr>	
	<%end if%>
	
	<%
	'strsql = "SELECT * FROM View_patrimonio_emprestimos where cd_situacao >= 4 and cd_patrimonio = '"&cd_pat_codigo&"' and dt_emprestimo <= '"&day(now)&"' and dt_retorno is NULL OR cd_situacao >= 4 and cd_patrimonio = '"&cd_pat_codigo&"' and dt_emprestimo <= '"&day(now)&"' and dt_retorno >= '"&day(now)&"'"
	strsql = "SELECT * FROM View_patrimonio_emprestimos where cd_situacao >= 4 and cd_patrimonio = '"&cd_pat_codigo&"' and dt_emprestimo <= '"&dt_hoje&"' and dt_retorno is NULL OR cd_situacao >= 4 and cd_patrimonio = '"&cd_pat_codigo&"' and dt_emprestimo <= '"&dt_hoje&"' and dt_retorno >= '"&dt_hoje&"'"
		...
		'if cd_emprestimo = "" Then%>
			<tr onmouseover="mOvr(this,'#CFC8FF');"   onmouseout="mOut(this,'');" style="border:2px solid black;">		
				...
				<!--td valign="top" align="center" onmouseover="mOvr(this,'#CFC8FF');"  onmouseout="mOut(this,'');" style="<%=estilo_corpo%>"><%=num_hospital%>---->
					<%
					strsql = "SELECT [nm_estado] FROM [dbo].[TBL_patrimonio_estado] where cd_codigo = "&cd_estado&""
				    Set rs_estado = dbconn.execute(strsql)
					if not rs_estado.eof then
