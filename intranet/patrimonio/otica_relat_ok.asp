...
<table class="txt" border="0" width="600" align="center" cellpadding="0" cellspacing="0" bgcolor="#C0C0C0" >
	...
	'rack_anterior = 0
				...
				'strsql = "SELECT count* FROM VIEW_patrimonio_lista WHERE cd_unidade = "&strcd_unidade&" AND nm_tipo = 'O' order by cd_especialidade,cd_posicao,nm_equipamento"
						...
						'	
							For Each espec_item_junto In espec_array
								...
								espec_reg = mid(espec_item_junto,instr(espec_item_junto,",")+1,len(espec_item_junto))
									total_qtd_reg = total_qtd_reg + int(qtd_reg)
								response.write("<tr><td>"&total_qtd_reg&"<br></td></tr>")
								
							next
							
							if abs(total_qtd_reg+3) <= qtd_linhas_pg then
								response.write("Total é menor ou igual que a qtd de linhas pré-estabelecidas")
							else
				...
				strsql = "SELECT * FROM VIEW_patrimonio_lista WHERE cd_unidade = "&strcd_unidade&" AND nm_tipo = 'O' order by cd_especialidade,cd_posicao,nm_equipamento"
	...
	if especialidade_anterior = "" OR especialidade_anterior <> nm_especialidade  Then%>				
