...
<body>
	<br class="no_print">
<table align="center"  class="no_print" width="400" style="border:1 solid orange;">
	<%if session("cd_codigo") = 46 then%>
		<tr><td colspan="100" align="center"><span style="font-size=8px;color:red;">otica_relat.asp</span></td></tr>
	<%end if%>
	...
	<tr>		
		...
		<td><a href="javascript:print();void(0);" ><img src="../imagens/ic_print.gif" alt="" width="24" height="24" border="0" ></A>&nbsp;&nbsp;<a href="javascript:visualizarImpressao();void(0);" ><img src="../imagens/ic_print_view.gif" alt="" width="24" height="26" border="0" onclick="visualizarImpressao();"></a>
...
<table class="txt" border="0" width="500" align="center" cellpadding="0" cellspacing="0" bgcolor="#C0C0C0" >
	<tr class="ok_print"><td rowspan="1000%" class="ok_print"><img src="../imagens/px.gif" alt="" width="150" height="1" border="0"></td></tr>
	...
	estilo_acerto = "border:1px solid white; border-collapse:collapse; font-size:12px;"
				...
				strsql = "SELECT     COUNT(cd_especialidade) AS qtd_reg, cd_especialidade FROM VIEW_patrimonio_lista WHERE cd_unidade_comodato = "&strcd_unidade&" AND nm_tipo = 'O' AND cd_status = "&str_status&" GROUP BY cd_especialidade, cd_especialidade ORDER BY cd_especialidade"
				...
				strsql = "SELECT * FROM VIEW_patrimonio_lista WHERE cd_unidade_comodato = "&strcd_unidade&" AND nm_tipo = 'O' AND cd_especialidade = "&espec_reg&" AND cd_status = "&str_status&" order by cd_especialidade,cd_posicao,nm_equipamento"
				...
				cd_unidade_comodato = rs_patrimonio("cd_unidade_comodato")
					strsql = "SELECT * FROM TBL_unidades where cd_codigo = "&strcd_unidade&" AND cd_status <> 0"
					Set rs_uni = dbconn.execute(strsql)
					while not rs_uni.EOF
						nm_unidade = rs_uni("nm_unidade")
						nm_unidade_comodato = rs_uni("nm_unidade")
					rs_uni.movenext
					wend
				...
				ordem = " "&ordem&" "
					...
					strsql_rack ="SELECT * FROM TBL_rack Where a056_codrac='"&cd_rack&"' "
				  	Set rs_rack = dbconn.execute(strsql_rack)
						...
						'*********************************************
						strsql = "SELECT COUNT (no_patrimonio)as conta FROM VIEW_patrimonio_lista WHERE cd_rack = '"&cd_rack&"' and cd_unidade_comodato = "&cd_unidade&""
							 Set rs_conta = dbconn.execute(strsql)
						if not rs_conta.EOF Then
							conta_patr = rs_conta("conta")
								...
								if int(saltar_pg) = int(1) then
									...
									<%wend%>
									<tr>
										<td colspan="7" style="page-break-after:always;" align="right"><%=zero(day(now))&"/"&zero(month(now))&"/"&year(now)%><%'=linha&"-"&qtd_linhas_pg%><%'=linhas_dif%></td>
									</tr>									
									<%linha = 0
								end if%>
								...
								'estilo_titulo = "border:1px solid gray; border-collapse:collapse; font-size:12px; color:white;"
								'estilo_corpo = "border:1px solid gray; border-collapse:collapse; font-size:12px;"%>
									<tr><td><img src="../imagens/px.gif" alt="" width="1" height="20" border="0"></td></tr>
									<tr style="border:0px solid gray; border-collapse:collapse; font-size:14px; color:black;">			
										<td colspan="5" align="center" valign="middle">
													<b>HOSPITAL - <%=nm_unidade_comodato%> <br>
								...
								<tr><td colspan="100" style="<%=estilo_acerto%>">&nbsp;</td></tr>
								<tr style="background-color:#C0C0C0;"><td colspan="7">&nbsp;Total de óticas: <%=total_oticas%></td></tr>
									<%linha = linha + 2%>
								<%while not linha = qtd_total_pag 'Acerta as linhas até o fim
										linha = linha + 1%>
										<tr><td colspan="100" style="<%=estilo_acerto%>">&nbsp;<%'=linha%></td></tr>
	...
	<tr>
		...
		<!--td class="ok_print"><img src="../imagens/blackdot.gif" alt="" width="90" height="3" border="0"></td-->	
