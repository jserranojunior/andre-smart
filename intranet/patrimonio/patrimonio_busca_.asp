...
<body>
<br class="no_print">
<br class="no_print"><!-- LISTAGEM DO PATRIMONIO -->
<table align="center" class="no_print">
	...
	<%end if%>
	<tr><td colspan="100%"><%if usuario = 4 OR usuario = 46 then%><a href="javascript:void(0);" onclick="window.open('patrimonio/patrimonio_cad.asp?jan=1&caminho=patrimonio','nome','width=700,height=500,scrollbars=1'); return false;">:: Novo ::</a><%else%>&nbsp;<%end if%></td></tr>
...
<table class="txt" border="0" width="10" align="center">
	<%if usuario = "46" then%>
		<tr class="no_print"><td colspan="10"><%="<b>SELECT</b> cd_codigo,"&campos&" <br><b>FROM</b> View_patrimonio_lista "&condicao&" <br>"&ordem&""%></td></tr>
	<%end if%>
	<tr class="no_print">
	...
	cabeca = 0
		...
		'*** CONDIÇÕES ***
			if tipo_patr <> "" OR  patrimonio <> "" OR equipamento <> "" OR modelo <> "" OR nserie <> "" OR marca > 0  OR especialidade > 0 OR codhosp <> "" OR rack > 0 OR unidade > 0 OR dia_compra_i <> "" OR dia_compra_f <> ""  OR mes_compra_i <> ""  OR mes_compra_f <> "" OR nao_constar >= 1 OR cd_status >= 1 Then
				condicao = "WHERE "&tipo_cond&" "& patr_cond&" "&equip_cond&" "&model_cond&" "&nserie_cond&" "&marca_cond&" "&espec_cond&" "&codhosp_cond&" "&rack_cond&" "&unidade_cond&" "&compra_cond&" "&constar_cond&" "&status_cond&""'dt_descarte is null"
			end if		
	...
	<%if cabeca = 0 then%>
	<tr>
		<td colspan="100%" align="center" style="background-color:gray; color:white; font-size:15px; font-weight:bold;">RELAÇÃO PATRIMONIAL - RESULTADO DE BUSCA</td>
	</tr>
	<tr bgcolor="#C0C0C0">
			<td>&nbsp;Nº</td>
			<%if campo_0 <> 0 Then%><td><b>&nbsp;PAT</b></td><%end if%>
			<%if campo_1 <> 0 Then%><td><b>&nbsp;NOME</b></td><%end if%>
			<%if campo_2 <> 0 Then%><td><b>&nbsp;MODELO</b></td><%end if%>
			<%if campo_3 <> 0 Then%><td><b>&nbsp;Nº SÉRIE</b></td><%end if%>
			<%if campo_4 <> 0 Then%><td><b>&nbsp;MARCA</b></td><%end if%>
			<%if campo_5 <> 0 Then%><td><b>&nbsp;ESPECIALIDADE</b></td><%end if%>
			<%if campo_6 <> 0 Then%><td><b>&nbsp;Nº HOSPITAL</b></td><%end if%>
			<%if campo_7 <> 0 Then%><td><b>&nbsp;RACK</b></td><%end if%>
			<%if campo_8 <> 0 Then%><td><b>&nbsp;UNIDADE</b></td><%end if%>
			<%if campo_9 <> 0 Then%><td><b>&nbsp;COMPRA</b></td><%end if%>		
		</tr>
	<%end if%>
	<tr onmouseover="mOvr(this,'#CFC8FF');"   onmouseout="mOut(this,'');" onclick="window.open('patrimonio/<%=abre_pag%>.asp?tipo=cadastro&cd_pat_codigo=<%=cd_pat_codigo%>&acao=alterar&jan=1&caminho=patrimonio','Alterações','width=705,height=<%=tamanho_janela%>,scrollbars=1'); return false;">
		<td align="center" valign="top" bgcolor="#C0C0C0">&nbsp;<b><%=zero(num_lista)%></b></td>
			...
			<%if campo_5 <> 0 Then%>
	   		...
	   		<%end if%>
			<%if campo_6 <> 0 Then%>
				<td valign="top" style="<%=stilo_linha%>">&nbsp;<%=num_hospital%></td>
	   		<%end if%>
			<%if campo_7 <> 0 Then%>
	...
	<%if cabeca = 39 then%><tr style="page-break-after:always;"><td colspan="100%">&nbsp;</td></tr><%end if%>
				<%nm_especialidade = ""
				cabeca = cabeca + 1
				if cabeca = 40 then
					cabeca = 0 
