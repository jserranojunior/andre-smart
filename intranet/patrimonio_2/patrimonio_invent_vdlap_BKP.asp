...
cd_nao_constar = request("cd_nao_constar")
	if int(cd_nao_constar) = 1 then
		if itens <> "" Then
			var_constar = " AND "
		else
			var_constar = " Where "
		end if
		strcd_constar = var_constar&" nao_constar <> 1 "
	end if
%>
<head>
...
<table align="center" id="no_print" style="border:1 solid blue;">
	...
	<tr>
		...
		<td valign="top"><input type="checkbox" name="cd_nao_constar" value="1" <%if cd_nao_constar = 1 then%>Checked<%end if%>> Esconder os "não constar" <br>(cabeçote, carrinho etc)</td>
	</tr>
	<tr>		
...
<table class="txt" border="0" width="600" align="center" style="border-collapse:collapse; border:0px solid red;">	
	...
	strsql="SELECT * FROM View_patrimonio_emprestimos WHERE (cd_situacao = 2) AND (dt_emprestimo <= '"&data_agora&"') AND (dt_retorno IS NULL) OR (cd_situacao = 2) AND (dt_emprestimo <= '"&data_agora&"') AND (dt_retorno >= '"&data_agora&"')"
		...
		if cod_emprestimos <> "" then%>	
			...
			<%end if%>
					<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'');">
						...
						<td valign="top" align="center" style="<%=estilo_corpo%>">&nbsp;<%="0"&no_patrimonio%></td>		
						<td valign="top" align= "left"  style="<%=estilo_corpo%>">&nbsp;<a href="javascript:viod(0);" onclick="window.open('patrimonio/patrimonio_cad.asp?tipo=cadastro&cd_pat_codigo=<%=str_pat_codigo%>&acao=alterar','Alterações','scrollbars=1,width=700,height=600'); return false;"><%=nm_equipamento%></a></td>
						<td valign="top" align="center" style="<%=estilo_corpo%>">&nbsp;<%=nm_marca%></td>
						<td valign="top" align="center" style="<%=estilo_corpo%>">&nbsp;<%=cd_pn%></td>
						<td valign="top" align="center" style="<%=estilo_corpo%>">&nbsp;<%=cd_ns%></td>
	...
	<tr>		
		<td>&nbsp;</td>
		<td align="center" colspan="8">&nbsp;<!--Pág.:<%'=zero(pagina+1)%>--><b id="no_print"><%=nm_unidade%></b></td>
		<td align="center"><b><%=zero(day(now))&"/"&zero(month(now))&"/"&year(now)%></b></td>
	</tr>
	<tr>
