...
patrimonio = request("patrimonio")
	if patrimonio <> "" then
	 	patr_cond = prox_cond&"cd_patrimonio = '"&patrimonio&"'"
		prox_cond = " AND "
	end if
...
<%end if%>
	<tr onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'');">
		<td><input type="checkbox" name="campos" value="nm_tipo,cd_patrimonio" <%if campo_0 <> 0 Then response.write("CHECKED") end if%>>Patrimonio:</td>		
		<td><input type="text" name="patrimonio" size="20" maxlength="20" value="<%=patrimonio%>" style="background-color:white;"></td>
		<td>
			<img src="../imagens/blackdot.gif" alt="Incluí" width="15" height="15" border="0" onClick="ordem('nm_tipo,cd_patrimonio','',document.form.ordem_total.value)">
			<img src="../imagens/blackdot.gif" alt="exclui da ordem" width="15" height="15" border="0" onClick="ordem('','nm_tipo,cd_patrimonio',document.form.ordem_total.value)">
...
<tr onmouseover="mOvr(this,'#CFC8FF');"   onmouseout="mOut(this,'');" onclick="window.open('patrimonio/patrimonio_cad.asp?tipo=cadastro&cd_pat_codigo=<%=cd_pat_codigo%>&acao=alterar','Alterações','width=700','height=600'); return false;">
	<td align="center" valign="top" bgcolor="#C0C0C0">&nbsp;<b><%=zero(num_lista)%></b></td>
		<%if campo_0 <> 0 Then%>
			<td valign="top">&nbsp;<%=nm_tipo%><%=cd_patrimonio%></td>
   		<%end if%>
		<%if campo_1 <> 0 Then%>
