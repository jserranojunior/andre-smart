...
<table align="center" border="0">
		...
		<td>
			<table width="650" border="0" cellspacing="2" cellpadding="0" bordercolordark="#FFFFFF" bordercolorlight="#efefef">
				...
				<tr id="no_print">		
					...
					<tr id="no_print">
						<td colspan="7">NOVA O.S.: <input type="text" name="cd_unidade" size="2" maxlength="2"  onKeyPress="ChecarTAB();" onKeyUp="Mostra(this, 2)" onFocus="PararTAB(this);"><input type="text" name="num_os" size="10" maxlength="10"><input type="submit" value="ok"></td>
						<td colspan="4"><img src="../imagens/ic_print.gif" alt="" width="24" height="24" border="0" onClick="window.print();"> &nbsp; &nbsp; &nbsp; &nbsp; <img src="../imagens/ic_print_view.gif" alt="" width="24" height="26" border="0" onclick="visualizarImpressao();"></td>
					</tr>
					</form>
					<tr id="no_print"><td colspan="100%"><img src="imagens/blackdot.gif" alt="" width="100%" height="1" border="0"></td></tr>
					<tr id="no_print">
				...
				<tr id="ok_print"><td colspan="100%"><img src="imagens/blackdot.gif" width="100%" height="2"></td></tr-->
					...
					<tr bgcolor="darkgray">
						...
						<td><img src="imagens/px.gif" width="3" height="1"></td>
						<td><b><a href="manutencao.asp?tipo=pendencias&filtro=pendentes&org=envio">DATA ENTRADA</a></b></td>
						<td><img src="imagens/px.gif" width="3" height="1"></td>
	...
	ordenacao = 1
		...
		cond_icoes = " AND (CONVERT(VARCHAR(8), GETDATE(), 112) <= CONVERT(varchar(8), dt_prev_retorno, 112))"
		cor_aviso = "black"
...
<%'2º
				...
				'***** Fim do contador *******************************
					...
					If mov = "retr" Then
						...
						condicoes = " Where num_os > "&regf&" AND nm_solicitante <> "&solicitante&" OR fecha = 0"
					...
					Elseif mov = "avan" Then
						...
						condicoes = " Where num_os < "&regl&" AND nm_solicitante <> '"&solicitante&"' OR fecha = 0"
...
<%'3º
				...
				'xsql = "SELECT "&selecao&" FROM "&tabela&" "&condicoes&" "&criterios&""
				xsql = "up_listagem @selecao='"&selecao&"', @tabela='"&tabela&"', @condicoes='"&condicoes&"', @criterios='"&criterios&"'"
			...
			Do while not rs.EOF
					...
					if year(dt_prev_retorno) = 1950 Then
						...
						if dt_prev_retorno <> "" Then
							...
							ano_prev_retorno = year(dt_prev_retorno)
								...
								dt_prev_retorno=zero(day(dt_prev_retorno))&"/"&zero(month(dt_prev_retorno))&"/"&year(dt_prev_retorno)
						end if
				...
				<%if cor_aviso = "red" AND item = 1 Then%><tr><td colspan="100%" align="center" id="ok_print">_______________________________________________ <b>Prazo do fornecedor vencido</b> _______________________________________________</td></tr><%end if%>
				...
				<tr onmouseover="mOvr(this,'#CFC8FF');"  onclick="javascript:;" onmouseout="mOut(this,'<%=cor_alerta%>');" bgcolor="<%=cor_alerta%>">
						<td valign="top" align="center" bgcolor="darkgray"><b><%=num%></b><%'="-"cd_alerta%></td>
						<td><img src="imagens/px.gif" width="1" height="1"></td>
						<td valign="top"><a href="manutencao.asp?tipo=andamento&cd_codigo=<%=cd_codigo%>&campo=cd_codigo"><font color="<%=cor_aviso%>"><%=zero(cd_unidade)%>.<%=manutencao(num_os)%></font></a></td>
						<!--td><img src="imagens/px.gif" width="1" height="1"></td>
						<td valign="top"><a href="../manut_os_andamento.asp?cd_codigo=<%=cd_codigo%>&campo=cd_codigo"><font color="<%=cor_aviso%>"><%'=dt_os%><%'=zero(day(dt_entrada))&"/"&zero(month(dt_entrada))&"/"&year(dt_entrada)%></font></a></td-->
						<!--td><img src="imagens/px.gif" width="1" height="1"></td>
						<td valign="top"><a href="manutencao.asp?tipo=andamento&cd_codigo=<%=cd_codigo%>&campo=cd_codigo"><font color="<%=cor_aviso%>"><%'=dt_entrada%><%if dt_manut_orc = "01/01/1950"  then%>//<%else%><%'=zero(day(dt_manut_orc))&"/"&zero(month(dt_manut_orc))&"/"&year(dt_manut_orc)%><%end if%></font></a></td-->
						<td><img src="imagens/px.gif" width="1" height="1"></td>
						<td valign="top"><font color="<%=cor_aviso%>"><%=zero(day(dt_os))&"/"&zero(month(dt_os))&"/"&year(dt_os)%><%'=dt_entrada%></font></td>
						<td><img src="imagens/px.gif" width="1" height="1"></td>
						...
						<td><img src="imagens/px.gif" width="1" height="1"></td>
						<td valign="top"><a href="manutencao.asp?tipo=andamento&cd_codigo=<%=cd_codigo%>&campo=cd_codigo"><font color="<%=cor_aviso%>"><%'=zero(day(dt_prev_retorno))&"/"&zero(month(dt_prev_retorno))&"/"&year(dt_prev_retorno)%><%=dt_prev_retorno%></font></a></td>
						<td><img src="imagens/px.gif" width="1" height="1"></td>
						...
						<td valign="top" id="no_print"><font color="<%=cor_aviso%>"><%=zero(day(dt_prev_retorno))&"/"&zero(month(dt_prev_retorno))&"/"&year(dt_prev_retorno)%></font></td>
...
loop%>
				...
				<!--table>
					<tr><td><%'=selecao&" "&tabela&" "&condicoes&" "&criterios%></td></tr>
					<tr>
						<td></td>
					</tr>
				</table-->
				<!--table>
					<tr>
						<td colspan="100%">Select <%'=selecao&" "&tabela&" "&condicoes&" "&criterios%></td>
					</tr>
				</table-->
