...
if questao = "quantidade" then
	...
	selecao = "SELECT COUNT('"&select_option&"') as conta, cd_unidade, fecha, "&codigo&", "&campos&""
...
<%if campo_14 <> 0 Then%><td>&nbsp;Situação</td><%end if%>
			...
			xsql = ""&selecao&" FROM VIEW_os_lista "&cond&" "&cond_periodo&" "&cond_fechada&" "&cond_ns&" "&cond_patrimonio&" "&cond_pat_vazio&" "&cond_eqp&" "&cond_und&" "&cond_forn&" "&cond_esp&" "&cond_aval&" "&cond_marca&" "&cond_fecha&" "&cond_valor&" "&cond_motivo&" "&cond_situacao&"  "&cond_os_assist&" "&agrupamento&" "&ordem&""'
...
<%if campo_0 <> 0 Then%><td valign="top"><a href="javascript:;" onclick="window.open('manutencao/manutencao_andamento.asp?cd_codigo=<%=cd_cod%>&campo=cd_codigo&visual=1&jan=1', 'principal', 'width=600, height=500,scrollbars=1'); return false;"><%=zero(cd_unidade)%>.<%=manutencao(num_os)%></a></td><%end if%>
