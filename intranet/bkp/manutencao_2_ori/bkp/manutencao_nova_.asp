	...
	Set rs_verifica = dbconn.execute(xsql)
		...
		loop
			if num_os_verifica = int(num_os) then 'Redireciona à O.S. existente
				...
				response.redirect("manutencao.asp?tipo=andamento&cd_codigo="&cd_codigo_verifica&"&campo=cd_codigo")
			End if
end if
'*************************************
			...
			<!--Início da etapa 1.1-->
				<tr>			
					<form method="post" action="acoes/manutencao_acao.asp" name="form"  id="form" onsubmit="return validar_os(document.form);">
						<input type="hidden" name="tipo" value="nova">
...
'						loop
			...
			<!--Início da etapa 1.2-->				
				...
				<%end if%>
				<tr><td colspan="100%"><!-- *** Inclui o registro de ocorrencias *** -->
