...
ordem_valor_orc = request("ordem_valor_orc")
ordem_motivo = request("ordem_motivo")
...
'	cond_ano = " AND (dt_ano BETWEEN '"&ano_i&"' AND '"&ano_f&"')"
	...
	'cond_motivo = " AND (nm_natureza_defeito = '"&nm_natureza_defeito&"')"
	...
	<tr>
		...
		<tr><td align="center">		
			<table border="1" bordercolor="#C0C0C0" cellpadding="0" cellspacing="0" bordercolordark="#FFFFFF" align="center">
				...
				<tr id="no_print" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'#e2e2e2');" bgcolor="#e2e2e2">
				...
				<tr id="no_print" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'');">
				...
				<tr id="no_print" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'');">
					...
					<input type="radio" name="cd_patrimonio_vazio" value="0" <%=patrimonio_vazio2%>> (vazios)
...
<!-- --------------------------------------------------------------------------------- -->				
				<tr id="no_print" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'');">
					...
					<select name="ordem_valor_orc" size="1">
						...
						<%numero = 1
						do while NOT numero = 12
						if int(ordem_valor_orc) = numero Then
							check = "selected"
						end if
						%>
						<option value="<%=numero%>" <%=check%>><%=numero%>º</option>
				...
				<tr id="no_print" onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'#e2e2e2');" bgcolor="#e2e2e2">
					...
					<!--select name="nm_natureza_defeito" class="inputs">
						<option value=""></option>
						<option value="1" <%if cd_nm_natureza_defeito = 1 then response.write("selected") end if%>>Motivo desconhecido</option>
						<option value="2" <%if cd_nm_natureza_defeito = 2 then response.write("selected") end if%>>Falha do operador</option>
						<option value="3" <%if cd_nm_natureza_defeito = 3 then response.write("selected") end if%>>Desgaste por uso</option>
						<option value="4" <%if cd_nm_natureza_defeito = 4 then response.write("selected") end if%>>Quebra por terceiros</option>
						<option value="5" <%if cd_nm_natureza_defeito = 5 then response.write("selected") end if%>>Solicitação de compras por terceiros</option>						
					</select--></td>
					<td><!--input type="text" name="ordem_equip" value="" size="1"-->
					<select name="ordem_motivo" size="1">
						<option value="0"></option>
						<%numero = 1
						do while NOT numero = 13
						if int(ordem_motivo) = numero Then
							check = "selected"
						end if
						%>
						<option value="<%=numero%>" <%=check%>><%=numero%>º</option>
...
<%if campo_14 <> 0 Then%><td>&nbsp;Situação</td><%end if%>
			...
			xsql = ""&selecao&" FROM VIEW_os_lista "&cond&" "&cond_periodo&" "&cond_ns&" "&cond_patrimonio&" "&cond_pat_vazio&" "&cond_eqp&" "&cond_und&" "&cond_forn&" "&cond_esp&" "&cond_aval&" "&cond_marca&" "&cond_fecha&" "&cond_valor&" "&cond_motivo&" "&cond_os_assist&" "&agrupamento&" "&ordem&" "&sentido&""'
...
<%if campo_8 <> 0 Then%><td><img src="imagens/px.gif" width="50" height="1"></td><%end if%>
<%if campo_12 <> 0 Then%><td><img src="imagens/px.gif" width="65" height="1"></td><%end if%>
<%if campo_13 <> 0 Then%><td><img src="imagens/px.gif" width="120" height="1"></td><%end if%>
<%if campo_14 <> 0 Then%><td><img src="imagens/px.gif" width="100" height="1"></td><%end if%>
				</tr>
		...
		<%=agrupamento%><br>
		 <%=ordem%> <%=sentido%><br><%="++ "&ordem_patrimonio&" ++"%><br>
