...
<table style="border-collapse:collapse; border:black solid 1px; height:20px; overflow:scroll;" cellspacing="1" >
	...
	<tr><td align="center" colspan="13"><input type="button" name="pagtos" value="Emitir pagamento" onclick="monta_pgto(document.diario.total_pgto.value);"></td></tr>
	<tr>
	...
	<tr bgcolor="<%=cor_linha%>"  onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=cor_linha%>');">
		...
		<td align="right" <%if cd_tipo_valor <> 1 Then%> onClick="manipulacao_valor('<%=cd_diario%>','<%=dt_vencimento_anterior%>','<%=dt_mes%>','<%=dt_ano%>','<%=str_acao%>','<%=cd_user%>')";><%end if%>
							<%if nm_valor <> "0" then 
								response.write(FormatNumber(nm_valor,2))
								'response.write(nm_valor)
							end if%></td>
							<%if nm_valor_extra = "0" Then 
								str_acao="inserir_valor"
							else
								str_acao = "editar_valor"
							end if%>
		<td align="right" <%if cd_tipo_valor = 1 Then%>onClick="manipulacao_valor('<%=cd_diario%>','<%=dt_vencimento_anterior%>','<%=dt_mes%>','<%=dt_ano%>','<%=str_acao%>','<%=cd_user%>')"; return false;<%end if%>><%if nm_valor_extra <> "0" then 
									response.write(FormatNumber(nm_valor_extra,2))
								end if%></td>
		<!--td align="right" <%'if cd_tipo_valor <> 1 Then%>><%'end if%>
							<%'if nm_valor <> "0" then 
								'response.write(FormatNumber(nm_valor_pg,2))
								'response.write(nm_valor)
							'end if%></td-->
		<!--td><%if cd_modo_pagamento > 0 then
			total_pago = total_pago + nm_valor + nm_valor_extra
		end if
		%></td-->
	...
	<tr>
		...
		<!--td>&nbsp;<%'if total_pago <> "" Then response.write(formatnumber(total_pago,2))%></td-->
		<!--td>&nbsp;<%%>Pagamento</td-->
	</tr>
	...
	<tr>
		...
		<td><img src="../imagens/px.gif" alt="" width="340" height="1" border="0"></td>
		<td><img src="../imagens/px.gif" alt="" width="15" height="1" border="0"></td>
