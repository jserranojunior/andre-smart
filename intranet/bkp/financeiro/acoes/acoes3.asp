...
IF x = "" THEN
	...
	ELSEIF acao = "insere_cheque" THEN
		'*** registra o cheque  (TBL_)***
			xsql = "up_financeiro_pagamento_insert3 @nm_lista_pagto='"&lista_pagto&"', @cd_conta_banco="&cd_conta_banco&", @nm_cheque="&nm_cheque&", @dt_pagto='"&dt_pagto&"', @cd_modo_pagto='"&modo_pagamento&"', @cd_status="&cd_status&", @cd_user="&cd_user&", @data_atual='"&data_atual&"'"
			dbconn.execute(xsql)
				'*** Seleciona o registro recém incluído 
	...
	if cd_fatura = "" then
			...
			Set rs = dbconn.execute(xsql)%>
				<%while not rs.EOF
					...
					SET rs_proc = dbconn.execute(xsql)
						while not rs_proc.EOF
							...
							'if qtd_proc <> "" then
								response.write(strcd_unidade&","&qtd_proc&","&cd_procedimento_tipo&":receb:"&dt_recebimento&"- fat:"&dt_faturamento&":"&cd_tipo_proc&"<br>")
				...
				rs.movenext
				wend
				response.redirect("../../financeiro.asp?tipo=fatura&cd_unidade="&cd_unidade&"&mes_sel="&mes_sel&"&ano_sel="&ano_sel&"")
				...
				<%while not rs.EOF
					...
					SET rs_proc = dbconn.execute(xsql)
						while not rs_proc.EOF
							...
							'if qtd_proc <> "" then
								response.write(cd_fatura&",U:"&strcd_unidade&",P:"&qtd_proc&",T:"&cd_procedimento_tipo&":receb:"&dt_recebimento&"- fat:"&dt_faturamento&":"&cd_tipo_proc&"<br>")
							
								'xsql = "up_financeiro_fatura_insert3 @cd_unidade="&strcd_unidade&", @qtd_proc='"&qtd_proc&"', @qtd_proc2='"&qtd_proc2&"', @qtd_proc3='"&qtd_proc3&"', @qtd_empr='"&qtd_empr&"', @dt_faturamento='"&dt_faturamento&"', @nm_obs='"&nm_obs&"', @cd_user="&cd_user&", @data_atual='"&data_atual&"'"
								'xsql = "up_financeiro_fatura_insert3 @cd_unidade="&strcd_unidade&", @cd_procedimento_tipo="&cd_procedimento_tipo&",    @qtd_proc='"&qtd_proc&"', @dt_faturamento='"&dt_faturamento&"', @dt_recebimento='"&dt_recebimento&"', @cd_user="&cd_user&", @data_atual='"&data_atual&"'"
								'xsql = "up_financeiro_fatura_update3 @cd_unidade="&strcd_unidade&", @cd_procedimento_tipo="&cd_procedimento_tipo&",    @qtd_proc='"&qtd_proc&"', @dt_faturamento='"&dt_faturamento&"', @dt_recebimento='"&dt_recebimento&"', @cd_user="&cd_user&", @data_atual='"&data_atual&"'"
