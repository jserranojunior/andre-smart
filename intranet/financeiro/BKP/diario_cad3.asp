...
<table border="1" align="center" style="font-size:12px; font-family:arial;">
	...
	<tr style="background-color:gray; font-size:16px; color:white;">
		<td colspan="3">Cadastramento de pagamentos - v3 <%'=str_origem%><%'="-"&acao%><%'=cd_os%><%'=cd_diario& "-" &dt_mes&"/1/"&dt_ano&" - "&dt_mes&"/"&dia_final&"/"&dt_ano%><%=dif_datas%> </td>		
	</tr>
	<%if cd_diario <> "" Then
		...
		if codigo_origem <> "" Then
			...
			cd_tipo_valor = rs_diario("cd_tipo_valor")
				...
				if not rs_tipopag.EOF then
					nm_tipo_pagamento = rs_tipopag("nm_tipo_pagamento")
					reponse.write(nm_tipo_pagamento&"ç")
				end if
			reponse.write(nm_tipo_pagamento&"ç")
	...
	'else%>
	<tr>
		<td>Tipo <%'=cd_tipo_valor%></td>
		<%if acao = "inserir_valor" OR acao = "editar_valor" then '*** Impede que apareçam campos desnecessários ***%>
		
			<td><%=nm_tipo_pagamento%>
			<input type="hidden" name="cd_tipo_valor" value="<%=cd_tipo_valor%>">
			<input type="hidden" name="cd_qtd_parcelas_atual" value="<%=cd_qtd_parcelas%>" size="2"> <%if cd_tipo_valor = 3 then%> (<%=cd_qtd_parcelas%>x)<%end if%>
		...
		<%else%>
			...
			<input type="hidden" name="cd_tipo_valor_atual" value="<%=cd_tipo_valor%>" size="3""><%'=cd_tipo_valor%></td>
			<td><span style="display:<%if cd_tipo_valor = 3 then%>block<%else%>none<%end if%>;" class="mostra_parcelas"><b>Qtd. Parcelas:</b>
				<%=cd_parcela%>/<%=cd_qtd_parcelas%>
		
				
				<input type="text" name="cd_qtd_parcelas" value="<%=cd_qtd_parcelas%>" size="2" maxlength="2" onkeyup="parcelamento()">
	...
	<tr>
		<td><%if acao = "editar_valor" OR str_origem <> "" Then%>Vencimento<%else%>Dia venc.<%end if%><%'=ordem_padrao%></td>
	...
	<tr bgcolor="#ebebeb">
		...
		<td align="center"><%if isnumeric(mes_final) AND isnumeric(ano_final) Then%>&nbsp;<%else%><img src="../imagens/ic_cancela.gif" alt="" width="15" height="15" border="0" onclick="mostra_cancelamento('<%=ano_inicial&mes_inicial&"_"&dt_ano&zero(dt_mes)%>');" title="Cancela o pagamento"><%end if%><%'=ano_inicial&mes_inicial&"_"&dt_ano&zero(dt_mes)%></td>
	</tr>
	<tr bgcolor="#ebebeb">
		<td>Final</td>
		<td colspan="2">
			<div id="divCanc_mostra">
				<%if isnumeric(mes_final) AND isnumeric(ano_final) Then%>
					...
					<%else%>
						<input type="text" name="mes_final" value="<%=mes_final%>" size="2" maxlength="2">/<input type="text" name="ano_final" value="<%=ano_final%>" size="4" maxlength="4">				
					<%end if%>
				<%else%>
					Conta ativa (Para cancelar clique no botão acima)
				<%end if%>
			</div></td>
	</tr>
	...
	<tr><td align="center" colspan="3">
		...
		<input type="submit" name="incluir" value="<%=nm_botao%>"><%'=cd_tipo_valor%><img src="../imagens/px.gif" alt="" width="80" height="1" border="0">
...
<SCRIPT LANGUAGE="javascript">
	...
	function JsDelete_registro(cod)
	   {
		if (confirm ("Tem certeza que deseja excluir este registro?")){
	  		if (confirm ("Esta conta será apagada dos meses anteriores também. Confirma?")){
				document.location.href('acoes/acoes3.asp?cd_diario='+cod+'&acao=excluir_pagamento_reg');
			}
		}
	}
	...
	function JsDelete_valor(cod){
		if (confirm ("Tem certeza que deseja excluir registro deste valor?")){
	  		document.location.href('acoes/acoes3.asp?cd_valor='+cod+'&acao=excluir_pagamento_valor');
		}
	}
</SCRIPT>
