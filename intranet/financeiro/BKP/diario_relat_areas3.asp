...
dia_f = request("dia_f")
	...
	if ano_f&zero(mes_f)&zero(dia_f) < ano_i&zero(mes_i)&zero(dia_i) then
		mes_f = mes_i
		ano_f = ano_i
		dia_f = dia_i
	end if
...
<table align="center" border="0" style="border-collapse:collapse; border:white solid 1px">
	<tr>
		<td align="center" bgcolor="#C0C0C0"><b>Relatório Áreas <%'=no_patrimonio_vazio%></b></td>
	</tr>
			...
			dt_os = dt_os_d&"/"&dt_os_m&"/"&dt_os_a%>
				...
				if compara1 <> grava_info AND sub_total_valor > 0 AND subtotais <> "" then%>
					<tr style="background-color:gray;">
						<td colspan="9<%'=espacos%>">&nbsp;<%'=espacos%></td>
