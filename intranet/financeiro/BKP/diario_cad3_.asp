...
<body onLoad="document.diario.cd_fornecedor.focus();" onUnload="window.opener.location.reload(true);">
...
<table border="1" align="center" style="font-size:12px; font-family:arial; border-collapse:collapse;>
		...
		<td><%strsql = "Select * From TBL_financeiro_tipo_pagamento order by nm_tipo_pagamento"
				...
				<select name="cd_tipo_valor" onclick="parcelado();" id="tipo_valor">
					...
					<%While not rs_tipopag.EOF
						...
						<option value="<%=cd_tipo_pagamento%>" <%if int(cd_tipo_valor) = cd_tipo_pagamento then%>SELECTED<%end if%>><%=nm_tipo_pagamento&","&cd_tipo_pagamento%></option>
					<%rs_tipopag.movenext
					wend
	...
	<tr>
		<td>&nbsp;<b>Vencimento<%=ordem_padrao%></b></td>
	...
	<tr>
		<td>&nbsp;<b>Valor</b></td><input type="hidden" name="cd_valor" value="<%=cd_valor%>">
