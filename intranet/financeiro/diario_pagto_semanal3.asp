...
<script language="javascript">
	function manipulacao_valor(cd_diario,dt_vencimento_anterior,dt_mes,dt_ano,acao,user){
		...
		mes_hoje = hoje.getMonth()+1;
			...
			if (data_informada<data_hoje || user == 46){
				window.alert ("O mês selecionado já está fechado.");  return (false);}
			else{	
				window.open('financeiro/diario_cad3.asp?cd_diario='+cd_diario+'&dia_sel='+dt_vencimento_anterior+'&mes_sel='+dt_mes+'&ano_sel='+dt_ano+'&acao='+acao+'','Cadastro','width=500,height=250,scrollbars=1');}
	}
	...
	function monta_pgto(lista_pgto){
		mywindow = window.open("financeiro/diario_pagamentos3.asp?lista_pagto="+lista_pgto+"&mes_sel=<%=dt_mes%>&ano_sel=<%=dt_ano%>", "Emissao_Cheques", "location=0,status=1,width=700,height=400,scrollbars=1");  
		mywindow.moveTo(0, 0);
}

</script>
<br class="no_print">
<table align="center" style="border:white solid 2px; border-collapse:collapse; position:relative;">
	<tr>
		<td valign="top">
...
<table style="border-collapse:collapse; border:black solid 1px; height:20px; overflow:scroll;" cellspacing="1" >
	<form action="../financeiro.asp" name="diario" id="diario" method="post">
	<input type="hidden" name="total_pgto" value="">
	<tr>
		<td align="center" colspan="13" style="font-size:15px; color:red;"><b>Contas de <%=mes_selecionado(dt_mes)%> de <%=dt_ano%></b></td>
	</tr>
	
	<%qtd_dias_semana = 7
	for i = 1 to dia_final
		
		pri_dia_semana = weekday(i&"/"&dt_mes&"/"&dt_ano)
		dif_dias = qtd_dias_semana - pri_dia_semana
		ult_dia_semana = i + dif_dias%>
		
		<tr>
			<td colspan="13" style="background-color:black; color:white;" align="center">SEMANA DE <%=i&" A "&ult_dia_semana%></td>
		</tr>
			<%xsql = "up_financeiro_diario3_lista_pagtos @dt_i='"&dt_mes&"/"&i&"/"&dt_ano&"', @dt_f='"&dt_mes&"/"&ult_dia_semana&"/"&dt_ano&"',@dia_i="&i&", @dia_f="&ult_dia_semana&", @dt_i_a='"&mes_anterior&"/"&i&"/"&ano_anterior&"', @dt_f_a='"&mes_anterior&"/"&dia_final_anterior&"/"&ano_anterior&"', @ordem="&ordem&""
	...
	<tr bgcolor="<%=cor_linha%>"  onmouseover="mOvr(this,'#CFC8FF');" onmouseout="mOut(this,'<%=cor_linha%>');">
		...
		if not IsNumeric(cd_valor) Then%>
			<img src="../../imagens/check_proibido.gif" alt="Inclua o valor" width="12" height="12" border="0" onClick="" return false;>
		...
		<td align="right" <%if cd_tipo_valor <> 1 Then%> onClick="manipulacao_valor('<%=cd_diario%>','<%=dt_vencimento_anterior%>','<%=zero(dt_mes)%>','<%=dt_ano%>','<%=str_acao%>','<%=cd_user%>')";><%end if%>
		
								<%if nm_valor <> "0" then 
									response.write(FormatNumber(nm_valor,2))
									'response.write(nm_valor)
								end if%></td>
		<td align="right" <%if cd_tipo_valor = 1 Then%>onClick="manipulacao_valor('<%=cd_diario%>','<%=dt_vencimento_anterior%>','<%=zero(dt_mes)%>','<%=dt_ano%>','<%=str_acao%>','<%=cd_user%>')"; return false;<%end if%>><%if nm_valor_extra <> "0" then 
		...
		<td align="center" bgcolor="<%=cor_variacao%>">&nbsp;<img src="../imagens/px.gif<%'=seta%>" alt="<%=variacao_valores%>%" width="10" height="10" border="0"></td>
	</tr>	
	<%
	...
	<%'if ult_dia < dia_final then
		i = ult_dia_semana' + 1
	'end if
	next
	
	
 	'pri_dia_semana = weekday(pri_dia_mes&"/"&dt_mes&"/"&dt_ano)
	'ultimo_dia
	%>
	...
	<tr>
		<td colspan="13"><br><br><br><br><br></td>
	</tr>
	...
	<tr style="background-color:gray; font-size:11px; color:white; cellspacing:1px;">
		<!--td align="center">&nbsp;</td>
	...
	<tr>
		...
		<td align="right" bgcolor="#c0c0c0"><b><%if soma_valores <> "" Then response.write(formatNumber(soma_valores_anteriores,2))%></b></td>
		<td>&nbsp;<%%></td>
	...
	<tr>
		...
		<td><img src="../imagens/px.gif" alt="" width="25" height="1" border="0"></td>
		<td><img src="../imagens/px.gif" alt="" width="370" height="1" border="0"></td>
