...
<table border="1" align="center" style="font-size:12px; font-family:arial;">
	...
	<tr style="background-color:gray; font-size:16px; color:white;">
		<td colspan="2">Cadastramento de pagamentos - v3 <%=acao%><%'=cd_diario& "-" &dt_mes&"/1/"&dt_ano&" - "&dt_mes&"/"&dia_final&"/"&dt_ano%></td>		
	</tr>
	...
	<tr>
		...
		<%if acao = "inserir" then%><%=ucase(left(mes_selecionado(dt_mes),3))&" / "&dt_ano%><%end if%>
	...
	<tr>
		...
		<span style="display:none;" class="mostra_parcelas"><b>Qtd. Parcelas:</b> <input type="text" name="cd_qtd_parcelas" value="<%=cd_qtd_parcelas%>" size="2" maxlength="2"> &nbsp;
		Atual:<input type="text" name="cd_parcela" value="<%=cd_parcela%>" size="2" maxlength="2"></span><%'=cd_parcela%></td>
	</tr>
	<%end if%>
	<!--tr>
		<td>Nota Fiscal</td>
	...
	<tr><td align="center" colspan="2">
		...
		<input type="submit" name="incluir" value="<%=nm_botao%>"> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; 
		<input type="button" name="Finalizar" value="Sair" onclick="window.close();"></td></tr>
	</form>
</table>
