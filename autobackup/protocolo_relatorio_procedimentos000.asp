		...
		if not rs.EOF then
				...
				if cd_unidade = 20 then 
					...
					a056_codrac = rs("a056_codrac")
						...
						strsql_conta_rack = "SELECT COUNT(A002_numpro) AS conta, A056_codrac FROM View_Protocolo_lista WHERE "&rack_cirurgiao&" A002_datpro BETWEEN '"&dt_mes&"/01/"&ano_sel&"' AND '"&dt_mes&"/"&UltimoDiaMes(dt_mes,ano_sel)&"/"&ano_sel&"' AND A053_codung = "&cd_unidade&" AND a056_codrac = "&a056_codrac&"  AND cd_co <> '1' AND cd_cancelado is null  GROUP BY A056_codrac"
	...
	'*************************************************************%>
		...
		<tr style="font-size:11px;">
			<td style="border:1px; solid red;" colspan="100%" align="center" >
				...
				<b><%=nm_unidade%></b><br>
				<b><%=mes_selecionado(int(dt_mes))%>/<%=ano_sel%></b> <%'=procedimentos%> <%'=emprestimos%></td>
		</tr>
		<tr><td colspan="100%"><img src="../imagens/blackdot.gif" alt="" width="100%" height="1" border="0"></td></tr>
...
'********************************************************************************************************
	...
	strsql = "SELECT a002_numpro, a002_datpro, a002_pacreg, a002_pacnom, a054_codcon, nm_convenio, a055_desmed, cd_protocolo, "&rack_ed&" a002_tipage = 'E' FROM VIEW_protocolo_lista Where "&rack_cirurgiao&" A002_datpro BETWEEN '"&dt_mes&"/1/"&ano_sel&"' AND '"&dt_mes&"/"&UltimoDiaMes(dt_mes,ano_sel)&"/"&ano_sel&"' AND a053_codung = '"&cd_unidade&"' AND a002_tipage <> 'E'  AND cd_co = '1' AND cd_cancelado is null  Group by a002_numpro,a002_datpro, a002_pacreg, a002_pacnom, a054_codcon, nm_convenio, a055_desmed, cd_protocolo, "&rack_ed&" a002_tipage ORDER BY "&rack_ed&" a002_datpro"
	Set rs = dbconn.execute(strsql)%>
		...
		if not rs.EOF then
				...
				if cd_unidade = 20 then 
					...
					a056_codrac = rs("a056_codrac")
						...
						strsql_conta_rack = "SELECT COUNT(A002_numpro) AS conta, A056_codrac FROM View_Protocolo_lista WHERE "&rack_cirurgiao&" A002_datpro BETWEEN '"&dt_mes&"/01/"&ano_sel&"' AND '"&dt_mes&"/"&UltimoDiaMes(dt_mes,ano_sel)&"/"&ano_sel&"' AND A053_codung = "&cd_unidade&" AND a056_codrac = "&a056_codrac&" AND cd_cancelado is null  GROUP BY A056_codrac"
						Set rs_conta_rack = dbconn.execute(strsql_conta_rack)
