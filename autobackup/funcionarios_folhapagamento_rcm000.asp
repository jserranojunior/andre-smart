...
arquivo_loc = "funcionarios_folhapagamento_rcm.asp"
...
dt_ano = request("dt_ano")
	if dt_dia = "" Then dt_dia = ultimodiames(dt_mes,dt_ano) end if
...
'*** Quantidade de domingos da competencia ***
	...
	next
			...
			if ordem_total <> "" Then
				...
				ordem_funcionarios = "cd_unidade, nm_nome, cd_contrato"
			end if
...
<!--#include file="../../includes/arquivo_loc.asp"-->
	...
	<%'if str_ok = "Visualizar" then%>
			...
			<!-- ---------------------------------------------------------------- -->				
							<tr bgcolor="#000000">
								<td colspan="19" align="center" class="cabecalho" style="background-color:black; color:white;"><b>RCM - <%=mesdoano(mes_competencia)%>/<%=ano_competencia%></b></td>
							</tr>
					...
					final_competencia_f = 1
					
					xsql = "up_funcionario_contrato_lista4 @dt_data_i='"&mes_competencia_i&"/"&dia_competencia_i&"/"&ano_competencia_i&"', @dt_data_f='"&mes_competencia_f&"/"&final_competencia_f&"/"&ano_competencia_f&"', @dt_atualizacao = '"&dt_mes&"/"&dia_final&"/"&dt_ano&"', @ordem_funcionarios='"&nm_ordem&"', @outras_variaveis='"&outras_variaveis&"'"
						Set rs = dbconn.execute(xsql)
							while not rs.EOF
								...
								'***** VT Cancelado **********************************************
								'if not isnull(dt_mes) OR not isnull(dt_dia) OR not isnull(dt_ano) Then
								if ultimodiames(dt_mes,dt_ano) = dt_dia then
							...
							if ultima_unidade =	"" OR ultima_unidade <> cd_unidade AND subtotal = 1 then
								...
								<tr>
                                    ...
                                    <td>&nbsp;</td>
									<td align="center"><b><%=subtotal_folga_enf%></b></td>
								</tr>
							...
							if strcd_unidade = 0 OR int(strcd_unidade) = cd_unidade then
								...
								if not rs_rcm.EOF then
                                    ...
                                    total_folga_enf = rs_rcm("total_folga_enf")
                                        ...
                                        subtotal_folga_enf = subtotal_folga_enf + total_folga_enf
                                            ...
                                            total_geral_folga_enf = total_geral_folga_enf + total_folga_enf
								end if
					...
					<%if cond_1 = 1 AND cond_2 = 1 then 
						...
						if ultima_unidade =	"" OR ultima_unidade <> cd_unidade then%>
							<tr><td align="center" colspan="19" style="background-color:gray; color:white;"><b><%=nm_unidade%></b></td></tr></tr>
							<tr style="background-color:gray; color:white;">
								...
								<td align="center"><b>Atrasos</b><br><img src="../../imagens/px.gif" alt="" width="40" height="1" border="0"></td>
                                    <td bgcolor="silver"><img src="../../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
								<td align="center"><b>Folga Enf.</b><br><img src="../../imagens/px.gif" alt="" width="40" height="1" border="0"></td>
							</tr>							
						<%subtotal = 1
						...
						<%variaveis1 = "empresa/funcionario/funcionarios_cadastro.asp?tipo=cadastro&cod="&strcod&"&func_ativos=S','cadastro','width=80,height=350,scrollbars=1,resizable=1" '
						variaveis2 = "empresa/funcionario/funcionarios_folhapagamento_rcm_cad.asp?tipo=cadastro&cod="&strcod&"&func_ativos=S&dt_dia="&dt_dia&"&dt_mes="&dt_mes&"&dt_ano="&dt_ano&"&dia_sel="&dia_sel&"&mes_sel="&mes_sel&"&ano_sel="&ano_sel&"&var=1&cat="&strcat&"','variaveis','width=650,height=510,scrollbars=1"%>
								...
								<td style="color:<%=cor_registro%>;" align="left" onClick="window.open('<%=variaveis1%>')"><%=nm_nome%> <%'=cor_aviso&" - "&total_he%><%'if cd_vt_canc > 0 then response.write("*")%><!-- &nbsp; --><%'if expediente = 6 then response.write("**")%></td>
                    ...
                    <td bgcolor="silver"><img src="../../imagens/px.gif" alt="" width="1" height="1" border="0"></td>
					<!-- *** Folga Enfermagem *** -->
								<td align="center" valign="top" onClick="window.open('<%=variaveis2%>')"><%if total_folga_enf > 0 then%><%=total_folga_enf%><%else%>0<%end if%></td>
